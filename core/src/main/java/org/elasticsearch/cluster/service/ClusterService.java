...
specifier|private
specifier|volatile
name|ClusterServiceTaskBatcher
name|taskBatcher
decl_stmt|;
...
name|discoverySettings
argument_list|,
literal|"please set discovery settings before starting"
argument_list|)
expr_stmt|;
name|addListener
argument_list|(
...
parameter_list|()
block|{     }
DECL|class|ClusterServiceTaskBatcher
class|class
name|ClusterServiceTaskBatcher
extends|extends
name|TaskBatcher
block|{
DECL|method|ClusterServiceTaskBatcher
name|ClusterServiceTaskBatcher
parameter_list|(
name|Logger
name|logger
...
begin_function
DECL|method|setDiscoverySettings
specifier|public
name|void
...
name|newClusterState
init|=
name|patchVersionsAndNoMasterBlocks
argument_list|(
name|previousClusterState
argument_list|,
name|clusterTasksResult
...
begin_function
DECL|method|patchVersionsAndNoMasterBlocks
specifier|private
name|ClusterState
name|patchVersionsAndNoMasterBlocks
parameter_list|(
...
name|previousClusterState
operator|:
literal|"state can only be changed by ClusterService when noMaster = true"
assert|;
if|if
condition|(
name|previousClusterState
...
condition|)
block|{
comment|// remove block if it already exists before adding new one
assert|assert
name|previousClusterState
operator|.
name|blocks
...
literal|false
operator|:
literal|"NO_MASTER_BLOCK should only be added by ClusterService"
assert|;
name|ClusterBlocks
name|clusterBlocks
init|=
...
expr_stmt|;
block|}
comment|// remove the no master block, if it exists
if|if
condition|(
name|newClusterState
operator|.
...
literal|false
operator|:
literal|"cluster state with master node must not have NO_MASTER_BLOCK"
assert|;
return|return
name|newClusterState
return|;
