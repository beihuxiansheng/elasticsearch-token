...
name|mapperRegistry
decl_stmt|;
DECL|field|indexScopedSettigns
specifier|private
specifier|final
name|IndexScopedSettings
name|indexScopedSettigns
decl_stmt|;
annotation|@
name|Inject
...
argument_list|()
operator|+
literal|"] was created before v2.0.0.beta1 and wasn't "
operator|+
literal|"upgraded."
operator|+
...
name|warn
argument_list|(
literal|"[{}] found unknown index setting: {} value: {} - archiving"
argument_list|,
name|indexMetaData
operator|.
name|getIndex
...
argument_list|)
expr_stmt|;
comment|// we put them back in here such that tools can check from the outside if there are any indices with broken
comment|// settings. The setting can remain there
comment|// but we want users to be aware that some of their setting are broken and they can research why and what they
comment|// need to do to replace them.
name|builder
operator|.
name|put
argument_list|(
...
name|warn
argument_list|(
literal|"[{}] found invalid index setting: {} value: {} - archiving"
argument_list|,
name|ex
argument_list|,
name|indexMetaData
...
argument_list|)
expr_stmt|;
comment|// we put them back in here such that tools can check from the outside if there are any indices with broken settings. The
comment|// setting can remain there
comment|// but we want users to be aware that some of their setting sare broken and they can research why and what they need to
comment|// do to replace them.
name|builder
operator|.
name|put
argument_list|(
