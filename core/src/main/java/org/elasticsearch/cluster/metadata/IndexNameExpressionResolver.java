...
return|;
block|}
comment|/**      * Iterates through the list of indices and selects the effective list of required aliases for the      * given index.      *<p>Only aliases where the given predicate tests successfully are returned. If the indices list contains a non-required reference to      * the index itself - null is returned. Returns<tt>null</tt> if no filtering is required.      */
DECL|method|indexAliases
specifier|public
name|String
index|[]
name|indexAliases
...
condition|)
block|{
comment|// If required - add it to the list of aliases
if|if
condition|(
name|aliases
operator|==
...
block|{
comment|// If not, we have a non required alias for this index - no futher checking needed
return|return
literal|null
return|;
block|}
...
condition|)
block|{
comment|// if there is a negation without a wildcard being previously seen, add it verbatim,
comment|// otherwise return the expression
if|if
condition|(
name|wildcardSeen
condition|)
