...
block|{
comment|// spawn to another thread to do in parallel
name|threadPool
operator|.
name|executor
argument_list|(
...
parameter_list|)
block|{
comment|// both errors and rejections are logged here. the service
comment|// will try again after `cluster.nodes.reconnect_interval` on all nodes but the current master.
comment|// On the master, node fault detection will remove these nodes from the cluster as their are not
comment|// connected. Note that it is very rare that we end up here on the master.
name|logger
operator|.
name|warn
argument_list|(
...
init|)
block|{
name|validateAndConnectIfNeeded
argument_list|(
name|node
argument_list|)
expr_stmt|;
...
DECL|method|disconnectFromNodesExcept
specifier|public
name|void
...
block|}
block|}
DECL|method|validateAndConnectIfNeeded
name|void
name|validateAndConnectIfNeeded
parameter_list|(
name|DiscoveryNode
...
argument_list|)
operator|:
literal|"validateAndConnectIfNeeded must be called under lock"
assert|;
if|if
condition|(
name|lifecycle
