...
argument_list|)
decl_stmt|;
comment|// as failing primaries also fail associated replicas, we fail replicas first here to avoid re-resolving replica ShardRouting
name|List
argument_list|<
name|FailedRerouteAllocation
operator|.
...
argument_list|()
operator|:
literal|"can only fail replicas for primary shard: "
operator|+
name|failedPrimary
assert|;
name|List
...
block|}
block|}
comment|/**      * Applies the relevant logic to handle a failed shard.      */
DECL|method|applyFailedShard
specifier|private
name|void
name|applyFailedShard
