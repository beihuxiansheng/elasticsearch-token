...
condition|)
block|{
comment|// only for unassigned - we filter allocation right after the index creation ie. for shard shrinking etc. to ensure
comment|// that once it has been allocated post API the replicas can be allocated elsewhere without user interaction
comment|// this is a setting that can only be set within the system!
name|IndexMetaData
name|indexMd
init|=
name|allocation
...
name|indexMd
operator|.
name|getInitialRecoveryFilters
argument_list|()
decl_stmt|;
if|if
condition|(
