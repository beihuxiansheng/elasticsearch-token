...
argument_list|)
expr_stmt|;
comment|// if this is a primary shard with ongoing replica recoveries, reinitialize them as their recovery source changed
if|if
condition|(
name|startedShard
operator|.
...
condition|)
block|{
comment|// find the relocation source
name|ShardRouting
name|sourceShard
init|=
name|getByAllocationId
...
argument_list|)
decl_stmt|;
comment|// cancel relocation and start relocation to same node again
name|ShardRouting
name|startedReplica
init|=
...
condition|)
block|{
comment|// promote active replica to primary if active replica exists (only the case for shadow replicas)
name|ShardRouting
name|activeReplica
init|=
name|activeReplica
...
parameter_list|)
block|{
comment|// if the activeReplica was relocating before this call to failShard, its relocation was cancelled earlier when we
...
literal|false
operator|:
literal|"shard must be a replica: "
operator|+
name|shard
assert|;
assert|assert
...
argument_list|()
operator|:
literal|"can only reinitialize an initializing replica: "
operator|+
name|shard
assert|;
assert|assert
...
literal|false
operator|:
literal|"replication target cannot be reinitialized: "
operator|+
name|shard
assert|;
name|ShardRouting
...
expr_stmt|;
block|}
comment|/**              * updates the unassigned info and recovery source on the current unassigned shard              *              * @param  unassignedInfo the new unassigned info to use              * @param  recoverySource the new recovery source to use              * @return the shard with unassigned info updated              */
