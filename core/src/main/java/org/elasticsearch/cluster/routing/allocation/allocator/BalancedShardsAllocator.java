...
name|NO
argument_list|,
literal|"cannot rebalance due to in-flight shard store fetches, otherwise allocation may prematurely rebalance a shard to "
operator|+
literal|"a node that is soon to receive another shard assignment upon completion of the shard store fetch, "
operator|+
literal|"rendering the cluster imbalanced again"
argument_list|)
return|;
block|}
name|sorter
...
block|{
continue|continue;
comment|// skip over node we're currently allocated to it
block|}
specifier|final
name|Decision
name|canAllocate
...
name|explain
argument_list|(
literal|"cannot allocate shard in its current state"
argument_list|)
argument_list|)
return|;
block|}
...
return|;
block|}
comment|// provide an explanation, if in explain mode
DECL|method|explain
specifier|private
name|String
name|explain
...
return|;
block|}
comment|/**          * Gets the individual node-level decisions that went into making the final decision as represented by          * {@link #getFinalDecisionType()}.  The map that is returned has the node id as the key and a {@link WeightedDecision}.          */
annotation|@
name|Nullable
...
return|;
block|}
comment|/**          * Gets the individual node-level decisions that went into making the final decision as represented by          * {@link #getFinalDecisionType()}.  The map that is returned has the node id as the key and a {@link NodeRebalanceDecision}.          */
annotation|@
name|Nullable
DECL|method|getNodeDecisions
specifier|public
...
DECL|class|NodeRebalanceDecision
specifier|public
specifier|static
specifier|final
class|class
...
name|weightWithShardAdded
decl_stmt|;
DECL|method|NodeRebalanceDecision
name|NodeRebalanceDecision
parameter_list|(
name|Type
name|nodeDecisionType
