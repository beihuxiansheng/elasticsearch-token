...
expr_stmt|;
block|}
DECL|method|maybeWarnFileDescriptors
specifier|private
name|void
name|maybeWarnFileDescriptors
parameter_list|()
...
block|}
block|}
comment|// a key for the shard lock. we can't use shardIds, because the contain
comment|// the index uuid, but we want the lock semantics to the same as we map indices to disk folders, i.e., without the uuid (for now).
DECL|class|ShardLockKey
specifier|private
specifier|final
class|class
name|ShardLockKey
