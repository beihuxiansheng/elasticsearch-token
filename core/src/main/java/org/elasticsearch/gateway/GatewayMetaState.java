...
argument_list|)
decl_stmt|;
comment|// We finished global state validation and successfully checked all indices for backward compatibility
comment|// and found no non-upgradable indices, which means the upgrade can continue.
comment|// Now it's safe to overwrite global and index metadata.
if|if
condition|(
name|metaData
operator|!=
...
name|Exception
block|{
comment|// upgrade index meta data
name|boolean
name|changed
init|=
literal|false
...
expr_stmt|;
block|}
comment|// upgrade global custom meta data
if|if
condition|(
name|applyPluginUpgraders
argument_list|(
name|metaData
operator|.
name|getCustoms
...
expr_stmt|;
block|}
comment|// upgrade current templates
if|if
condition|(
name|applyPluginUpgraders
argument_list|(
...
begin_function
DECL|method|applyPluginUpgraders
specifier|private
specifier|static
parameter_list|<
name|Data
...
condition|)
block|{
comment|// remove all data first so a plugin can remove custom metadata or templates if needed
name|existingMap
operator|.
name|keySet
argument_list|()
