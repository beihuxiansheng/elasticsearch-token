...
argument_list|()
operator|+
literal|" cannot have leading or trailing whitespace "
operator|+
literal|"["
operator|+
name|value
...
expr_stmt|;
block|}
comment|// this must be captured after the node name is possibly added to the settings
specifier|final
name|String
name|nodeName
init|=
...
argument_list|()
expr_stmt|;
comment|// TODO hack around circular dependencies problems in AllocationService
name|clusterModule
operator|.
name|getAllocationService
argument_list|()
...
name|debug
argument_list|(
literal|"initializing HTTP handlers ..."
argument_list|)
expr_stmt|;
name|actionModule
operator|.
...
block|}
block|}
comment|/**      * Checks for path.data and default.path.data being configured, and there being index data in any of the paths in default.path.data.      *      * @param settings the settings to check for path.data and default.path.data      * @param nodeEnv  the current node environment      * @param logger   a logger where messages regarding the detection will be logged      * @throws IOException if an I/O exception occurs reading the directory structure      */
DECL|method|checkForIndexDataInDefaultPathData
specifier|static
name|void
name|checkForIndexDataInDefaultPathData
parameter_list|(
...
if|if
condition|(
name|isDefaultPathDataInPathData
argument_list|(
name|nodeEnv
argument_list|,
name|defaultNodeDirectory
...
throw|;
block|}
DECL|method|isDefaultPathDataInPathData
specifier|private
specifier|static
name|boolean
name|isDefaultPathDataInPathData
...
name|reason
operator|=
literal|"read path that is not configured in environment"
argument_list|)
DECL|method|getPath
specifier|private
specifier|static
...
argument_list|)
operator|:
literal|"transportService has a different local node than the factory provided"
assert|;
...
argument_list|)
expr_stmt|;
comment|// start after transport service so the local disco is known
name|discovery
operator|.
name|start
argument_list|()
expr_stmt|;
comment|// start before cluster service so that it can set initial state on ClusterApplierService
name|clusterService
operator|.
name|start
argument_list|()
...
argument_list|)
operator|:
literal|"clusterService has a different local node than the factory provided"
assert|;
name|transportService
operator|.
...
argument_list|()
expr_stmt|;
comment|// close discovery early to not react to pings anymore.
comment|// This can confuse other nodes and delay things - mostly if we're the master and we're running tests.
name|injector
operator|.
name|getInstance
argument_list|(
...
argument_list|()
decl_stmt|;
DECL|field|persistentNodeId
specifier|private
specifier|final
name|String
name|persistentNodeId
decl_stmt|;
DECL|field|settings
specifier|private
specifier|final
