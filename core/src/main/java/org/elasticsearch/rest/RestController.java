...
name|headersToCopy
decl_stmt|;
DECL|field|isContentTypeRequired
specifier|private
specifier|final
name|boolean
name|isContentTypeRequired
decl_stmt|;
...
literal|0
operator|&&
name|hasContentTypeOrCanAutoDetect
argument_list|(
name|request
argument_list|,
name|handler
...
block|}
block|}
comment|/**      * If a request contains content, this method will return {@code true} if the {@code Content-Type} header is present, matches an      * {@link XContentType} or the request is plain text, and content type is required. If content type is not required then this method      * returns true unless a content type could not be inferred from the body and the rest handler does not support plain text      */
DECL|method|hasContentTypeOrCanAutoDetect
specifier|private
name|boolean
name|hasContentTypeOrCanAutoDetect
parameter_list|(
...
condition|)
block|{
comment|// content type of null with a handler that supports plain text gets through for now. Once we remove plain text this can
comment|// be removed!
name|deprecationLogger
operator|.
name|deprecated
argument_list|(
literal|"Plain text request bodies are deprecated. Use request parameters or body "
operator|+
literal|"in a supported format."
argument_list|)
expr_stmt|;
block|}
elseif|else
...
return|;
block|}
DECL|method|autoDetectXContentType
specifier|private
name|boolean
name|autoDetectXContentType
parameter_list|(
...
name|deprecated
argument_list|(
literal|"Content type detection for rest requests is deprecated. Specify the content type using "
operator|+
literal|"the [Content-Type] header."
argument_list|)
expr_stmt|;
name|XContentType
name|xContentType
