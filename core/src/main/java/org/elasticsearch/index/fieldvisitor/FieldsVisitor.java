...
condition|)
block|{
comment|// we can derive the exact type for the mapping
name|postProcess
argument_list|(
name|documentMapper
argument_list|)
...
block|}
block|}
comment|// can't derive exact mapping type
for|for
control|(
name|Map
operator|.
...
condition|)
block|{
comment|// it's possible index name doesn't match field name (legacy feature)
for|for
control|(
name|FieldMapper
name|mapper
...
condition|)
block|{
comment|// no index name or full name found, so skip
continue|continue;
block|}
block|}
name|List
