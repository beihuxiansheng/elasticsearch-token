...
name|Sort
argument_list|>
name|indexSortSupplier
decl_stmt|;
DECL|field|translogOpToEngineOpConverter
specifier|private
specifier|final
...
name|path
decl_stmt|;
DECL|field|indexShardOperationPermits
specifier|private
specifier|final
...
argument_list|()
operator|:
literal|"primary term can only be explicitly updated on a primary shard"
assert|;
...
literal|false
operator|:
literal|"a started primary shard should never update its term; "
operator|+
literal|"shard "
operator|+
name|shardRouting
...
name|primaryTerm
operator|:
literal|"primary terms can only go up; current term ["
operator|+
name|primaryTerm
operator|+
literal|"], new term ["
...
lambda|->
block|{
comment|// no shard operation permits are being held here, move state from started to relocated
assert|assert
name|indexShardOperationPermits
operator|.
...
return|;
block|}
comment|/**      * Applies an engine operation to the shard, which can be either an index, delete or noop operation.      */
DECL|method|applyOperation
specifier|public
name|Engine
operator|.
name|Result
...
return|;
block|}
DECL|method|extractUidForDelete
specifier|private
name|Term
name|extractUidForDelete
parameter_list|(
...
condition|)
block|{
comment|// This is only correct because we create types dynamically on delete operations
comment|// otherwise this could match the same _id from a different type
return|return
operator|new
name|Term
argument_list|(
...
DECL|method|runTranslogRecovery
name|int
name|runTranslogRecovery
parameter_list|(
name|Engine
name|engine
parameter_list|,
...
literal|false
operator|||
name|assertMaxUnsafeAutoIdInCommit
argument_list|()
assert|;
specifier|final
name|EngineConfig
...
block|}
block|}
DECL|method|assertMaxUnsafeAutoIdInCommit
specifier|private
name|boolean
name|assertMaxUnsafeAutoIdInCommit
parameter_list|()
...
argument_list|)
operator|:
literal|"opening index which was created post 5.5.0 but "
operator|+
name|InternalEngine
operator|.
name|MAX_UNSAFE_AUTO_ID_TIMESTAMP_COMMIT_ID
...
block|}
block|}
comment|/**      * Notifies the service to update the local checkpoint for the shard with the provided allocation ID. See      * {@link org.elasticsearch.index.seqno.GlobalCheckpointTracker#updateLocalCheckpoint(String, long)} for      * details.      *      * @param allocationId the allocation ID of the shard to update the local checkpoint for      * @param checkpoint   the local checkpoint for the shard      */
...
begin_decl_stmt
DECL|field|primaryTermMutex
specifier|private
specifier|final
name|Object
name|primaryTermMutex
init|=
operator|new
name|Object
argument_list|()
...
name|ROOT
argument_list|,
literal|"%s operation primary term [%d] is too old (current [%d])"
argument_list|,
name|shardId
argument_list|,
name|operationPrimaryTerm
...
begin_class
DECL|class|RefreshMetricUpdater
specifier|private
specifier|static
class|class
name|RefreshMetricUpdater
...
name|refreshMetric
decl_stmt|;
DECL|field|currentRefreshStartTime
specifier|private
name|long
name|currentRefreshStartTime
decl_stmt|;
...
literal|null
decl_stmt|;
DECL|method|RefreshMetricUpdater
specifier|private
name|RefreshMetricUpdater
parameter_list|(
name|MeanMetric
...
literal|null
operator|:
literal|"beforeRefresh was called by "
operator|+
name|callingThread
operator|.
name|getName
argument_list|()
operator|+
literal|" without a corresponding call to afterRefresh"
assert|;
name|callingThread
operator|=
name|Thread
...
literal|null
operator|:
literal|"afterRefresh called but not beforeRefresh"
assert|;
assert|assert
name|callingThread
operator|==
...
argument_list|()
operator|:
literal|"beforeRefreshed called by a different thread. current ["
operator|+
name|Thread
operator|.
name|currentThread
...
argument_list|()
operator|+
literal|"], thread that called beforeRefresh ["
operator|+
name|callingThread
operator|.
name|getName
