...
condition|)
block|{
comment|// we really only optimize this for testing since we use this to rewrite
comment|// named queries TemplateQueryBuilder and WrapperQueryBuilder.
comment|// it's equivalent but will anyways happen later down the road in the BQ#rewrite method
return|return
name|mustClauses
operator|.
name|get
