...
begin_comment
comment|/**  * This class provides the basic functionality needed for adding a decay  * function.  *  * This parser parses this kind of input  *  *<pre>  *<code>  * {  *      "fieldname1" : {  *          "origin" = "someValue",  *          "scale" = "someValue"  *      },  *      "multi_value_mode" : "min"  * }  *</code>  *</pre>  *  * "origin" here refers to the reference point and "scale" to the level of  * uncertainty you have in your origin.  *<p>  *  * For example, you might want to retrieve an event that took place around the  * 20 May 2010 somewhere near Berlin. You are mainly interested in events that  * are close to the 20 May 2010 but you are unsure about your guess, maybe it  * was a week before or after that. Your "origin" for the date field would be  * "20 May 2010" and your "scale" would be "7d".  *  * This class parses the input and creates a scoring function from the  * parameters origin and scale.  *<p>  * To write a new scoring function, create a new class that inherits from this  * one and implements {@link #getBuilderPrototype()} and {@link #getNames()}.  * Also create its corresponding {@link DecayFunctionBuilder}. The latter needs to  * implement {@link DecayFunctionBuilder#doReadFrom(StreamInput)} and  * {@link DecayFunctionBuilder#doWriteTo(StreamOutput)} for serialization purposes,  * {@link DecayFunctionBuilder#doEquals(DecayFunctionBuilder)} and  * {@link DecayFunctionBuilder#doHashCode()} for equality checks,  * {@link DecayFunctionBuilder#getName()} that returns the name of the function and  * {@link DecayFunctionBuilder#getDecayFunction()} which returns the corresponding lucene function.  *<p>  * See {@link GaussDecayFunctionBuilder} and {@link GaussDecayFunctionParser}  * for an example. The parser furthermore needs to be registered in the  * {@link org.elasticsearch.search.SearchModule SearchModule}.  *  */
end_comment
