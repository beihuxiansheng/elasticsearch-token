...
argument_list|)
decl_stmt|;
comment|/**      * Same as {@link #convert(List)} but on an {@link Iterable}.      */
DECL|method|convert
specifier|private
specifier|static
name|List
...
return|;
block|}
comment|/**      * Convert the internal {@link List} of values back to a user-friendly list.      * Integers are kept as-is since the terms query does not make any difference      * between {@link Integer}s and {@link Long}s, but {@link BytesRef}s are      * converted back to {@link String}s.      */
DECL|method|convertBack
specifier|static
name|List
argument_list|<
name|Object
...
expr_stmt|;
block|}
comment|// we do not convert longs, all integer types are equivalent
comment|// as far as this query is concerned
return|return
name|o
return|;
block|}
