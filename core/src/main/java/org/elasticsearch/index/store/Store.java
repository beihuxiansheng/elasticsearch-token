...
block|}
block|}
comment|/**      * Loads the local checkpoint and the maximum sequence number from the latest Lucene commit point and returns the triplet of local and      * global checkpoints, and maximum sequence number as an instance of {@link SeqNoStats}. The global checkpoint must be provided      * externally as it is not stored in the commit point.      *      * @param globalCheckpoint the provided global checkpoint      * @return an instance of {@link SeqNoStats} populated with the local and global checkpoints, and the maximum sequence number      * @throws IOException if an I/O exception occurred reading the latest Lucene commit point from disk      */
...
literal|true
operator|:
literal|"IW lock should not be obtained if there is a commit point available"
assert|;
comment|// if we lock the directory we also acquire the write lock since that makes sure that nobody else tries to lock the IW
comment|// on this store at the same time.
name|java
operator|.
name|util
operator|.
...
name|ParameterizedMessage
argument_list|(
literal|"{}: failed to obtain shard lock"
argument_list|,
name|shardId
argument_list|)
argument_list|,
...
block|}
block|}
comment|/**      * The returned IndexOutput validates the files checksum.      *<p>      * Note: Checksums are calculated by default since version 4.8.0. This method only adds the      * verification against the checksum in the given metadata and does not add any significant overhead.      */
