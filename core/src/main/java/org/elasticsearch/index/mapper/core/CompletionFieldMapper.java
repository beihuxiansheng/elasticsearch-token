...
name|contextMapping
decl_stmt|;
comment|/**      *       * @param contextMappings Configuration of context type. If none should be used set {@link ContextMapping.EMPTY_MAPPING}      * @param wrappedPostingsFormat the postings format to wrap, or {@code null} to wrap the codec's default postings format      */
comment|// Custom postings formats are deprecated but we still accept a postings format here to be able to test backward compatibility
comment|// with older postings formats such as Elasticsearch090
...
condition|)
block|{
comment|// delayed until postingsFormat() is called
name|this
operator|.
name|postingsFormat
operator|=
