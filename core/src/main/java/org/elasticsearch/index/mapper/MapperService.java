...
argument_list|)
expr_stmt|;
name|checkPercolatorFieldLimit
argument_list|(
name|fieldTypes
argument_list|)
expr_stmt|;
...
block|}
block|}
comment|/**      * We only allow upto 1 percolator field per index.      *      * Reasoning here is that the PercolatorQueryCache only supports a single document having a percolator query.      * Also specifying multiple queries per document feels like an anti pattern      */
DECL|method|checkPercolatorFieldLimit
specifier|private
name|void
name|checkPercolatorFieldLimit
parameter_list|(
...
operator|+
literal|"found the following percolator fields ["
operator|+
name|percolatorFieldTypes
operator|+
literal|"]"
