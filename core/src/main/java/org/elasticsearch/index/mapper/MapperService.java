...
argument_list|)
decl_stmt|;
DECL|field|INDEX_MAPPING_SINGLE_TYPE_SETTING
specifier|public
specifier|static
specifier|final
name|Setting
...
comment|/*boolean singleType = true;             if (settings.getAsVersion(IndexMetaData.SETTING_VERSION_CREATED, null) != null) {                 singleType = Version.indexCreated(settings).onOrAfter(Version.V_6_0_0_alpha1_UNRELEASED);             }*/
name|boolean
name|singleType
init|=
literal|false
...
block|}
block|}
comment|/**      * Update mapping by only merging the metadata that is different between received and stored entries      */
...
argument_list|)
operator|:
literal|"index mismatch: expected "
operator|+
name|index
argument_list|()
operator|+
...
try|try
block|{
comment|// only update entries if needed
name|updatedEntries
operator|=
name|internalMerge
argument_list|(
...
name|ParameterizedMessage
argument_list|(
literal|"[{}] failed to apply mappings"
argument_list|,
name|index
argument_list|()
argument_list|)
...
specifier|final
name|String
name|defaultMappingSourceOrLastStored
decl_stmt|;
if|if
condition|(
name|defaultMappingSource
...
throw|;
block|}
comment|// compute the merged DocumentMapper
name|DocumentMapper
name|oldMapper
init|=
name|mappers
...
expr_stmt|;
block|}
comment|// check basic sanity of the new mapping
name|List
argument_list|<
name|ObjectMapper
argument_list|>
...
argument_list|)
expr_stmt|;
name|checkPartitionedIndexConstraints
argument_list|(
name|newMapper
argument_list|)
expr_stmt|;
comment|// update lookup data-structures
...
expr_stmt|;
block|}
name|checkIndexSortCompatibility
argument_list|(
name|indexSettings
operator|.
...
condition|)
block|{
comment|// update both mappers and result
name|entry
operator|.
name|setValue
argument_list|(
...
name|IllegalArgumentException
argument_list|(
literal|"Rejecting mapping update to ["
operator|+
name|index
argument_list|()
operator|.
...
argument_list|()
operator|+
literal|"] as the final mapping would have more than 1 type: "
operator|+
name|actualTypes
argument_list|)
throw|;
block|}
block|}
comment|// make structures immutable
name|mappers
operator|=
name|Collections
operator|.
...
block|}
block|}
DECL|method|checkPartitionedIndexConstraints
specifier|private
name|void
name|checkPartitionedIndexConstraints
parameter_list|(
...
argument_list|()
operator|+
literal|"] cannot have a "
operator|+
literal|"_parent field for the partitioned index ["
operator|+
name|indexSettings
operator|.
name|getIndex
...
operator|+
literal|"required for partitioned index ["
operator|+
name|indexSettings
operator|.
name|getIndex
...
block|}
block|}
DECL|method|checkIndexSortCompatibility
specifier|private
name|void
name|checkIndexSortCompatibility
parameter_list|(
...
name|IllegalArgumentException
argument_list|(
literal|"cannot have nested fields when index sort is activated"
argument_list|)
throw|;
block|}
block|}
