...
parameter_list|)
function_decl|;
comment|/** Build a constant-scoring query that matches all values. The default implementation uses a      * {@link ConstantScoreQuery} around a {@link BooleanQuery} whose {@link Occur#SHOULD} clauses      * are generated with {@link #termQuery}. */
...
argument_list|()
operator|+
literal|"] does not support range queries"
argument_list|)
throw|;
block|}
...
name|IllegalArgumentException
argument_list|(
literal|"Can only use fuzzy queries on keyword and text fields - not on ["
operator|+
name|name
operator|+
...
name|context
argument_list|,
literal|"Can only use prefix queries on keyword and text fields - not on ["
operator|+
name|name
operator|+
literal|"] which is of type ["
...
name|context
argument_list|,
literal|"Can only use regexp queries on keyword and text fields - not on ["
operator|+
name|name
operator|+
literal|"] which is of type ["
...
return|;
block|}
comment|/**      * Extract a {@link Term} from a query created with {@link #termQuery} by      * recursively removing {@link BoostQuery} wrappers.      * @throws IllegalArgumentException if the wrapped query is not a {@link TermQuery}      */
...
name|IllegalArgumentException
argument_list|(
literal|"Cannot extract a term from a query of type "
operator|+
name|termQuery
operator|.
name|getClass
