...
parameter_list|)
block|{
comment|// HalfFloatPoint.nextDown considers that -0 is the same as +0
comment|// while point ranges are consistent with Float.compare, so
comment|// they consider that -0< +0, so we explicitly make sure
comment|// that nextDown(+0) returns -0
if|if
condition|(
name|Float
operator|.
...
parameter_list|)
block|{
comment|// HalfFloatPoint.nextUp considers that -0 is the same as +0
comment|// while point ranges are consistent with Float.compare, so
comment|// they consider that -0< +0, so we explicitly make sure
comment|// that nextUp(-0) returns +0
if|if
condition|(
name|Float
operator|.
...
parameter_list|)
block|{
comment|// Math.nextDown considers that -0 is the same as +0
comment|// while point ranges are consistent with Float.compare, so
comment|// they consider that -0< +0, so we explicitly make sure
comment|// that nextDown(+0) returns -0
if|if
...
parameter_list|)
block|{
comment|// Math.nextUp considers that -0 is the same as +0
comment|// while point ranges are consistent with Float.compare, so
comment|// they consider that -0< +0, so we explicitly make sure
comment|// that nextUp(-0) returns +0
if|if
...
block|{
comment|// Math.nextDown considers that -0 is the same as +0
comment|// while point ranges are consistent with Double.compare, so
comment|// they consider that -0< +0, so we explicitly make sure
comment|// that nextDown(+0) returns -0
if|if
condition|(
...
name|newMatchNoDocsQuery
argument_list|(
literal|"All values have a decimal part"
argument_list|)
return|;
block|}
if|if
...
argument_list|)
expr_stmt|;
comment|// if the lower bound is decimal:
comment|// - if the bound is positive then we increment it:
comment|//      if lowerTerm=1.5 then the (inclusive) bound becomes 2
comment|// - if the bound is negative then we leave it as is:
comment|//      if lowerTerm=-1.5 then the (inclusive) bound becomes -1 due to the call to longValue
name|boolean
name|lowerTermHasDecimalPart
init|=
name|hasDecimalPart
argument_list|(
name|lowerTerm
...
return|;
block|}
comment|/** Get the associated numeric type */
...
return|;
block|}
comment|/**          * Returns true if the object is a number and has a decimal part          */
DECL|method|hasDecimalPart
name|boolean
name|hasDecimalPart
parameter_list|(
name|Object
...
return|;
block|}
comment|/**          * Returns -1, 0, or 1 if the value is lower than, equal to, or greater than 0          */
DECL|method|signum
name|double
name|signum
parameter_list|(
