...
block|}
block|}
comment|/**      * Perform document index operation on the engine      * @param index operation to perform      * @return {@link IndexResult} containing updated translog location, version and      * document specific failures      *      * Note: engine level failures (i.e. persistent engine failures) are thrown      */
DECL|method|index
specifier|public
specifier|abstract
name|IndexResult
...
return|;
block|}
comment|/**          * Get the sequence number on the primary.          *          * @return the sequence number          */
...
name|IllegalStateException
argument_list|(
literal|"result is already frozen"
argument_list|)
throw|;
block|}
block|}
...
expr_stmt|;
block|}
comment|/**          * use in case of index operation failed before getting to internal engine          * (e.g while preparing operation or updating mappings)          * */
DECL|method|IndexResult
specifier|public
name|IndexResult
parameter_list|(
...
return|return;
block|}
comment|// this must happen before we close IW or Translog such that we can check this state to opt out of failing the engine
comment|// again on any caught AlreadyClosedException
name|failedEngine
operator|.
name|set
argument_list|(
...
name|Operation
block|{
comment|/** type of operation (index, delete), subclasses use static types */
DECL|enum|TYPE
specifier|public
enum|enum
name|TYPE
...
return|;
block|}
comment|/**          * Returns a positive timestamp if the ID of this document is auto-generated by elasticsearch.          * if this property is non-negative indexing code might optimize the addition of this document          * due to it's append only nature.          */
...
return|;
block|}
comment|/**          * Returns<code>true</code> if this index requests has been retried on the coordinating node and can therefor be delivered          * multiple times. Note: this might also be set to true if an equivalent event occurred like the replay of the transaction log          */
DECL|method|isRetry
specifier|public
name|boolean
name|isRetry
