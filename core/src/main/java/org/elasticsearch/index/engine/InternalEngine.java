...
argument_list|)
decl_stmt|;
DECL|field|MAX_UNSAFE_AUTO_ID_TIMESTAMP_COMMIT_ID
specifier|public
specifier|static
specifier|final
name|String
...
name|engineConfig
operator|.
name|isAutoGeneratedIDsOptimizationEnabled
argument_list|()
operator|==
literal|false
condition|)
...
argument_list|)
expr_stmt|;
name|updateMaxUnsafeAutoIdTimestampFromWriter
argument_list|(
name|writer
argument_list|)
expr_stmt|;
...
block|}
block|}
DECL|method|updateMaxUnsafeAutoIdTimestampFromWriter
specifier|private
name|void
name|updateMaxUnsafeAutoIdTimestampFromWriter
parameter_list|(
...
block|{
name|long
name|commitMaxUnsafeAutoIdTimestamp
init|=
name|Long
operator|.
name|MIN_VALUE
...
expr_stmt|;
block|}
DECL|method|sequenceNumberService
specifier|private
specifier|static
name|SequenceNumbersService
name|sequenceNumberService
...
try|try
block|{
name|recoverFromTranslogInternal
argument_list|()
expr_stmt|;
block|}
catch|catch
...
return|;
block|}
DECL|method|recoverFromTranslogInternal
specifier|private
name|void
name|recoverFromTranslogInternal
parameter_list|()
...
name|translogUUID
operator|=
name|loadTranslogUUIDFromCommit
argument_list|(
name|writer
argument_list|)
expr_stmt|;
...
annotation|@
name|Nullable
DECL|method|loadTranslogUUIDFromCommit
specifier|private
name|String
name|loadTranslogUUIDFromCommit
parameter_list|(
...
name|IllegalStateException
argument_list|(
literal|"commit doesn't contain translog generation id"
argument_list|)
throw|;
block|}
return|return
...
block|}
block|}
comment|/**      * the status of the current doc version in lucene, compared to the version in an incoming      * operation      */
DECL|enum|OpVsLuceneDocStatus
enum|enum
name|OpVsLuceneDocStatus
block|{
comment|/** the op is more recent than the one that last modified the doc found in lucene*/
DECL|enum constant|OP_NEWER
name|OP_NEWER
block|,
comment|/** the op is older or the same as the one that last modified the doc found in lucene*/
DECL|enum constant|OP_STALE_OR_EQUAL
name|OP_STALE_OR_EQUAL
block|,
comment|/** no doc was found in lucene */
DECL|enum constant|LUCENE_DOC_NOT_FOUND
name|LUCENE_DOC_NOT_FOUND
block|}
DECL|method|compareOpToLuceneDocBasedOnSeqNo
specifier|private
name|OpVsLuceneDocStatus
name|compareOpToLuceneDocBasedOnSeqNo
parameter_list|(
specifier|final
name|Operation
name|op
...
name|UNASSIGNED_SEQ_NO
operator|:
literal|"resolving ops based on seq# but no seqNo is found"
assert|;
specifier|final
name|OpVsLuceneDocStatus
name|status
...
condition|)
block|{
comment|// load term to tie break
specifier|final
name|long
name|existingTerm
init|=
...
DECL|method|resolveDocVersion
specifier|private
name|VersionValue
name|resolveDocVersion
parameter_list|(
specifier|final
name|Operation
name|op
...
argument_list|()
assert|;
comment|// used for asserting in tests
name|VersionValue
name|versionValue
init|=
name|versionMap
...
return|;
block|}
DECL|method|compareOpToLuceneDocBasedOnVersions
specifier|private
name|OpVsLuceneDocStatus
name|compareOpToLuceneDocBasedOnVersions
parameter_list|(
specifier|final
name|Operation
name|op
...
name|UNASSIGNED_SEQ_NO
operator|:
literal|"op is resolved based on versions but have a seq#"
assert|;
assert|assert
name|op
operator|.
...
literal|0
operator|:
literal|"versions should be non-negative. got "
operator|+
name|op
operator|.
name|version
...
literal|true
return|;
comment|// allow to optimize in order to update the max safe time stamp
default|default:
throw|throw
operator|new
name|IllegalArgumentException
...
return|;
block|}
DECL|method|assertVersionType
specifier|private
name|boolean
name|assertVersionType
parameter_list|(
specifier|final
name|Engine
operator|.
...
condition|)
block|{
comment|// ensure that replica operation has expected version type for replication
comment|// ensure that versionTypeForReplicationAndRecovery is idempotent
assert|assert
name|operation
operator|.
name|versionType
...
argument_list|()
operator|:
literal|"unexpected version type in request from ["
operator|+
name|operation
operator|.
name|origin
...
return|;
block|}
DECL|method|assertIncomingSequenceNumber
specifier|private
name|boolean
name|assertIncomingSequenceNumber
parameter_list|(
specifier|final
name|Engine
operator|.
...
return|;
block|}
DECL|method|assertSequenceNumberBeforeIndexing
specifier|private
name|boolean
name|assertSequenceNumberBeforeIndexing
parameter_list|(
specifier|final
name|Engine
operator|.
...
condition|)
block|{
comment|// sequence number should be set when operation origin is primary or when all shards are on new nodes
assert|assert
name|seqNo
operator|>=
literal|0
operator|:
literal|"ops should have an assigned seq no.; origin: "
operator|+
name|origin
assert|;
block|}
...
block|{
comment|// non-primary mode (i.e., replica or recovery)
name|plan
operator|=
name|planIndexingAsNonPrimary
argument_list|(
name|index
argument_list|)
expr_stmt|;
...
name|plan
operator|.
name|earlyResultOnPreFlightError
operator|.
name|isPresent
argument_list|()
condition|)
...
name|plan
operator|.
name|versionForIndexing
argument_list|,
name|plan
operator|.
name|seqNoForIndexing
argument_list|,
name|plan
operator|.
name|currentNotFoundOrDeleted
argument_list|)
expr_stmt|;
block|}
if|if
...
condition|)
block|{
comment|// if we have document failure, record it as a no-op in the translog with the generated seq_no
name|location
operator|=
name|translog
operator|.
...
block|}
block|}
DECL|method|planIndexingAsNonPrimary
specifier|private
name|IndexingStrategy
name|planIndexingAsNonPrimary
parameter_list|(
...
condition|)
block|{
comment|// no need to deal with out of order delivery - we never saw this one
assert|assert
name|index
operator|.
name|version
...
literal|1L
operator|:
literal|"can optimize on replicas but incoming version is ["
operator|+
name|index
operator|.
name|version
...
block|{
comment|// drop out of order operations
assert|assert
name|index
operator|.
name|versionType
...
argument_list|()
operator|:
literal|"resolving out of order delivery based on versioning but version type isn't fit for it. got ["
operator|+
name|index
operator|.
name|versionType
...
literal|"]"
assert|;
comment|// unlike the primary, replicas don't really care to about creation status of documents
...
block|{
comment|// This can happen if the primary is still on an old node and send traffic without seq# or we recover from translog
comment|// created by an old version.
assert|assert
name|config
argument_list|()
operator|.
...
argument_list|)
operator|:
literal|"index is newly created but op has no sequence numbers. op: "
operator|+
name|index
assert|;
name|opVsLucene
...
return|;
block|}
DECL|method|planIndexingAsPrimary
specifier|private
name|IndexingStrategy
name|planIndexingAsPrimary
parameter_list|(
...
name|PRIMARY
operator|:
literal|"planing as primary but origin isn't. got "
operator|+
name|index
operator|.
name|origin
...
name|plan
decl_stmt|;
comment|// resolve an external operation into an internal one which is safe to replay
if|if
condition|(
name|canOptimizeAddDocument
argument_list|(
...
name|IndexingStrategy
operator|.
name|overrideExistingAsIfNotThere
argument_list|(
name|seqNoService
argument_list|()
operator|.
...
block|{
comment|// resolves incoming version
specifier|final
name|VersionValue
name|versionValue
init|=
...
literal|0
operator|:
literal|"version must be set. got "
operator|+
name|plan
operator|.
name|versionForIndexing
...
block|{
comment|// document does not exists, we can optimize for create, but double check if assertions are running
assert|assert
name|assertDocDoesNotExist
argument_list|(
name|index
argument_list|,
name|canOptimizeAddDocument
...
block|}
block|}
comment|/**      * returns true if the indexing operation may have already be processed by this engine.      * Note that it is OK to rarely return true even if this is not the case. However a `false`      * return value must always be correct.      *      */
DECL|method|mayHaveBeenIndexedBefore
specifier|private
name|boolean
name|mayHaveBeenIndexedBefore
parameter_list|(
...
name|IndexingStrategy
block|{
DECL|field|currentNotFoundOrDeleted
specifier|final
name|boolean
name|currentNotFoundOrDeleted
decl_stmt|;
DECL|field|useLuceneUpdateDocument
specifier|final
name|boolean
name|useLuceneUpdateDocument
decl_stmt|;
DECL|field|seqNoForIndexing
specifier|final
name|long
name|seqNoForIndexing
decl_stmt|;
DECL|field|versionForIndexing
specifier|final
name|long
name|versionForIndexing
decl_stmt|;
DECL|field|indexIntoLucene
specifier|final
name|boolean
name|indexIntoLucene
decl_stmt|;
DECL|field|earlyResultOnPreFlightError
specifier|final
name|Optional
argument_list|<
name|IndexResult
...
name|indexIntoLucene
operator|:
literal|"use lucene update is set to true, but we're not indexing into lucene"
assert|;
assert|assert
operator|(
name|indexIntoLucene
...
literal|false
operator|:
literal|"can only index into lucene or have a preflight result but not both."
operator|+
literal|"indexIntoLucene: "
operator|+
name|indexIntoLucene
operator|+
literal|"  earlyResultOnPreFlightError:"
operator|+
name|earlyResultOnPreFlightError
assert|;
name|this
...
expr_stmt|;
block|}
DECL|method|optimizedAppendOnly
specifier|static
name|IndexingStrategy
name|optimizedAppendOnly
parameter_list|(
...
return|;
block|}
DECL|method|skipDueToVersionConflict
specifier|static
name|IndexingStrategy
name|skipDueToVersionConflict
parameter_list|(
...
return|;
block|}
DECL|method|overrideExistingAsIfNotThere
specifier|static
name|IndexingStrategy
name|overrideExistingAsIfNotThere
parameter_list|(
...
return|;
block|}
DECL|method|processButSkipLucene
specifier|static
name|IndexingStrategy
name|processButSkipLucene
parameter_list|(
...
block|}
block|}
comment|/**      * Asserts that the doc in the index operation really doesn't exist      */
DECL|method|assertDocDoesNotExist
specifier|private
name|boolean
name|assertDocDoesNotExist
parameter_list|(
...
argument_list|()
operator|+
literal|"] exists in version map (version "
operator|+
name|versionValue
operator|+
literal|")"
...
name|acquireSearcher
argument_list|(
literal|"assert doc doesn't exist"
argument_list|)
init|)
block|{
specifier|final
...
name|plan
operator|.
name|versionOfDeletion
argument_list|,
name|plan
operator|.
name|seqNoOfDeletion
argument_list|,
name|plan
operator|.
...
return|;
block|}
DECL|method|planDeletionAsNonPrimary
specifier|private
name|DeletionStrategy
name|planDeletionAsNonPrimary
parameter_list|(
...
name|PRIMARY
operator|:
literal|"planing as primary but got "
operator|+
name|delete
operator|.
name|origin
...
literal|"]"
assert|;
comment|// unlike the primary, replicas don't really care to about found status of documents
comment|// this allows to ignore the case where a document was found in the live version maps in
...
return|;
block|}
DECL|method|planDeletionAsPrimary
specifier|private
name|DeletionStrategy
name|planDeletionAsPrimary
parameter_list|(
...
argument_list|()
assert|;
comment|// resolve operation from external to internal
specifier|final
name|VersionValue
name|versionValue
init|=
...
condition|)
block|{
comment|// any exception that comes from this is a either an ACE or a fatal exception there
comment|// can't be any document failures  coming from this
name|indexWriter
operator|.
...
condition|)
block|{
comment|// there is no tragic event and such it must be a document level failure
return|return
operator|new
name|DeleteResult
argument_list|(
...
name|DeletionStrategy
block|{
comment|// of a rare double delete
DECL|field|deleteFromLucene
specifier|final
name|boolean
name|deleteFromLucene
decl_stmt|;
DECL|field|currentlyDeleted
specifier|final
name|boolean
name|currentlyDeleted
decl_stmt|;
DECL|field|seqNoOfDeletion
specifier|final
name|long
name|seqNoOfDeletion
decl_stmt|;
DECL|field|versionOfDeletion
specifier|final
name|long
name|versionOfDeletion
decl_stmt|;
...
literal|false
operator|:
literal|"can only delete from lucene or have a preflight result but not both."
operator|+
literal|"deleteFromLucene: "
operator|+
name|deleteFromLucene
operator|+
literal|"  earlyResultOnPreFlightError:"
...
block|}
comment|// testing
DECL|method|clearDeletedTombstones
name|void
...
name|error
argument_list|(
literal|"tragic event in index writer"
argument_list|,
name|ex
argument_list|)
expr_stmt|;
...
name|trace
argument_list|(
literal|"committing writer with commit data [{}]"
argument_list|,
name|commitData
argument_list|)
expr_stmt|;
...
return|;
block|}
comment|/**      * Gets the commit data from {@link IndexWriter} as a map.      */
DECL|method|commitDataAsMap
specifier|private
specifier|static
name|Map
argument_list|<
