...
name|seqNoStats
operator|=
name|loadSeqNoStatsFromLuceneAndTranslog
argument_list|(
name|engineConfig
operator|.
...
return|;
block|}
comment|/**      * Reads the sequence number stats from the Lucene commit point (maximum sequence number and local checkpoint) and the translog      * checkpoint (global checkpoint).      *      * @param translogConfig the translog config (for the global checkpoint)      * @param indexWriter    the index writer (for the Lucene commit point)      * @return the sequence number stats      * @throws IOException if an I/O exception occurred reading the Lucene commit point or the translog checkpoint      */
DECL|method|loadSeqNoStatsFromLuceneAndTranslog
specifier|private
specifier|static
name|SeqNoStats
name|loadSeqNoStatsFromLuceneAndTranslog
...
block|{
comment|/* Version conflicts during recovery and on replicas are normal due to asynchronous execution; as such, we should return a                  * successful result.*/
return|return
literal|true
return|;
block|}
...
return|;
block|}
DECL|method|assertSequenceNumber
specifier|private
name|boolean
name|assertSequenceNumber
parameter_list|(
specifier|final
name|Engine
operator|.
...
block|}
block|}
DECL|method|isForceUpdateDocument
specifier|private
name|boolean
name|isForceUpdateDocument
parameter_list|(
...
comment|// any exception that comes from this is a either an ACE or a fatal exception there can't be any document failures coming
comment|// from this.
name|indexWriter
operator|.
