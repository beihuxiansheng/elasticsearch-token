...
name|trace
argument_list|(
literal|"recovering max sequence number: [{}], local checkpoint: [{}], global checkpoint: [{}]"
argument_list|,
name|seqNoStats
operator|.
name|getMaxSeqNo
...
return|;
block|}
comment|/**      * Reads the sequence number stats from the Lucene commit point (maximum sequence number and local checkpoint) and the Translog      * checkpoint (global checkpoint).      *      * @param engineConfig the engine configuration (for the open mode and the translog path)      * @param writer       the index writer (for the Lucene commit point)      * @return the sequence number stats      * @throws IOException if an I/O exception occurred reading the Lucene commit point or the translog checkpoint      */
DECL|method|loadSeqNoStats
specifier|private
specifier|static
name|SeqNoStats
name|loadSeqNoStats
