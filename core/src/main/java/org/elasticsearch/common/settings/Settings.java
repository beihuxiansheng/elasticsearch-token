...
name|settings
decl_stmt|;
comment|/** The secure settings storage associated with these settings. */
DECL|field|secureSettings
specifier|private
specifier|final
name|SecureSettings
name|secureSettings
decl_stmt|;
comment|/** The first level of setting names. This is constructed lazily in {@link #names()}. */
DECL|field|firstLevelNames
specifier|private
specifier|final
name|SetOnce
argument_list|<
...
argument_list|()
decl_stmt|;
comment|/**      * Setting names found in this Settings for both string and secure settings.      * This is constructed lazily in {@link #keySet()}.      */
DECL|field|keys
specifier|private
specifier|final
name|SetOnce
...
expr_stmt|;
block|}
comment|/**      * Retrieve the secure settings in these settings.      */
...
parameter_list|()
block|{
comment|// settings is always unmodifiable
return|return
name|this
operator|.
name|settings
...
return|;
block|}
comment|// TODO #22298: Delete this method and update call sites to<code>#getAsBoolean(String, Boolean)</code>.
comment|/**      * Returns the setting value (as boolean) associated with the setting key. If it does not exist, returns the default value provided.      * If the index was created on Elasticsearch below 6.0, booleans will be parsed leniently otherwise they are parsed strictly.      *      * See {@link Booleans#isBooleanLenient(char[], int, int)} for the definition of a "lenient boolean"      * and {@link Booleans#isBoolean(char[], int, int)} for the definition of a "strict boolean".      *      * @deprecated Only used to provide automatic upgrades for pre 6.0 indices.      */
annotation|@
name|Deprecated
DECL|method|getAsBooleanLenientForPreEs6Indices
specifier|public
name|Boolean
...
condition|)
block|{
comment|//Only emit a warning if the setting's value is not a proper boolean
specifier|final
name|String
name|value
init|=
...
name|deprecated
argument_list|(
literal|"The value [{}] of setting [{}] is not coerced into boolean anymore. Please change "
operator|+
literal|"this value to [{}]."
argument_list|,
name|value
argument_list|,
name|setting
...
name|ROOT
argument_list|,
literal|"settings object contains values for [%s=%s] and [%s=%s]"
argument_list|,
name|settingPrefix
argument_list|,
name|valueFromPrefix
...
argument_list|)
decl_stmt|;
comment|// uniquify, since for legacy reasons the same setting name may exist in both
name|keys
operator|.
name|set
argument_list|(
...
name|IllegalStateException
argument_list|(
literal|"Secure settings must already be loaded"
argument_list|)
throw|;
block|}
name|this
...
block|}
block|}
comment|// TODO We could use an FST internally to make things even faster and more compact
DECL|class|FilteredMap
specifier|private
specifier|static
specifier|final
class|class
...
name|prefix
decl_stmt|;
comment|// we cache that size since we have to iterate the entire set
comment|// this is safe to do since this map is only used with unmodifiable maps
DECL|field|size
specifier|private
name|int
name|size
...
literal|true
return|;
comment|// protect against calling hasNext twice
block|}
else|else
block|{
if|if
...
condition|)
block|{
comment|// protect against no #hasNext call or not respecting it
throw|throw
operator|new
name|NoSuchElementException
argument_list|(
literal|"make sure to call hasNext first"
argument_list|)
throw|;
block|}
specifier|final
...
block|}
block|}
DECL|class|PrefixedSecureSettings
specifier|private
specifier|static
class|class
name|PrefixedSecureSettings
...
argument_list|()
decl_stmt|;
DECL|method|PrefixedSecureSettings
name|PrefixedSecureSettings
parameter_list|(
name|SecureSettings
name|delegate
