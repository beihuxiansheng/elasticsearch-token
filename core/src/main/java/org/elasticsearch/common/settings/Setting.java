...
block|}
block|}
comment|/**      * Add this setting to the builder if it doesn't exists in the source settings.      * The value added to the builder is taken from the given default settings object.      * @param builder the settings builder to fill the diff into      * @param source the source settings object to diff      * @param defaultSettings the default settings object to diff against      */
DECL|method|diff
specifier|public
name|void
name|diff
...
block|}
block|}
comment|/**      * Updates settings that depend on each other.      * See {@link AbstractScopedSettings#addSettingsUpdateConsumer(Setting, Setting, BiConsumer)} and its usage for details.      */
...
parameter_list|)
block|{
comment|// we collect all concrete keys and then delegate to the actual setting for validation and settings extraction
specifier|final
name|Map
argument_list|<
name|AbstractScopedSettings
...
condition|)
block|{
comment|// only the ones that have changed otherwise we might get too many updates
comment|// the hasChanged above checks only if there are any changes
name|T
name|value
init|=
name|updater
...
argument_list|(
literal|"affix settings can't return values"
operator|+
literal|" use #getConcreteSetting to obtain a concrete setting"
argument_list|)
throw|;
block|}
annotation|@
...
block|}
block|}
comment|/**          * Get a setting with the given namespace filled in for prefix and suffix.          */
DECL|method|getConcreteSettingForNamespace
specifier|public
name|Setting
argument_list|<
name|T
...
expr_stmt|;
block|}
comment|/**          * Returns the namespace for a concrete settting. Ie. an affix setting with prefix:<tt>search.</tt> and suffix:<tt>username</tt>          * will return<tt>remote</tt> as a namespace for the setting<tt>search.remote.username</tt>          */
...
return|;
block|}
comment|/**          * Returns a stream of all concrete setting instances for the given settings. AffixSetting is only a specification, concrete          * settings depend on an actual set of setting keys.          */
DECL|method|getAllConcreteSettings
specifier|public
name|Stream
argument_list|<
name|Setting
...
name|IllegalArgumentException
argument_list|(
literal|"default value function must not return null"
argument_list|)
throw|;
block|}
name|Function
...
begin_comment
comment|/**      * This setting type allows to validate settings that have the same type and a common prefix and suffix. For instance      * storage.${backend}.enable=[true|false] can easily be added with this setting. Yet, affix key settings don't support updaters      * out of the box unless {@link #getConcreteSetting(String)} is used to pull the updater.      */
end_comment
...
begin_comment
comment|/**      * A key that allows for static pre and suffix. This is used for settings      * that have dynamic namespaces like for different accounts etc.      */
end_comment
...
block|{
comment|// the last part of this regexp is for lists since they are represented as x.${namespace}.y.1, x.${namespace}.y.2
name|pattern
operator|=
name|Pattern
operator|.
...
return|;
block|}
comment|/**          * Returns a string representation of the concrete setting key          */
DECL|method|getConcreteString
name|String
name|getConcreteString
parameter_list|(
name|String
...
name|IllegalStateException
argument_list|(
literal|"can't get concrete string for key "
operator|+
name|key
operator|+
literal|" key doesn't match"
argument_list|)
throw|;
block|}
return|return
