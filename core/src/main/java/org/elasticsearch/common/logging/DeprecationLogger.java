...
parameter_list|)
block|{
comment|/*          * We know the exact format of the warning header, so to extract the warning value we can skip forward from the front to the first          * quote, and skip backwards from the end to the penultimate quote:          *          *   299 Elasticsearch-6.0.0 "warning value" "Sat, 25, Feb 2017 10:27:43 GMT"          *                           ^               ^                              ^          *                           firstQuote      penultimateQuote               lastQuote          *          * We do it this way rather than seeking forward after the first quote because there could be escaped quotes in the warning value          * but since there are none in the warning date, we can skip backwards to find the quote that closes the quoted warning value.          *          * We parse this manually rather than using the capturing regular expression because the regular expression involves a lot of          * backtracking and carries a performance penalty. However, when assertions are enabled, we still use the regular expression to          * verify that we are maintaining the warning header format.          */
specifier|final
name|int
name|firstQuote
init|=
...
return|;
block|}
comment|/**      * Assert that the specified string has the warning value equal to the provided warning value.      *      * @param s            the string representing a full warning header      * @param warningValue the expected warning header      * @return {@code true} if the specified string has the expected warning value      */
DECL|method|assertWarningValue
specifier|private
specifier|static
name|boolean
name|assertWarningValue
