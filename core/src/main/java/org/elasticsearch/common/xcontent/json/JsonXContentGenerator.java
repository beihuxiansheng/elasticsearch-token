...
name|includes
argument_list|,
literal|"Including filters must not be null"
argument_list|)
expr_stmt|;
name|Objects
operator|.
...
name|excludes
argument_list|,
literal|"Excluding filters must not be null"
argument_list|)
expr_stmt|;
name|this
operator|.
...
return|;
block|}
DECL|method|getLowLevelGenerator
specifier|private
name|JsonGenerator
name|getLowLevelGenerator
parameter_list|()
block|{
if|if
condition|(
...
condition|)
block|{
comment|// In case of combined inclusion and exclusion filters, we have one and only one another delegating level
name|delegate
operator|=
operator|(
operator|(
...
condition|)
block|{
comment|// Use the low level generator to write the startObject so that the root
comment|// start object is always written even if a filtered generator is used
name|getLowLevelGenerator
argument_list|()
operator|.
name|writeStartObject
...
operator|)
operator|:
literal|"Expected an instance of JsonWriteContext but was: "
operator|+
name|context
operator|.
name|getClass
...
condition|)
block|{
comment|// If we've just started a field we'll need to add the separator
name|generator
operator|.
name|writeRaw
argument_list|(
...
argument_list|()
expr_stmt|;
comment|// Bypass generator to always write the line feed
name|getLowLevelGenerator
argument_list|()
operator|.
name|writeRaw
