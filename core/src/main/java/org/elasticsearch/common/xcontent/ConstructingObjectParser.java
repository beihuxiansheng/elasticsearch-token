...
argument_list|>
block|{
comment|/**      * Consumer that marks a field as a constructor argument instead of a real object field.      */
DECL|field|CONSTRUCTOR_ARG_MARKER
specifier|private
specifier|static
specifier|final
name|BiConsumer
...
DECL|field|constructorArgNames
specifier|private
specifier|final
name|List
argument_list|<
...
return|;
block|}
comment|// The object hasn't been built which ought to mean we're missing some constructor arguments.
name|StringBuilder
name|message
init|=
literal|null
...
block|}
block|}
comment|/*              * There won't be if there weren't any constructor arguments declared. That is fine, we'll just throw that error back at the to              * the user. This will happen every time so we can be confident that this'll be caught in testing so we can talk to the user              * like they are a developer. The only time a user will see this is if someone writes a parser and never tests it which seems              * like a bad idea.              */
if|if
condition|(
name|constructorArgNames
operator|.
...
argument_list|()
operator|+
literal|"] must configure at least on constructor argument. If it "
operator|+
literal|"doens't have any it should use ObjectParser instead of ConstructingObjectParser. This is a bug in the parser "
operator|+
literal|"declaration."
argument_list|)
throw|;
...
name|IllegalStateException
argument_list|(
literal|"The targetObject wasn't built but we aren't missing any constructor args. This is a bug "
operator|+
literal|" in ConstructingObjectParser. Here are the constructor arguments "
operator|+
name|Arrays
operator|.
name|toString
...
argument_list|)
operator|+
literal|" and here are is the count ["
operator|+
name|constructorArgsCollected
operator|+
literal|"]. Good luck figuring out what happened."
operator|+
literal|" I'm truly sorry you got here."
argument_list|)
throw|;
block|}
throw|throw
