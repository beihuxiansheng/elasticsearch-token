...
return|;
block|}
comment|/**      * Return a new executor that will automatically adjust the queue size based on queue throughput.      *      * @param size number of fixed threads to use for executing tasks      * @param initialQueueCapacity initial size of the executor queue      * @param minQueueSize minimum queue size that the queue can be adjusted to      * @param maxQueueSize maximum queue size that the queue can be adjusted to      * @param frameSize number of tasks during which stats are collected before adjusting queue size      */
DECL|method|newAutoQueueFixed
specifier|public
specifier|static
name|EsThreadPoolExecutor
...
argument_list|(
literal|"initial queue capacity for ["
operator|+
name|name
operator|+
literal|"] executor must be positive, got: "
operator|+
name|initialQueueCapacity
argument_list|)
throw|;
