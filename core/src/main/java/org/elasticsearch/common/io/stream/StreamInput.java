...
argument_list|)
return|;
comment|// Lucene 5.3 will have getters for all these
case|case
literal|2
case|:
return|return
...
return|;
block|}
comment|/**      * Reads a {@link NamedWriteable} from the current stream, by first reading its name and then looking for      * the corresponding entry in the registry by name, so that the proper object can be read and returned.      */
