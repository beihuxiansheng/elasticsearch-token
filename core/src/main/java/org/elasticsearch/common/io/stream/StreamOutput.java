...
name|LUCENE_5_2_1
operator|:
literal|"Remove these regex once we upgrade to Lucene 5.3 and get proper getters for these expections"
assert|;
block|}
DECL|field|CORRUPT_INDEX_EXCEPTION_REGEX
specifier|private
specifier|final
specifier|static
name|Pattern
name|CORRUPT_INDEX_EXCEPTION_REGEX
init|=
name|Regex
operator|.
name|compile
...
argument_list|)
decl_stmt|;
DECL|field|INDEX_FORMAT_TOO_NEW_EXCEPTION_REGEX
specifier|private
specifier|final
specifier|static
name|Pattern
name|INDEX_FORMAT_TOO_NEW_EXCEPTION_REGEX
init|=
name|Regex
operator|.
name|compile
argument_list|(
literal|"Format version is not supported \\(resource (.+)\\): (-?\\d+) \\(needs to be between (-?\\d+) and (-?\\d+)\\)"
argument_list|,
literal|""
argument_list|)
decl_stmt|;
DECL|field|INDEX_FORMAT_TOO_OLD_EXCEPTION_REGEX_1
specifier|private
specifier|final
specifier|static
name|Pattern
name|INDEX_FORMAT_TOO_OLD_EXCEPTION_REGEX_1
init|=
name|Regex
operator|.
name|compile
argument_list|(
literal|"Format version is not supported \\(resource (.+)\\): (-?\\d+)(?: \\(needs to be between (-?\\d+) and (-?\\d+)\\)). This version of Lucene only supports indexes created with release 4.0 and later\\."
argument_list|,
literal|""
argument_list|)
decl_stmt|;
DECL|field|INDEX_FORMAT_TOO_OLD_EXCEPTION_REGEX_2
specifier|private
specifier|final
specifier|static
name|Pattern
name|INDEX_FORMAT_TOO_OLD_EXCEPTION_REGEX_2
init|=
name|Regex
operator|.
name|compile
argument_list|(
literal|"Format version is not supported \\(resource (.+)\\): (.+). This version of Lucene only supports indexes created with release 4.0 and later\\."
argument_list|,
literal|""
argument_list|)
decl_stmt|;
DECL|method|parseIntSafe
specifier|private
specifier|static
name|int
name|parseIntSafe
...
comment|// we should switch to using getters instead of trying to parse the message:
comment|// writeOptionalString(((CorruptIndexException)throwable).getDescription());
comment|// writeOptionalString(((CorruptIndexException)throwable).getResource());
name|Matcher
name|matcher
init|=
name|CORRUPT_INDEX_EXCEPTION_REGEX
...
comment|// we should switch to using getters instead of trying to parse the message:
comment|// writeOptionalString(((CorruptIndexException)throwable).getResource());
comment|// writeInt(((IndexFormatTooNewException)throwable).getVersion());
comment|// writeInt(((IndexFormatTooNewException)throwable).getMinVersion());
comment|// writeInt(((IndexFormatTooNewException)throwable).getMaxVersion());
name|Matcher
name|matcher
init|=
name|INDEX_FORMAT_TOO_NEW_EXCEPTION_REGEX
...
condition|)
block|{
comment|// version with numeric version in constructor
name|writeOptionalString
argument_list|(
name|matcher
operator|.
