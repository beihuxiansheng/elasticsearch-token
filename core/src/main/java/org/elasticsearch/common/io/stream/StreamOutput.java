begin_unit|revision:0.9.5;language:Java;cregit-version:0.0.1
begin_comment
comment|/*  * Licensed to Elasticsearch under one or more contributor  * license agreements. See the NOTICE file distributed with  * this work for additional information regarding copyright  * ownership. Elasticsearch licenses this file to you under  * the Apache License, Version 2.0 (the "License"); you may  * not use this file except in compliance with the License.  * You may obtain a copy of the License at  *  *    http://www.apache.org/licenses/LICENSE-2.0  *  * Unless required by applicable law or agreed to in writing,  * software distributed under the License is distributed on an  * "AS IS" BASIS, WITHOUT WARRANTIES OR CONDITIONS OF ANY  * KIND, either express or implied.  See the License for the  * specific language governing permissions and limitations  * under the License.  */
end_comment

begin_package
DECL|package|org.elasticsearch.common.io.stream
package|package
name|org
operator|.
name|elasticsearch
operator|.
name|common
operator|.
name|io
operator|.
name|stream
package|;
end_package

begin_import
import|import
name|org
operator|.
name|apache
operator|.
name|lucene
operator|.
name|index
operator|.
name|CorruptIndexException
import|;
end_import

begin_import
import|import
name|org
operator|.
name|apache
operator|.
name|lucene
operator|.
name|index
operator|.
name|IndexFormatTooNewException
import|;
end_import

begin_import
import|import
name|org
operator|.
name|apache
operator|.
name|lucene
operator|.
name|index
operator|.
name|IndexFormatTooOldException
import|;
end_import

begin_import
import|import
name|org
operator|.
name|apache
operator|.
name|lucene
operator|.
name|store
operator|.
name|AlreadyClosedException
import|;
end_import

begin_import
import|import
name|org
operator|.
name|apache
operator|.
name|lucene
operator|.
name|store
operator|.
name|LockObtainFailedException
import|;
end_import

begin_import
import|import
name|org
operator|.
name|apache
operator|.
name|lucene
operator|.
name|util
operator|.
name|ArrayUtil
import|;
end_import

begin_import
import|import
name|org
operator|.
name|apache
operator|.
name|lucene
operator|.
name|util
operator|.
name|BitUtil
import|;
end_import

begin_import
import|import
name|org
operator|.
name|apache
operator|.
name|lucene
operator|.
name|util
operator|.
name|BytesRef
import|;
end_import

begin_import
import|import
name|org
operator|.
name|apache
operator|.
name|lucene
operator|.
name|util
operator|.
name|BytesRefBuilder
import|;
end_import

begin_import
import|import
name|org
operator|.
name|elasticsearch
operator|.
name|ElasticsearchException
import|;
end_import

begin_import
import|import
name|org
operator|.
name|elasticsearch
operator|.
name|Version
import|;
end_import

begin_import
import|import
name|org
operator|.
name|elasticsearch
operator|.
name|common
operator|.
name|Nullable
import|;
end_import

begin_import
import|import
name|org
operator|.
name|elasticsearch
operator|.
name|common
operator|.
name|bytes
operator|.
name|BytesReference
import|;
end_import

begin_import
import|import
name|org
operator|.
name|elasticsearch
operator|.
name|common
operator|.
name|geo
operator|.
name|GeoPoint
import|;
end_import

begin_import
import|import
name|org
operator|.
name|elasticsearch
operator|.
name|common
operator|.
name|io
operator|.
name|stream
operator|.
name|Writeable
operator|.
name|Writer
import|;
end_import

begin_import
import|import
name|org
operator|.
name|elasticsearch
operator|.
name|common
operator|.
name|text
operator|.
name|Text
import|;
end_import

begin_import
import|import
name|org
operator|.
name|joda
operator|.
name|time
operator|.
name|DateTimeZone
import|;
end_import

begin_import
import|import
name|org
operator|.
name|joda
operator|.
name|time
operator|.
name|ReadableInstant
import|;
end_import

begin_import
import|import
name|java
operator|.
name|io
operator|.
name|EOFException
import|;
end_import

begin_import
import|import
name|java
operator|.
name|io
operator|.
name|FileNotFoundException
import|;
end_import

begin_import
import|import
name|java
operator|.
name|io
operator|.
name|IOException
import|;
end_import

begin_import
import|import
name|java
operator|.
name|io
operator|.
name|OutputStream
import|;
end_import

begin_import
import|import
name|java
operator|.
name|nio
operator|.
name|file
operator|.
name|AccessDeniedException
import|;
end_import

begin_import
import|import
name|java
operator|.
name|nio
operator|.
name|file
operator|.
name|AtomicMoveNotSupportedException
import|;
end_import

begin_import
import|import
name|java
operator|.
name|nio
operator|.
name|file
operator|.
name|DirectoryNotEmptyException
import|;
end_import

begin_import
import|import
name|java
operator|.
name|nio
operator|.
name|file
operator|.
name|FileAlreadyExistsException
import|;
end_import

begin_import
import|import
name|java
operator|.
name|nio
operator|.
name|file
operator|.
name|FileSystemException
import|;
end_import

begin_import
import|import
name|java
operator|.
name|nio
operator|.
name|file
operator|.
name|FileSystemLoopException
import|;
end_import

begin_import
import|import
name|java
operator|.
name|nio
operator|.
name|file
operator|.
name|NoSuchFileException
import|;
end_import

begin_import
import|import
name|java
operator|.
name|nio
operator|.
name|file
operator|.
name|NotDirectoryException
import|;
end_import

begin_import
import|import
name|java
operator|.
name|util
operator|.
name|Collections
import|;
end_import

begin_import
import|import
name|java
operator|.
name|util
operator|.
name|Date
import|;
end_import

begin_import
import|import
name|java
operator|.
name|util
operator|.
name|HashMap
import|;
end_import

begin_import
import|import
name|java
operator|.
name|util
operator|.
name|Iterator
import|;
end_import

begin_import
import|import
name|java
operator|.
name|util
operator|.
name|LinkedHashMap
import|;
end_import

begin_import
import|import
name|java
operator|.
name|util
operator|.
name|List
import|;
end_import

begin_import
import|import
name|java
operator|.
name|util
operator|.
name|Map
import|;
end_import

begin_comment
comment|/**  * A stream from another node to this node. Technically, it can also be streamed from a byte array but that is mostly for testing.  *  * This class's methods are optimized so you can put the methods that read and write a class next to each other and you can scan them  * visually for differences. That means that most variables should be read and written in a single line so even large objects fit both  * reading and writing on the screen. It also means that the methods on this class are named very similarly to {@link StreamInput}. Finally  * it means that the "barrier to entry" for adding new methods to this class is relatively low even though it is a shared class with code  * everywhere. That being said, this class deals primarily with {@code List}s rather than Arrays. For the most part calls should adapt to  * lists, either by storing {@code List}s internally or just converting to and from a {@code List} when calling. This comment is repeated  * on {@link StreamInput}.  */
end_comment

begin_class
DECL|class|StreamOutput
specifier|public
specifier|abstract
class|class
name|StreamOutput
extends|extends
name|OutputStream
block|{
DECL|field|version
specifier|private
name|Version
name|version
init|=
name|Version
operator|.
name|CURRENT
decl_stmt|;
comment|/**      * The version of the node on the other side of this stream.      */
DECL|method|getVersion
specifier|public
name|Version
name|getVersion
parameter_list|()
block|{
return|return
name|this
operator|.
name|version
return|;
block|}
comment|/**      * Set the version of the node on the other side of this stream.      */
DECL|method|setVersion
specifier|public
name|void
name|setVersion
parameter_list|(
name|Version
name|version
parameter_list|)
block|{
name|this
operator|.
name|version
operator|=
name|version
expr_stmt|;
block|}
DECL|method|position
specifier|public
name|long
name|position
parameter_list|()
throws|throws
name|IOException
block|{
throw|throw
operator|new
name|UnsupportedOperationException
argument_list|()
throw|;
block|}
DECL|method|seek
specifier|public
name|void
name|seek
parameter_list|(
name|long
name|position
parameter_list|)
throws|throws
name|IOException
block|{
throw|throw
operator|new
name|UnsupportedOperationException
argument_list|()
throw|;
block|}
comment|/**      * Writes a single byte.      */
DECL|method|writeByte
specifier|public
specifier|abstract
name|void
name|writeByte
parameter_list|(
name|byte
name|b
parameter_list|)
throws|throws
name|IOException
function_decl|;
comment|/**      * Writes an array of bytes.      *      * @param b the bytes to write      */
DECL|method|writeBytes
specifier|public
name|void
name|writeBytes
parameter_list|(
name|byte
index|[]
name|b
parameter_list|)
throws|throws
name|IOException
block|{
name|writeBytes
argument_list|(
name|b
argument_list|,
literal|0
argument_list|,
name|b
operator|.
name|length
argument_list|)
expr_stmt|;
block|}
comment|/**      * Writes an array of bytes.      *      * @param b      the bytes to write      * @param length the number of bytes to write      */
DECL|method|writeBytes
specifier|public
name|void
name|writeBytes
parameter_list|(
name|byte
index|[]
name|b
parameter_list|,
name|int
name|length
parameter_list|)
throws|throws
name|IOException
block|{
name|writeBytes
argument_list|(
name|b
argument_list|,
literal|0
argument_list|,
name|length
argument_list|)
expr_stmt|;
block|}
comment|/**      * Writes an array of bytes.      *      * @param b      the bytes to write      * @param offset the offset in the byte array      * @param length the number of bytes to write      */
DECL|method|writeBytes
specifier|public
specifier|abstract
name|void
name|writeBytes
parameter_list|(
name|byte
index|[]
name|b
parameter_list|,
name|int
name|offset
parameter_list|,
name|int
name|length
parameter_list|)
throws|throws
name|IOException
function_decl|;
comment|/**      * Writes an array of bytes.      *      * @param b the bytes to write      */
DECL|method|writeByteArray
specifier|public
name|void
name|writeByteArray
parameter_list|(
name|byte
index|[]
name|b
parameter_list|)
throws|throws
name|IOException
block|{
name|writeVInt
argument_list|(
name|b
operator|.
name|length
argument_list|)
expr_stmt|;
name|writeBytes
argument_list|(
name|b
argument_list|,
literal|0
argument_list|,
name|b
operator|.
name|length
argument_list|)
expr_stmt|;
block|}
comment|/**      * Writes the bytes reference, including a length header.      */
DECL|method|writeBytesReference
specifier|public
name|void
name|writeBytesReference
parameter_list|(
annotation|@
name|Nullable
name|BytesReference
name|bytes
parameter_list|)
throws|throws
name|IOException
block|{
if|if
condition|(
name|bytes
operator|==
literal|null
condition|)
block|{
name|writeVInt
argument_list|(
literal|0
argument_list|)
expr_stmt|;
return|return;
block|}
name|writeVInt
argument_list|(
name|bytes
operator|.
name|length
argument_list|()
argument_list|)
expr_stmt|;
name|bytes
operator|.
name|writeTo
argument_list|(
name|this
argument_list|)
expr_stmt|;
block|}
comment|/**      * Writes an optional bytes reference including a length header. Use this if you need to differentiate between null and empty bytes      * references. Use {@link #writeBytesReference(BytesReference)} and {@link StreamInput#readBytesReference()} if you do not.      */
DECL|method|writeOptionalBytesReference
specifier|public
name|void
name|writeOptionalBytesReference
parameter_list|(
annotation|@
name|Nullable
name|BytesReference
name|bytes
parameter_list|)
throws|throws
name|IOException
block|{
if|if
condition|(
name|bytes
operator|==
literal|null
condition|)
block|{
name|writeVInt
argument_list|(
literal|0
argument_list|)
expr_stmt|;
return|return;
block|}
name|writeVInt
argument_list|(
name|bytes
operator|.
name|length
argument_list|()
operator|+
literal|1
argument_list|)
expr_stmt|;
name|bytes
operator|.
name|writeTo
argument_list|(
name|this
argument_list|)
expr_stmt|;
block|}
DECL|method|writeBytesRef
specifier|public
name|void
name|writeBytesRef
parameter_list|(
name|BytesRef
name|bytes
parameter_list|)
throws|throws
name|IOException
block|{
if|if
condition|(
name|bytes
operator|==
literal|null
condition|)
block|{
name|writeVInt
argument_list|(
literal|0
argument_list|)
expr_stmt|;
return|return;
block|}
name|writeVInt
argument_list|(
name|bytes
operator|.
name|length
argument_list|)
expr_stmt|;
name|write
argument_list|(
name|bytes
operator|.
name|bytes
argument_list|,
name|bytes
operator|.
name|offset
argument_list|,
name|bytes
operator|.
name|length
argument_list|)
expr_stmt|;
block|}
DECL|method|writeShort
specifier|public
specifier|final
name|void
name|writeShort
parameter_list|(
name|short
name|v
parameter_list|)
throws|throws
name|IOException
block|{
name|writeByte
argument_list|(
call|(
name|byte
call|)
argument_list|(
name|v
operator|>>
literal|8
argument_list|)
argument_list|)
expr_stmt|;
name|writeByte
argument_list|(
operator|(
name|byte
operator|)
name|v
argument_list|)
expr_stmt|;
block|}
comment|/**      * Writes an int as four bytes.      */
DECL|method|writeInt
specifier|public
name|void
name|writeInt
parameter_list|(
name|int
name|i
parameter_list|)
throws|throws
name|IOException
block|{
name|writeByte
argument_list|(
call|(
name|byte
call|)
argument_list|(
name|i
operator|>>
literal|24
argument_list|)
argument_list|)
expr_stmt|;
name|writeByte
argument_list|(
call|(
name|byte
call|)
argument_list|(
name|i
operator|>>
literal|16
argument_list|)
argument_list|)
expr_stmt|;
name|writeByte
argument_list|(
call|(
name|byte
call|)
argument_list|(
name|i
operator|>>
literal|8
argument_list|)
argument_list|)
expr_stmt|;
name|writeByte
argument_list|(
operator|(
name|byte
operator|)
name|i
argument_list|)
expr_stmt|;
block|}
comment|/**      * Writes an int in a variable-length format.  Writes between one and      * five bytes.  Smaller values take fewer bytes.  Negative numbers      * will always use all 5 bytes and are therefore better serialized      * using {@link #writeInt}      */
DECL|method|writeVInt
specifier|public
name|void
name|writeVInt
parameter_list|(
name|int
name|i
parameter_list|)
throws|throws
name|IOException
block|{
while|while
condition|(
operator|(
name|i
operator|&
operator|~
literal|0x7F
operator|)
operator|!=
literal|0
condition|)
block|{
name|writeByte
argument_list|(
call|(
name|byte
call|)
argument_list|(
operator|(
name|i
operator|&
literal|0x7f
operator|)
operator||
literal|0x80
argument_list|)
argument_list|)
expr_stmt|;
name|i
operator|>>>=
literal|7
expr_stmt|;
block|}
name|writeByte
argument_list|(
operator|(
name|byte
operator|)
name|i
argument_list|)
expr_stmt|;
block|}
comment|/**      * Writes a long as eight bytes.      */
DECL|method|writeLong
specifier|public
name|void
name|writeLong
parameter_list|(
name|long
name|i
parameter_list|)
throws|throws
name|IOException
block|{
name|writeInt
argument_list|(
call|(
name|int
call|)
argument_list|(
name|i
operator|>>
literal|32
argument_list|)
argument_list|)
expr_stmt|;
name|writeInt
argument_list|(
operator|(
name|int
operator|)
name|i
argument_list|)
expr_stmt|;
block|}
comment|/**      * Writes a non-negative long in a variable-length format. Writes between one and ten bytes. Smaller values take fewer bytes. Negative      * numbers use ten bytes and trip assertions (if running in tests) so prefer {@link #writeLong(long)} or {@link #writeZLong(long)} for      * negative numbers.      */
DECL|method|writeVLong
specifier|public
name|void
name|writeVLong
parameter_list|(
name|long
name|i
parameter_list|)
throws|throws
name|IOException
block|{
if|if
condition|(
name|i
operator|<
literal|0
condition|)
block|{
throw|throw
operator|new
name|IllegalStateException
argument_list|(
literal|"Negative longs unsupported, use writeLong or writeZLong for negative numbers ["
operator|+
name|i
operator|+
literal|"]"
argument_list|)
throw|;
block|}
name|writeVLongNoCheck
argument_list|(
name|i
argument_list|)
expr_stmt|;
block|}
comment|/**      * Writes a long in a variable-length format without first checking if it is negative. Package private for testing. Use      * {@link #writeVLong(long)} instead.      */
DECL|method|writeVLongNoCheck
name|void
name|writeVLongNoCheck
parameter_list|(
name|long
name|i
parameter_list|)
throws|throws
name|IOException
block|{
while|while
condition|(
operator|(
name|i
operator|&
operator|~
literal|0x7F
operator|)
operator|!=
literal|0
condition|)
block|{
name|writeByte
argument_list|(
call|(
name|byte
call|)
argument_list|(
operator|(
name|i
operator|&
literal|0x7f
operator|)
operator||
literal|0x80
argument_list|)
argument_list|)
expr_stmt|;
name|i
operator|>>>=
literal|7
expr_stmt|;
block|}
name|writeByte
argument_list|(
operator|(
name|byte
operator|)
name|i
argument_list|)
expr_stmt|;
block|}
comment|/**      * Writes a long in a variable-length format. Writes between one and ten bytes.      * Values are remapped by sliding the sign bit into the lsb and then encoded as an unsigned number      * e.g., 0 -;&gt; 0, -1 -;&gt; 1, 1 -;&gt; 2, ..., Long.MIN_VALUE -;&gt; -1, Long.MAX_VALUE -;&gt; -2      * Numbers with small absolute value will have a small encoding      * If the numbers are known to be non-negative, use {@link #writeVLong(long)}      */
DECL|method|writeZLong
specifier|public
name|void
name|writeZLong
parameter_list|(
name|long
name|i
parameter_list|)
throws|throws
name|IOException
block|{
comment|// zig-zag encoding cf. https://developers.google.com/protocol-buffers/docs/encoding?hl=en
name|long
name|value
init|=
name|BitUtil
operator|.
name|zigZagEncode
argument_list|(
name|i
argument_list|)
decl_stmt|;
while|while
condition|(
operator|(
name|value
operator|&
literal|0xFFFFFFFFFFFFFF80L
operator|)
operator|!=
literal|0L
condition|)
block|{
name|writeByte
argument_list|(
call|(
name|byte
call|)
argument_list|(
operator|(
name|value
operator|&
literal|0x7F
operator|)
operator||
literal|0x80
argument_list|)
argument_list|)
expr_stmt|;
name|value
operator|>>>=
literal|7
expr_stmt|;
block|}
name|writeByte
argument_list|(
call|(
name|byte
call|)
argument_list|(
name|value
operator|&
literal|0x7F
argument_list|)
argument_list|)
expr_stmt|;
block|}
DECL|method|writeOptionalLong
specifier|public
name|void
name|writeOptionalLong
parameter_list|(
annotation|@
name|Nullable
name|Long
name|l
parameter_list|)
throws|throws
name|IOException
block|{
if|if
condition|(
name|l
operator|==
literal|null
condition|)
block|{
name|writeBoolean
argument_list|(
literal|false
argument_list|)
expr_stmt|;
block|}
else|else
block|{
name|writeBoolean
argument_list|(
literal|true
argument_list|)
expr_stmt|;
name|writeLong
argument_list|(
name|l
argument_list|)
expr_stmt|;
block|}
block|}
DECL|method|writeOptionalString
specifier|public
name|void
name|writeOptionalString
parameter_list|(
annotation|@
name|Nullable
name|String
name|str
parameter_list|)
throws|throws
name|IOException
block|{
if|if
condition|(
name|str
operator|==
literal|null
condition|)
block|{
name|writeBoolean
argument_list|(
literal|false
argument_list|)
expr_stmt|;
block|}
else|else
block|{
name|writeBoolean
argument_list|(
literal|true
argument_list|)
expr_stmt|;
name|writeString
argument_list|(
name|str
argument_list|)
expr_stmt|;
block|}
block|}
DECL|method|writeOptionalVInt
specifier|public
name|void
name|writeOptionalVInt
parameter_list|(
annotation|@
name|Nullable
name|Integer
name|integer
parameter_list|)
throws|throws
name|IOException
block|{
if|if
condition|(
name|integer
operator|==
literal|null
condition|)
block|{
name|writeBoolean
argument_list|(
literal|false
argument_list|)
expr_stmt|;
block|}
else|else
block|{
name|writeBoolean
argument_list|(
literal|true
argument_list|)
expr_stmt|;
name|writeVInt
argument_list|(
name|integer
argument_list|)
expr_stmt|;
block|}
block|}
DECL|method|writeOptionalFloat
specifier|public
name|void
name|writeOptionalFloat
parameter_list|(
annotation|@
name|Nullable
name|Float
name|floatValue
parameter_list|)
throws|throws
name|IOException
block|{
if|if
condition|(
name|floatValue
operator|==
literal|null
condition|)
block|{
name|writeBoolean
argument_list|(
literal|false
argument_list|)
expr_stmt|;
block|}
else|else
block|{
name|writeBoolean
argument_list|(
literal|true
argument_list|)
expr_stmt|;
name|writeFloat
argument_list|(
name|floatValue
argument_list|)
expr_stmt|;
block|}
block|}
DECL|method|writeOptionalText
specifier|public
name|void
name|writeOptionalText
parameter_list|(
annotation|@
name|Nullable
name|Text
name|text
parameter_list|)
throws|throws
name|IOException
block|{
if|if
condition|(
name|text
operator|==
literal|null
condition|)
block|{
name|writeInt
argument_list|(
operator|-
literal|1
argument_list|)
expr_stmt|;
block|}
else|else
block|{
name|writeText
argument_list|(
name|text
argument_list|)
expr_stmt|;
block|}
block|}
DECL|field|spare
specifier|private
specifier|final
name|BytesRefBuilder
name|spare
init|=
operator|new
name|BytesRefBuilder
argument_list|()
decl_stmt|;
DECL|method|writeText
specifier|public
name|void
name|writeText
parameter_list|(
name|Text
name|text
parameter_list|)
throws|throws
name|IOException
block|{
if|if
condition|(
operator|!
name|text
operator|.
name|hasBytes
argument_list|()
condition|)
block|{
specifier|final
name|String
name|string
init|=
name|text
operator|.
name|string
argument_list|()
decl_stmt|;
name|spare
operator|.
name|copyChars
argument_list|(
name|string
argument_list|)
expr_stmt|;
name|writeInt
argument_list|(
name|spare
operator|.
name|length
argument_list|()
argument_list|)
expr_stmt|;
name|write
argument_list|(
name|spare
operator|.
name|bytes
argument_list|()
argument_list|,
literal|0
argument_list|,
name|spare
operator|.
name|length
argument_list|()
argument_list|)
expr_stmt|;
block|}
else|else
block|{
name|BytesReference
name|bytes
init|=
name|text
operator|.
name|bytes
argument_list|()
decl_stmt|;
name|writeInt
argument_list|(
name|bytes
operator|.
name|length
argument_list|()
argument_list|)
expr_stmt|;
name|bytes
operator|.
name|writeTo
argument_list|(
name|this
argument_list|)
expr_stmt|;
block|}
block|}
comment|// we use a small buffer to convert strings to bytes since we want to prevent calling writeByte
comment|// for every byte in the string (see #21660 for details).
comment|// This buffer will never be the oversized limit of 1024 bytes and will not be shared across streams
DECL|field|convertStringBuffer
specifier|private
name|byte
index|[]
name|convertStringBuffer
init|=
name|BytesRef
operator|.
name|EMPTY_BYTES
decl_stmt|;
comment|// TODO should we reduce it to 0 bytes once the stream is closed?
DECL|method|writeString
specifier|public
name|void
name|writeString
parameter_list|(
name|String
name|str
parameter_list|)
throws|throws
name|IOException
block|{
specifier|final
name|int
name|charCount
init|=
name|str
operator|.
name|length
argument_list|()
decl_stmt|;
specifier|final
name|int
name|bufferSize
init|=
name|Math
operator|.
name|min
argument_list|(
literal|3
operator|*
name|charCount
argument_list|,
literal|1024
argument_list|)
decl_stmt|;
comment|// at most 3 bytes per character is needed here
if|if
condition|(
name|convertStringBuffer
operator|.
name|length
operator|<
name|bufferSize
condition|)
block|{
comment|// we don't use ArrayUtils.grow since copying the bytes is unnecessary
name|convertStringBuffer
operator|=
operator|new
name|byte
index|[
name|ArrayUtil
operator|.
name|oversize
argument_list|(
name|bufferSize
argument_list|,
name|Byte
operator|.
name|BYTES
argument_list|)
index|]
expr_stmt|;
block|}
name|byte
index|[]
name|buffer
init|=
name|convertStringBuffer
decl_stmt|;
name|int
name|offset
init|=
literal|0
decl_stmt|;
name|writeVInt
argument_list|(
name|charCount
argument_list|)
expr_stmt|;
for|for
control|(
name|int
name|i
init|=
literal|0
init|;
name|i
operator|<
name|charCount
condition|;
name|i
operator|++
control|)
block|{
specifier|final
name|int
name|c
init|=
name|str
operator|.
name|charAt
argument_list|(
name|i
argument_list|)
decl_stmt|;
if|if
condition|(
name|c
operator|<=
literal|0x007F
condition|)
block|{
name|buffer
index|[
name|offset
operator|++
index|]
operator|=
operator|(
operator|(
name|byte
operator|)
name|c
operator|)
expr_stmt|;
block|}
elseif|else
if|if
condition|(
name|c
operator|>
literal|0x07FF
condition|)
block|{
name|buffer
index|[
name|offset
operator|++
index|]
operator|=
operator|(
call|(
name|byte
call|)
argument_list|(
literal|0xE0
operator||
name|c
operator|>>
literal|12
operator|&
literal|0x0F
argument_list|)
operator|)
expr_stmt|;
name|buffer
index|[
name|offset
operator|++
index|]
operator|=
operator|(
call|(
name|byte
call|)
argument_list|(
literal|0x80
operator||
name|c
operator|>>
literal|6
operator|&
literal|0x3F
argument_list|)
operator|)
expr_stmt|;
name|buffer
index|[
name|offset
operator|++
index|]
operator|=
operator|(
call|(
name|byte
call|)
argument_list|(
literal|0x80
operator||
name|c
operator|>>
literal|0
operator|&
literal|0x3F
argument_list|)
operator|)
expr_stmt|;
block|}
else|else
block|{
name|buffer
index|[
name|offset
operator|++
index|]
operator|=
operator|(
call|(
name|byte
call|)
argument_list|(
literal|0xC0
operator||
name|c
operator|>>
literal|6
operator|&
literal|0x1F
argument_list|)
operator|)
expr_stmt|;
name|buffer
index|[
name|offset
operator|++
index|]
operator|=
operator|(
call|(
name|byte
call|)
argument_list|(
literal|0x80
operator||
name|c
operator|>>
literal|0
operator|&
literal|0x3F
argument_list|)
operator|)
expr_stmt|;
block|}
comment|// make sure any possible char can fit into the buffer in any possible iteration
comment|// we need at most 3 bytes so we flush the buffer once we have less than 3 bytes
comment|// left before we start another iteration
if|if
condition|(
name|offset
operator|>
name|buffer
operator|.
name|length
operator|-
literal|3
condition|)
block|{
name|writeBytes
argument_list|(
name|buffer
argument_list|,
name|offset
argument_list|)
expr_stmt|;
name|offset
operator|=
literal|0
expr_stmt|;
block|}
block|}
name|writeBytes
argument_list|(
name|buffer
argument_list|,
name|offset
argument_list|)
expr_stmt|;
block|}
DECL|method|writeFloat
specifier|public
name|void
name|writeFloat
parameter_list|(
name|float
name|v
parameter_list|)
throws|throws
name|IOException
block|{
name|writeInt
argument_list|(
name|Float
operator|.
name|floatToIntBits
argument_list|(
name|v
argument_list|)
argument_list|)
expr_stmt|;
block|}
DECL|method|writeDouble
specifier|public
name|void
name|writeDouble
parameter_list|(
name|double
name|v
parameter_list|)
throws|throws
name|IOException
block|{
name|writeLong
argument_list|(
name|Double
operator|.
name|doubleToLongBits
argument_list|(
name|v
argument_list|)
argument_list|)
expr_stmt|;
block|}
DECL|method|writeOptionalDouble
specifier|public
name|void
name|writeOptionalDouble
parameter_list|(
annotation|@
name|Nullable
name|Double
name|v
parameter_list|)
throws|throws
name|IOException
block|{
if|if
condition|(
name|v
operator|==
literal|null
condition|)
block|{
name|writeBoolean
argument_list|(
literal|false
argument_list|)
expr_stmt|;
block|}
else|else
block|{
name|writeBoolean
argument_list|(
literal|true
argument_list|)
expr_stmt|;
name|writeDouble
argument_list|(
name|v
argument_list|)
expr_stmt|;
block|}
block|}
DECL|field|ZERO
specifier|private
specifier|static
name|byte
name|ZERO
init|=
literal|0
decl_stmt|;
DECL|field|ONE
specifier|private
specifier|static
name|byte
name|ONE
init|=
literal|1
decl_stmt|;
DECL|field|TWO
specifier|private
specifier|static
name|byte
name|TWO
init|=
literal|2
decl_stmt|;
comment|/**      * Writes a boolean.      */
DECL|method|writeBoolean
specifier|public
name|void
name|writeBoolean
parameter_list|(
name|boolean
name|b
parameter_list|)
throws|throws
name|IOException
block|{
name|writeByte
argument_list|(
name|b
condition|?
name|ONE
else|:
name|ZERO
argument_list|)
expr_stmt|;
block|}
DECL|method|writeOptionalBoolean
specifier|public
name|void
name|writeOptionalBoolean
parameter_list|(
annotation|@
name|Nullable
name|Boolean
name|b
parameter_list|)
throws|throws
name|IOException
block|{
if|if
condition|(
name|b
operator|==
literal|null
condition|)
block|{
name|writeByte
argument_list|(
name|TWO
argument_list|)
expr_stmt|;
block|}
else|else
block|{
name|writeBoolean
argument_list|(
name|b
argument_list|)
expr_stmt|;
block|}
block|}
comment|/**      * Forces any buffered output to be written.      */
annotation|@
name|Override
DECL|method|flush
specifier|public
specifier|abstract
name|void
name|flush
parameter_list|()
throws|throws
name|IOException
function_decl|;
comment|/**      * Closes this stream to further operations.      */
annotation|@
name|Override
DECL|method|close
specifier|public
specifier|abstract
name|void
name|close
parameter_list|()
throws|throws
name|IOException
function_decl|;
DECL|method|reset
specifier|public
specifier|abstract
name|void
name|reset
parameter_list|()
throws|throws
name|IOException
function_decl|;
annotation|@
name|Override
DECL|method|write
specifier|public
name|void
name|write
parameter_list|(
name|int
name|b
parameter_list|)
throws|throws
name|IOException
block|{
name|writeByte
argument_list|(
operator|(
name|byte
operator|)
name|b
argument_list|)
expr_stmt|;
block|}
annotation|@
name|Override
DECL|method|write
specifier|public
name|void
name|write
parameter_list|(
name|byte
index|[]
name|b
parameter_list|,
name|int
name|off
parameter_list|,
name|int
name|len
parameter_list|)
throws|throws
name|IOException
block|{
name|writeBytes
argument_list|(
name|b
argument_list|,
name|off
argument_list|,
name|len
argument_list|)
expr_stmt|;
block|}
DECL|method|writeStringArray
specifier|public
name|void
name|writeStringArray
parameter_list|(
name|String
index|[]
name|array
parameter_list|)
throws|throws
name|IOException
block|{
name|writeVInt
argument_list|(
name|array
operator|.
name|length
argument_list|)
expr_stmt|;
for|for
control|(
name|String
name|s
range|:
name|array
control|)
block|{
name|writeString
argument_list|(
name|s
argument_list|)
expr_stmt|;
block|}
block|}
comment|/**      * Writes a string array, for nullable string, writes it as 0 (empty string).      */
DECL|method|writeStringArrayNullable
specifier|public
name|void
name|writeStringArrayNullable
parameter_list|(
annotation|@
name|Nullable
name|String
index|[]
name|array
parameter_list|)
throws|throws
name|IOException
block|{
if|if
condition|(
name|array
operator|==
literal|null
condition|)
block|{
name|writeVInt
argument_list|(
literal|0
argument_list|)
expr_stmt|;
block|}
else|else
block|{
name|writeVInt
argument_list|(
name|array
operator|.
name|length
argument_list|)
expr_stmt|;
for|for
control|(
name|String
name|s
range|:
name|array
control|)
block|{
name|writeString
argument_list|(
name|s
argument_list|)
expr_stmt|;
block|}
block|}
block|}
comment|/**      * Writes a string array, for nullable string, writes false.      */
DECL|method|writeOptionalStringArray
specifier|public
name|void
name|writeOptionalStringArray
parameter_list|(
annotation|@
name|Nullable
name|String
index|[]
name|array
parameter_list|)
throws|throws
name|IOException
block|{
if|if
condition|(
name|array
operator|==
literal|null
condition|)
block|{
name|writeBoolean
argument_list|(
literal|false
argument_list|)
expr_stmt|;
block|}
else|else
block|{
name|writeBoolean
argument_list|(
literal|true
argument_list|)
expr_stmt|;
name|writeStringArray
argument_list|(
name|array
argument_list|)
expr_stmt|;
block|}
block|}
DECL|method|writeMap
specifier|public
name|void
name|writeMap
parameter_list|(
annotation|@
name|Nullable
name|Map
argument_list|<
name|String
argument_list|,
name|Object
argument_list|>
name|map
parameter_list|)
throws|throws
name|IOException
block|{
name|writeGenericValue
argument_list|(
name|map
argument_list|)
expr_stmt|;
block|}
comment|/**      * write map to stream with consistent order      * to make sure every map generated bytes order are same.      * This method is compatible with {@code StreamInput.readMap} and {@code StreamInput.readGenericValue}      * This method only will handle the map keys order, not maps contained within the map      */
DECL|method|writeMapWithConsistentOrder
specifier|public
name|void
name|writeMapWithConsistentOrder
parameter_list|(
annotation|@
name|Nullable
name|Map
argument_list|<
name|String
argument_list|,
name|?
extends|extends
name|Object
argument_list|>
name|map
parameter_list|)
throws|throws
name|IOException
block|{
if|if
condition|(
name|map
operator|==
literal|null
condition|)
block|{
name|writeByte
argument_list|(
operator|(
name|byte
operator|)
operator|-
literal|1
argument_list|)
expr_stmt|;
return|return;
block|}
assert|assert
literal|false
operator|==
operator|(
name|map
operator|instanceof
name|LinkedHashMap
operator|)
assert|;
name|this
operator|.
name|writeByte
argument_list|(
operator|(
name|byte
operator|)
literal|10
argument_list|)
expr_stmt|;
name|this
operator|.
name|writeVInt
argument_list|(
name|map
operator|.
name|size
argument_list|()
argument_list|)
expr_stmt|;
name|Iterator
argument_list|<
name|?
extends|extends
name|Map
operator|.
name|Entry
argument_list|<
name|String
argument_list|,
name|?
argument_list|>
argument_list|>
name|iterator
init|=
name|map
operator|.
name|entrySet
argument_list|()
operator|.
name|stream
argument_list|()
operator|.
name|sorted
argument_list|(
parameter_list|(
name|a
parameter_list|,
name|b
parameter_list|)
lambda|->
name|a
operator|.
name|getKey
argument_list|()
operator|.
name|compareTo
argument_list|(
name|b
operator|.
name|getKey
argument_list|()
argument_list|)
argument_list|)
operator|.
name|iterator
argument_list|()
decl_stmt|;
while|while
condition|(
name|iterator
operator|.
name|hasNext
argument_list|()
condition|)
block|{
name|Map
operator|.
name|Entry
argument_list|<
name|String
argument_list|,
name|?
argument_list|>
name|next
init|=
name|iterator
operator|.
name|next
argument_list|()
decl_stmt|;
name|this
operator|.
name|writeString
argument_list|(
name|next
operator|.
name|getKey
argument_list|()
argument_list|)
expr_stmt|;
name|this
operator|.
name|writeGenericValue
argument_list|(
name|next
operator|.
name|getValue
argument_list|()
argument_list|)
expr_stmt|;
block|}
block|}
comment|/**      * Write a {@link Map} of {@code K}-type keys to {@code V}-type {@link List}s.      *<pre><code>      * Map&lt;String, List&lt;String&gt;&gt; map = ...;      * out.writeMapOfLists(map, StreamOutput::writeString, StreamOutput::writeString);      *</code></pre>      *      * @param keyWriter The key writer      * @param valueWriter The value writer      */
DECL|method|writeMapOfLists
specifier|public
specifier|final
parameter_list|<
name|K
parameter_list|,
name|V
parameter_list|>
name|void
name|writeMapOfLists
parameter_list|(
specifier|final
name|Map
argument_list|<
name|K
argument_list|,
name|List
argument_list|<
name|V
argument_list|>
argument_list|>
name|map
parameter_list|,
specifier|final
name|Writer
argument_list|<
name|K
argument_list|>
name|keyWriter
parameter_list|,
specifier|final
name|Writer
argument_list|<
name|V
argument_list|>
name|valueWriter
parameter_list|)
throws|throws
name|IOException
block|{
name|writeMap
argument_list|(
name|map
argument_list|,
name|keyWriter
argument_list|,
parameter_list|(
name|stream
parameter_list|,
name|list
parameter_list|)
lambda|->
block|{
name|writeVInt
argument_list|(
name|list
operator|.
name|size
argument_list|()
argument_list|)
expr_stmt|;
for|for
control|(
specifier|final
name|V
name|value
range|:
name|list
control|)
block|{
name|valueWriter
operator|.
name|write
argument_list|(
name|this
argument_list|,
name|value
argument_list|)
expr_stmt|;
block|}
block|}
argument_list|)
expr_stmt|;
block|}
comment|/**      * Write a {@link Map} of {@code K}-type keys to {@code V}-type.      *<pre><code>      * Map&lt;String, String&gt; map = ...;      * out.writeMap(map, StreamOutput::writeString, StreamOutput::writeString);      *</code></pre>      *      * @param keyWriter The key writer      * @param valueWriter The value writer      */
DECL|method|writeMap
specifier|public
specifier|final
parameter_list|<
name|K
parameter_list|,
name|V
parameter_list|>
name|void
name|writeMap
parameter_list|(
specifier|final
name|Map
argument_list|<
name|K
argument_list|,
name|V
argument_list|>
name|map
parameter_list|,
specifier|final
name|Writer
argument_list|<
name|K
argument_list|>
name|keyWriter
parameter_list|,
specifier|final
name|Writer
argument_list|<
name|V
argument_list|>
name|valueWriter
parameter_list|)
throws|throws
name|IOException
block|{
name|writeVInt
argument_list|(
name|map
operator|.
name|size
argument_list|()
argument_list|)
expr_stmt|;
for|for
control|(
specifier|final
name|Map
operator|.
name|Entry
argument_list|<
name|K
argument_list|,
name|V
argument_list|>
name|entry
range|:
name|map
operator|.
name|entrySet
argument_list|()
control|)
block|{
name|keyWriter
operator|.
name|write
argument_list|(
name|this
argument_list|,
name|entry
operator|.
name|getKey
argument_list|()
argument_list|)
expr_stmt|;
name|valueWriter
operator|.
name|write
argument_list|(
name|this
argument_list|,
name|entry
operator|.
name|getValue
argument_list|()
argument_list|)
expr_stmt|;
block|}
block|}
DECL|field|WRITERS
specifier|private
specifier|static
specifier|final
name|Map
argument_list|<
name|Class
argument_list|<
name|?
argument_list|>
argument_list|,
name|Writer
argument_list|>
name|WRITERS
decl_stmt|;
static|static
block|{
name|Map
argument_list|<
name|Class
argument_list|<
name|?
argument_list|>
argument_list|,
name|Writer
argument_list|>
name|writers
init|=
operator|new
name|HashMap
argument_list|<>
argument_list|()
decl_stmt|;
name|writers
operator|.
name|put
argument_list|(
name|String
operator|.
name|class
argument_list|,
parameter_list|(
name|o
parameter_list|,
name|v
parameter_list|)
lambda|->
block|{
name|o
operator|.
name|writeByte
argument_list|(
operator|(
name|byte
operator|)
literal|0
argument_list|)
expr_stmt|;
name|o
operator|.
name|writeString
argument_list|(
operator|(
name|String
operator|)
name|v
argument_list|)
expr_stmt|;
block|}
argument_list|)
expr_stmt|;
name|writers
operator|.
name|put
argument_list|(
name|Integer
operator|.
name|class
argument_list|,
parameter_list|(
name|o
parameter_list|,
name|v
parameter_list|)
lambda|->
block|{
name|o
operator|.
name|writeByte
argument_list|(
operator|(
name|byte
operator|)
literal|1
argument_list|)
expr_stmt|;
name|o
operator|.
name|writeInt
argument_list|(
operator|(
name|Integer
operator|)
name|v
argument_list|)
expr_stmt|;
block|}
argument_list|)
expr_stmt|;
name|writers
operator|.
name|put
argument_list|(
name|Long
operator|.
name|class
argument_list|,
parameter_list|(
name|o
parameter_list|,
name|v
parameter_list|)
lambda|->
block|{
name|o
operator|.
name|writeByte
argument_list|(
operator|(
name|byte
operator|)
literal|2
argument_list|)
expr_stmt|;
name|o
operator|.
name|writeLong
argument_list|(
operator|(
name|Long
operator|)
name|v
argument_list|)
expr_stmt|;
block|}
argument_list|)
expr_stmt|;
name|writers
operator|.
name|put
argument_list|(
name|Float
operator|.
name|class
argument_list|,
parameter_list|(
name|o
parameter_list|,
name|v
parameter_list|)
lambda|->
block|{
name|o
operator|.
name|writeByte
argument_list|(
operator|(
name|byte
operator|)
literal|3
argument_list|)
expr_stmt|;
name|o
operator|.
name|writeFloat
argument_list|(
operator|(
name|float
operator|)
name|v
argument_list|)
expr_stmt|;
block|}
argument_list|)
expr_stmt|;
name|writers
operator|.
name|put
argument_list|(
name|Double
operator|.
name|class
argument_list|,
parameter_list|(
name|o
parameter_list|,
name|v
parameter_list|)
lambda|->
block|{
name|o
operator|.
name|writeByte
argument_list|(
operator|(
name|byte
operator|)
literal|4
argument_list|)
expr_stmt|;
name|o
operator|.
name|writeDouble
argument_list|(
operator|(
name|double
operator|)
name|v
argument_list|)
expr_stmt|;
block|}
argument_list|)
expr_stmt|;
name|writers
operator|.
name|put
argument_list|(
name|Boolean
operator|.
name|class
argument_list|,
parameter_list|(
name|o
parameter_list|,
name|v
parameter_list|)
lambda|->
block|{
name|o
operator|.
name|writeByte
argument_list|(
operator|(
name|byte
operator|)
literal|5
argument_list|)
expr_stmt|;
name|o
operator|.
name|writeBoolean
argument_list|(
operator|(
name|boolean
operator|)
name|v
argument_list|)
expr_stmt|;
block|}
argument_list|)
expr_stmt|;
name|writers
operator|.
name|put
argument_list|(
name|byte
index|[]
operator|.
expr|class
argument_list|,
parameter_list|(
name|o
parameter_list|,
name|v
parameter_list|)
lambda|->
block|{
name|o
operator|.
name|writeByte
argument_list|(
operator|(
name|byte
operator|)
literal|6
argument_list|)
expr_stmt|;
specifier|final
name|byte
index|[]
name|bytes
init|=
operator|(
name|byte
index|[]
operator|)
name|v
decl_stmt|;
name|o
operator|.
name|writeVInt
argument_list|(
name|bytes
operator|.
name|length
argument_list|)
expr_stmt|;
name|o
operator|.
name|writeBytes
argument_list|(
name|bytes
argument_list|)
expr_stmt|;
block|}
argument_list|)
expr_stmt|;
name|writers
operator|.
name|put
argument_list|(
name|List
operator|.
name|class
argument_list|,
parameter_list|(
name|o
parameter_list|,
name|v
parameter_list|)
lambda|->
block|{
name|o
operator|.
name|writeByte
argument_list|(
operator|(
name|byte
operator|)
literal|7
argument_list|)
expr_stmt|;
specifier|final
name|List
name|list
init|=
operator|(
name|List
operator|)
name|v
decl_stmt|;
name|o
operator|.
name|writeVInt
argument_list|(
name|list
operator|.
name|size
argument_list|()
argument_list|)
expr_stmt|;
for|for
control|(
name|Object
name|item
range|:
name|list
control|)
block|{
name|o
operator|.
name|writeGenericValue
argument_list|(
name|item
argument_list|)
expr_stmt|;
block|}
block|}
argument_list|)
expr_stmt|;
name|writers
operator|.
name|put
argument_list|(
name|Object
index|[]
operator|.
expr|class
argument_list|,
parameter_list|(
name|o
parameter_list|,
name|v
parameter_list|)
lambda|->
block|{
name|o
operator|.
name|writeByte
argument_list|(
operator|(
name|byte
operator|)
literal|8
argument_list|)
expr_stmt|;
specifier|final
name|Object
index|[]
name|list
init|=
operator|(
name|Object
index|[]
operator|)
name|v
decl_stmt|;
name|o
operator|.
name|writeVInt
argument_list|(
name|list
operator|.
name|length
argument_list|)
expr_stmt|;
for|for
control|(
name|Object
name|item
range|:
name|list
control|)
block|{
name|o
operator|.
name|writeGenericValue
argument_list|(
name|item
argument_list|)
expr_stmt|;
block|}
block|}
argument_list|)
expr_stmt|;
name|writers
operator|.
name|put
argument_list|(
name|Map
operator|.
name|class
argument_list|,
parameter_list|(
name|o
parameter_list|,
name|v
parameter_list|)
lambda|->
block|{
if|if
condition|(
name|v
operator|instanceof
name|LinkedHashMap
condition|)
block|{
name|o
operator|.
name|writeByte
argument_list|(
operator|(
name|byte
operator|)
literal|9
argument_list|)
expr_stmt|;
block|}
else|else
block|{
name|o
operator|.
name|writeByte
argument_list|(
operator|(
name|byte
operator|)
literal|10
argument_list|)
expr_stmt|;
block|}
annotation|@
name|SuppressWarnings
argument_list|(
literal|"unchecked"
argument_list|)
specifier|final
name|Map
argument_list|<
name|String
argument_list|,
name|Object
argument_list|>
name|map
init|=
operator|(
name|Map
argument_list|<
name|String
argument_list|,
name|Object
argument_list|>
operator|)
name|v
decl_stmt|;
name|o
operator|.
name|writeVInt
argument_list|(
name|map
operator|.
name|size
argument_list|()
argument_list|)
expr_stmt|;
for|for
control|(
name|Map
operator|.
name|Entry
argument_list|<
name|String
argument_list|,
name|Object
argument_list|>
name|entry
range|:
name|map
operator|.
name|entrySet
argument_list|()
control|)
block|{
name|o
operator|.
name|writeString
argument_list|(
name|entry
operator|.
name|getKey
argument_list|()
argument_list|)
expr_stmt|;
name|o
operator|.
name|writeGenericValue
argument_list|(
name|entry
operator|.
name|getValue
argument_list|()
argument_list|)
expr_stmt|;
block|}
block|}
argument_list|)
expr_stmt|;
name|writers
operator|.
name|put
argument_list|(
name|Byte
operator|.
name|class
argument_list|,
parameter_list|(
name|o
parameter_list|,
name|v
parameter_list|)
lambda|->
block|{
name|o
operator|.
name|writeByte
argument_list|(
operator|(
name|byte
operator|)
literal|11
argument_list|)
expr_stmt|;
name|o
operator|.
name|writeByte
argument_list|(
operator|(
name|Byte
operator|)
name|v
argument_list|)
expr_stmt|;
block|}
argument_list|)
expr_stmt|;
name|writers
operator|.
name|put
argument_list|(
name|Date
operator|.
name|class
argument_list|,
parameter_list|(
name|o
parameter_list|,
name|v
parameter_list|)
lambda|->
block|{
name|o
operator|.
name|writeByte
argument_list|(
operator|(
name|byte
operator|)
literal|12
argument_list|)
expr_stmt|;
name|o
operator|.
name|writeLong
argument_list|(
operator|(
operator|(
name|Date
operator|)
name|v
operator|)
operator|.
name|getTime
argument_list|()
argument_list|)
expr_stmt|;
block|}
argument_list|)
expr_stmt|;
name|writers
operator|.
name|put
argument_list|(
name|ReadableInstant
operator|.
name|class
argument_list|,
parameter_list|(
name|o
parameter_list|,
name|v
parameter_list|)
lambda|->
block|{
name|o
operator|.
name|writeByte
argument_list|(
operator|(
name|byte
operator|)
literal|13
argument_list|)
expr_stmt|;
specifier|final
name|ReadableInstant
name|instant
init|=
operator|(
name|ReadableInstant
operator|)
name|v
decl_stmt|;
name|o
operator|.
name|writeString
argument_list|(
name|instant
operator|.
name|getZone
argument_list|()
operator|.
name|getID
argument_list|()
argument_list|)
expr_stmt|;
name|o
operator|.
name|writeLong
argument_list|(
name|instant
operator|.
name|getMillis
argument_list|()
argument_list|)
expr_stmt|;
block|}
argument_list|)
expr_stmt|;
name|writers
operator|.
name|put
argument_list|(
name|BytesReference
operator|.
name|class
argument_list|,
parameter_list|(
name|o
parameter_list|,
name|v
parameter_list|)
lambda|->
block|{
name|o
operator|.
name|writeByte
argument_list|(
operator|(
name|byte
operator|)
literal|14
argument_list|)
expr_stmt|;
name|o
operator|.
name|writeBytesReference
argument_list|(
operator|(
name|BytesReference
operator|)
name|v
argument_list|)
expr_stmt|;
block|}
argument_list|)
expr_stmt|;
name|writers
operator|.
name|put
argument_list|(
name|Text
operator|.
name|class
argument_list|,
parameter_list|(
name|o
parameter_list|,
name|v
parameter_list|)
lambda|->
block|{
name|o
operator|.
name|writeByte
argument_list|(
operator|(
name|byte
operator|)
literal|15
argument_list|)
expr_stmt|;
name|o
operator|.
name|writeText
argument_list|(
operator|(
name|Text
operator|)
name|v
argument_list|)
expr_stmt|;
block|}
argument_list|)
expr_stmt|;
name|writers
operator|.
name|put
argument_list|(
name|Short
operator|.
name|class
argument_list|,
parameter_list|(
name|o
parameter_list|,
name|v
parameter_list|)
lambda|->
block|{
name|o
operator|.
name|writeByte
argument_list|(
operator|(
name|byte
operator|)
literal|16
argument_list|)
expr_stmt|;
name|o
operator|.
name|writeShort
argument_list|(
operator|(
name|Short
operator|)
name|v
argument_list|)
expr_stmt|;
block|}
argument_list|)
expr_stmt|;
name|writers
operator|.
name|put
argument_list|(
name|int
index|[]
operator|.
expr|class
argument_list|,
parameter_list|(
name|o
parameter_list|,
name|v
parameter_list|)
lambda|->
block|{
name|o
operator|.
name|writeByte
argument_list|(
operator|(
name|byte
operator|)
literal|17
argument_list|)
expr_stmt|;
name|o
operator|.
name|writeIntArray
argument_list|(
operator|(
name|int
index|[]
operator|)
name|v
argument_list|)
expr_stmt|;
block|}
argument_list|)
expr_stmt|;
name|writers
operator|.
name|put
argument_list|(
name|long
index|[]
operator|.
expr|class
argument_list|,
parameter_list|(
name|o
parameter_list|,
name|v
parameter_list|)
lambda|->
block|{
name|o
operator|.
name|writeByte
argument_list|(
operator|(
name|byte
operator|)
literal|18
argument_list|)
expr_stmt|;
name|o
operator|.
name|writeLongArray
argument_list|(
operator|(
name|long
index|[]
operator|)
name|v
argument_list|)
expr_stmt|;
block|}
argument_list|)
expr_stmt|;
name|writers
operator|.
name|put
argument_list|(
name|float
index|[]
operator|.
expr|class
argument_list|,
parameter_list|(
name|o
parameter_list|,
name|v
parameter_list|)
lambda|->
block|{
name|o
operator|.
name|writeByte
argument_list|(
operator|(
name|byte
operator|)
literal|19
argument_list|)
expr_stmt|;
name|o
operator|.
name|writeFloatArray
argument_list|(
operator|(
name|float
index|[]
operator|)
name|v
argument_list|)
expr_stmt|;
block|}
argument_list|)
expr_stmt|;
name|writers
operator|.
name|put
argument_list|(
name|double
index|[]
operator|.
expr|class
argument_list|,
parameter_list|(
name|o
parameter_list|,
name|v
parameter_list|)
lambda|->
block|{
name|o
operator|.
name|writeByte
argument_list|(
operator|(
name|byte
operator|)
literal|20
argument_list|)
expr_stmt|;
name|o
operator|.
name|writeDoubleArray
argument_list|(
operator|(
name|double
index|[]
operator|)
name|v
argument_list|)
expr_stmt|;
block|}
argument_list|)
expr_stmt|;
name|writers
operator|.
name|put
argument_list|(
name|BytesRef
operator|.
name|class
argument_list|,
parameter_list|(
name|o
parameter_list|,
name|v
parameter_list|)
lambda|->
block|{
name|o
operator|.
name|writeByte
argument_list|(
operator|(
name|byte
operator|)
literal|21
argument_list|)
expr_stmt|;
name|o
operator|.
name|writeBytesRef
argument_list|(
operator|(
name|BytesRef
operator|)
name|v
argument_list|)
expr_stmt|;
block|}
argument_list|)
expr_stmt|;
name|writers
operator|.
name|put
argument_list|(
name|GeoPoint
operator|.
name|class
argument_list|,
parameter_list|(
name|o
parameter_list|,
name|v
parameter_list|)
lambda|->
block|{
name|o
operator|.
name|writeByte
argument_list|(
operator|(
name|byte
operator|)
literal|22
argument_list|)
expr_stmt|;
name|o
operator|.
name|writeGeoPoint
argument_list|(
operator|(
name|GeoPoint
operator|)
name|v
argument_list|)
expr_stmt|;
block|}
argument_list|)
expr_stmt|;
name|WRITERS
operator|=
name|Collections
operator|.
name|unmodifiableMap
argument_list|(
name|writers
argument_list|)
expr_stmt|;
block|}
comment|/**      * Notice: when serialization a map, the stream out map with the stream in map maybe have the      * different key-value orders, they will maybe have different stream order.      * If want to keep stream out map and stream in map have the same stream order when stream,      * can use {@code writeMapWithConsistentOrder}      */
DECL|method|writeGenericValue
specifier|public
name|void
name|writeGenericValue
parameter_list|(
annotation|@
name|Nullable
name|Object
name|value
parameter_list|)
throws|throws
name|IOException
block|{
if|if
condition|(
name|value
operator|==
literal|null
condition|)
block|{
name|writeByte
argument_list|(
operator|(
name|byte
operator|)
operator|-
literal|1
argument_list|)
expr_stmt|;
return|return;
block|}
specifier|final
name|Class
name|type
decl_stmt|;
if|if
condition|(
name|value
operator|instanceof
name|List
condition|)
block|{
name|type
operator|=
name|List
operator|.
name|class
expr_stmt|;
block|}
elseif|else
if|if
condition|(
name|value
operator|instanceof
name|Object
index|[]
condition|)
block|{
name|type
operator|=
name|Object
index|[]
operator|.
name|class
expr_stmt|;
block|}
elseif|else
if|if
condition|(
name|value
operator|instanceof
name|Map
condition|)
block|{
name|type
operator|=
name|Map
operator|.
name|class
expr_stmt|;
block|}
elseif|else
if|if
condition|(
name|value
operator|instanceof
name|ReadableInstant
condition|)
block|{
name|type
operator|=
name|ReadableInstant
operator|.
name|class
expr_stmt|;
block|}
elseif|else
if|if
condition|(
name|value
operator|instanceof
name|BytesReference
condition|)
block|{
name|type
operator|=
name|BytesReference
operator|.
name|class
expr_stmt|;
block|}
else|else
block|{
name|type
operator|=
name|value
operator|.
name|getClass
argument_list|()
expr_stmt|;
block|}
specifier|final
name|Writer
name|writer
init|=
name|WRITERS
operator|.
name|get
argument_list|(
name|type
argument_list|)
decl_stmt|;
if|if
condition|(
name|writer
operator|!=
literal|null
condition|)
block|{
name|writer
operator|.
name|write
argument_list|(
name|this
argument_list|,
name|value
argument_list|)
expr_stmt|;
block|}
else|else
block|{
throw|throw
operator|new
name|IOException
argument_list|(
literal|"can not write type ["
operator|+
name|type
operator|+
literal|"]"
argument_list|)
throw|;
block|}
block|}
DECL|method|writeIntArray
specifier|public
name|void
name|writeIntArray
parameter_list|(
name|int
index|[]
name|values
parameter_list|)
throws|throws
name|IOException
block|{
name|writeVInt
argument_list|(
name|values
operator|.
name|length
argument_list|)
expr_stmt|;
for|for
control|(
name|int
name|value
range|:
name|values
control|)
block|{
name|writeInt
argument_list|(
name|value
argument_list|)
expr_stmt|;
block|}
block|}
DECL|method|writeVIntArray
specifier|public
name|void
name|writeVIntArray
parameter_list|(
name|int
index|[]
name|values
parameter_list|)
throws|throws
name|IOException
block|{
name|writeVInt
argument_list|(
name|values
operator|.
name|length
argument_list|)
expr_stmt|;
for|for
control|(
name|int
name|value
range|:
name|values
control|)
block|{
name|writeVInt
argument_list|(
name|value
argument_list|)
expr_stmt|;
block|}
block|}
DECL|method|writeLongArray
specifier|public
name|void
name|writeLongArray
parameter_list|(
name|long
index|[]
name|values
parameter_list|)
throws|throws
name|IOException
block|{
name|writeVInt
argument_list|(
name|values
operator|.
name|length
argument_list|)
expr_stmt|;
for|for
control|(
name|long
name|value
range|:
name|values
control|)
block|{
name|writeLong
argument_list|(
name|value
argument_list|)
expr_stmt|;
block|}
block|}
DECL|method|writeVLongArray
specifier|public
name|void
name|writeVLongArray
parameter_list|(
name|long
index|[]
name|values
parameter_list|)
throws|throws
name|IOException
block|{
name|writeVInt
argument_list|(
name|values
operator|.
name|length
argument_list|)
expr_stmt|;
for|for
control|(
name|long
name|value
range|:
name|values
control|)
block|{
name|writeVLong
argument_list|(
name|value
argument_list|)
expr_stmt|;
block|}
block|}
DECL|method|writeFloatArray
specifier|public
name|void
name|writeFloatArray
parameter_list|(
name|float
index|[]
name|values
parameter_list|)
throws|throws
name|IOException
block|{
name|writeVInt
argument_list|(
name|values
operator|.
name|length
argument_list|)
expr_stmt|;
for|for
control|(
name|float
name|value
range|:
name|values
control|)
block|{
name|writeFloat
argument_list|(
name|value
argument_list|)
expr_stmt|;
block|}
block|}
DECL|method|writeDoubleArray
specifier|public
name|void
name|writeDoubleArray
parameter_list|(
name|double
index|[]
name|values
parameter_list|)
throws|throws
name|IOException
block|{
name|writeVInt
argument_list|(
name|values
operator|.
name|length
argument_list|)
expr_stmt|;
for|for
control|(
name|double
name|value
range|:
name|values
control|)
block|{
name|writeDouble
argument_list|(
name|value
argument_list|)
expr_stmt|;
block|}
block|}
DECL|method|writeArray
specifier|public
parameter_list|<
name|T
extends|extends
name|Writeable
parameter_list|>
name|void
name|writeArray
parameter_list|(
name|T
index|[]
name|array
parameter_list|)
throws|throws
name|IOException
block|{
name|writeVInt
argument_list|(
name|array
operator|.
name|length
argument_list|)
expr_stmt|;
for|for
control|(
name|T
name|value
range|:
name|array
control|)
block|{
name|value
operator|.
name|writeTo
argument_list|(
name|this
argument_list|)
expr_stmt|;
block|}
block|}
DECL|method|writeOptionalArray
specifier|public
parameter_list|<
name|T
extends|extends
name|Writeable
parameter_list|>
name|void
name|writeOptionalArray
parameter_list|(
annotation|@
name|Nullable
name|T
index|[]
name|array
parameter_list|)
throws|throws
name|IOException
block|{
if|if
condition|(
name|array
operator|==
literal|null
condition|)
block|{
name|writeBoolean
argument_list|(
literal|false
argument_list|)
expr_stmt|;
block|}
else|else
block|{
name|writeBoolean
argument_list|(
literal|true
argument_list|)
expr_stmt|;
name|writeArray
argument_list|(
name|array
argument_list|)
expr_stmt|;
block|}
block|}
comment|/**      * Serializes a potential null value.      */
DECL|method|writeOptionalStreamable
specifier|public
name|void
name|writeOptionalStreamable
parameter_list|(
annotation|@
name|Nullable
name|Streamable
name|streamable
parameter_list|)
throws|throws
name|IOException
block|{
if|if
condition|(
name|streamable
operator|!=
literal|null
condition|)
block|{
name|writeBoolean
argument_list|(
literal|true
argument_list|)
expr_stmt|;
name|streamable
operator|.
name|writeTo
argument_list|(
name|this
argument_list|)
expr_stmt|;
block|}
else|else
block|{
name|writeBoolean
argument_list|(
literal|false
argument_list|)
expr_stmt|;
block|}
block|}
DECL|method|writeOptionalWriteable
specifier|public
name|void
name|writeOptionalWriteable
parameter_list|(
annotation|@
name|Nullable
name|Writeable
name|writeable
parameter_list|)
throws|throws
name|IOException
block|{
if|if
condition|(
name|writeable
operator|!=
literal|null
condition|)
block|{
name|writeBoolean
argument_list|(
literal|true
argument_list|)
expr_stmt|;
name|writeable
operator|.
name|writeTo
argument_list|(
name|this
argument_list|)
expr_stmt|;
block|}
else|else
block|{
name|writeBoolean
argument_list|(
literal|false
argument_list|)
expr_stmt|;
block|}
block|}
DECL|method|writeException
specifier|public
name|void
name|writeException
parameter_list|(
name|Throwable
name|throwable
parameter_list|)
throws|throws
name|IOException
block|{
if|if
condition|(
name|throwable
operator|==
literal|null
condition|)
block|{
name|writeBoolean
argument_list|(
literal|false
argument_list|)
expr_stmt|;
block|}
else|else
block|{
name|writeBoolean
argument_list|(
literal|true
argument_list|)
expr_stmt|;
name|boolean
name|writeCause
init|=
literal|true
decl_stmt|;
name|boolean
name|writeMessage
init|=
literal|true
decl_stmt|;
if|if
condition|(
name|throwable
operator|instanceof
name|CorruptIndexException
condition|)
block|{
name|writeVInt
argument_list|(
literal|1
argument_list|)
expr_stmt|;
name|writeOptionalString
argument_list|(
operator|(
operator|(
name|CorruptIndexException
operator|)
name|throwable
operator|)
operator|.
name|getOriginalMessage
argument_list|()
argument_list|)
expr_stmt|;
name|writeOptionalString
argument_list|(
operator|(
operator|(
name|CorruptIndexException
operator|)
name|throwable
operator|)
operator|.
name|getResourceDescription
argument_list|()
argument_list|)
expr_stmt|;
name|writeMessage
operator|=
literal|false
expr_stmt|;
block|}
elseif|else
if|if
condition|(
name|throwable
operator|instanceof
name|IndexFormatTooNewException
condition|)
block|{
name|writeVInt
argument_list|(
literal|2
argument_list|)
expr_stmt|;
name|writeOptionalString
argument_list|(
operator|(
operator|(
name|IndexFormatTooNewException
operator|)
name|throwable
operator|)
operator|.
name|getResourceDescription
argument_list|()
argument_list|)
expr_stmt|;
name|writeInt
argument_list|(
operator|(
operator|(
name|IndexFormatTooNewException
operator|)
name|throwable
operator|)
operator|.
name|getVersion
argument_list|()
argument_list|)
expr_stmt|;
name|writeInt
argument_list|(
operator|(
operator|(
name|IndexFormatTooNewException
operator|)
name|throwable
operator|)
operator|.
name|getMinVersion
argument_list|()
argument_list|)
expr_stmt|;
name|writeInt
argument_list|(
operator|(
operator|(
name|IndexFormatTooNewException
operator|)
name|throwable
operator|)
operator|.
name|getMaxVersion
argument_list|()
argument_list|)
expr_stmt|;
name|writeMessage
operator|=
literal|false
expr_stmt|;
name|writeCause
operator|=
literal|false
expr_stmt|;
block|}
elseif|else
if|if
condition|(
name|throwable
operator|instanceof
name|IndexFormatTooOldException
condition|)
block|{
name|writeVInt
argument_list|(
literal|3
argument_list|)
expr_stmt|;
name|IndexFormatTooOldException
name|t
init|=
operator|(
name|IndexFormatTooOldException
operator|)
name|throwable
decl_stmt|;
name|writeOptionalString
argument_list|(
name|t
operator|.
name|getResourceDescription
argument_list|()
argument_list|)
expr_stmt|;
if|if
condition|(
name|t
operator|.
name|getVersion
argument_list|()
operator|==
literal|null
condition|)
block|{
name|writeBoolean
argument_list|(
literal|false
argument_list|)
expr_stmt|;
name|writeOptionalString
argument_list|(
name|t
operator|.
name|getReason
argument_list|()
argument_list|)
expr_stmt|;
block|}
else|else
block|{
name|writeBoolean
argument_list|(
literal|true
argument_list|)
expr_stmt|;
name|writeInt
argument_list|(
name|t
operator|.
name|getVersion
argument_list|()
argument_list|)
expr_stmt|;
name|writeInt
argument_list|(
name|t
operator|.
name|getMinVersion
argument_list|()
argument_list|)
expr_stmt|;
name|writeInt
argument_list|(
name|t
operator|.
name|getMaxVersion
argument_list|()
argument_list|)
expr_stmt|;
block|}
name|writeMessage
operator|=
literal|false
expr_stmt|;
name|writeCause
operator|=
literal|false
expr_stmt|;
block|}
elseif|else
if|if
condition|(
name|throwable
operator|instanceof
name|NullPointerException
condition|)
block|{
name|writeVInt
argument_list|(
literal|4
argument_list|)
expr_stmt|;
name|writeCause
operator|=
literal|false
expr_stmt|;
block|}
elseif|else
if|if
condition|(
name|throwable
operator|instanceof
name|NumberFormatException
condition|)
block|{
name|writeVInt
argument_list|(
literal|5
argument_list|)
expr_stmt|;
name|writeCause
operator|=
literal|false
expr_stmt|;
block|}
elseif|else
if|if
condition|(
name|throwable
operator|instanceof
name|IllegalArgumentException
condition|)
block|{
name|writeVInt
argument_list|(
literal|6
argument_list|)
expr_stmt|;
block|}
elseif|else
if|if
condition|(
name|throwable
operator|instanceof
name|AlreadyClosedException
condition|)
block|{
name|writeVInt
argument_list|(
literal|7
argument_list|)
expr_stmt|;
block|}
elseif|else
if|if
condition|(
name|throwable
operator|instanceof
name|EOFException
condition|)
block|{
name|writeVInt
argument_list|(
literal|8
argument_list|)
expr_stmt|;
name|writeCause
operator|=
literal|false
expr_stmt|;
block|}
elseif|else
if|if
condition|(
name|throwable
operator|instanceof
name|SecurityException
condition|)
block|{
name|writeVInt
argument_list|(
literal|9
argument_list|)
expr_stmt|;
block|}
elseif|else
if|if
condition|(
name|throwable
operator|instanceof
name|StringIndexOutOfBoundsException
condition|)
block|{
name|writeVInt
argument_list|(
literal|10
argument_list|)
expr_stmt|;
name|writeCause
operator|=
literal|false
expr_stmt|;
block|}
elseif|else
if|if
condition|(
name|throwable
operator|instanceof
name|ArrayIndexOutOfBoundsException
condition|)
block|{
name|writeVInt
argument_list|(
literal|11
argument_list|)
expr_stmt|;
name|writeCause
operator|=
literal|false
expr_stmt|;
block|}
elseif|else
if|if
condition|(
name|throwable
operator|instanceof
name|FileNotFoundException
condition|)
block|{
name|writeVInt
argument_list|(
literal|12
argument_list|)
expr_stmt|;
name|writeCause
operator|=
literal|false
expr_stmt|;
block|}
elseif|else
if|if
condition|(
name|throwable
operator|instanceof
name|FileSystemException
condition|)
block|{
name|writeVInt
argument_list|(
literal|13
argument_list|)
expr_stmt|;
if|if
condition|(
name|throwable
operator|instanceof
name|NoSuchFileException
condition|)
block|{
name|writeVInt
argument_list|(
literal|0
argument_list|)
expr_stmt|;
block|}
elseif|else
if|if
condition|(
name|throwable
operator|instanceof
name|NotDirectoryException
condition|)
block|{
name|writeVInt
argument_list|(
literal|1
argument_list|)
expr_stmt|;
block|}
elseif|else
if|if
condition|(
name|throwable
operator|instanceof
name|DirectoryNotEmptyException
condition|)
block|{
name|writeVInt
argument_list|(
literal|2
argument_list|)
expr_stmt|;
block|}
elseif|else
if|if
condition|(
name|throwable
operator|instanceof
name|AtomicMoveNotSupportedException
condition|)
block|{
name|writeVInt
argument_list|(
literal|3
argument_list|)
expr_stmt|;
block|}
elseif|else
if|if
condition|(
name|throwable
operator|instanceof
name|FileAlreadyExistsException
condition|)
block|{
name|writeVInt
argument_list|(
literal|4
argument_list|)
expr_stmt|;
block|}
elseif|else
if|if
condition|(
name|throwable
operator|instanceof
name|AccessDeniedException
condition|)
block|{
name|writeVInt
argument_list|(
literal|5
argument_list|)
expr_stmt|;
block|}
elseif|else
if|if
condition|(
name|throwable
operator|instanceof
name|FileSystemLoopException
condition|)
block|{
name|writeVInt
argument_list|(
literal|6
argument_list|)
expr_stmt|;
block|}
else|else
block|{
name|writeVInt
argument_list|(
literal|7
argument_list|)
expr_stmt|;
block|}
name|writeOptionalString
argument_list|(
operator|(
operator|(
name|FileSystemException
operator|)
name|throwable
operator|)
operator|.
name|getFile
argument_list|()
argument_list|)
expr_stmt|;
name|writeOptionalString
argument_list|(
operator|(
operator|(
name|FileSystemException
operator|)
name|throwable
operator|)
operator|.
name|getOtherFile
argument_list|()
argument_list|)
expr_stmt|;
name|writeOptionalString
argument_list|(
operator|(
operator|(
name|FileSystemException
operator|)
name|throwable
operator|)
operator|.
name|getReason
argument_list|()
argument_list|)
expr_stmt|;
name|writeCause
operator|=
literal|false
expr_stmt|;
block|}
elseif|else
if|if
condition|(
name|throwable
operator|instanceof
name|IllegalStateException
condition|)
block|{
name|writeVInt
argument_list|(
literal|14
argument_list|)
expr_stmt|;
block|}
elseif|else
if|if
condition|(
name|throwable
operator|instanceof
name|LockObtainFailedException
condition|)
block|{
name|writeVInt
argument_list|(
literal|15
argument_list|)
expr_stmt|;
block|}
elseif|else
if|if
condition|(
name|throwable
operator|instanceof
name|InterruptedException
condition|)
block|{
name|writeVInt
argument_list|(
literal|16
argument_list|)
expr_stmt|;
name|writeCause
operator|=
literal|false
expr_stmt|;
block|}
elseif|else
if|if
condition|(
name|throwable
operator|instanceof
name|IOException
condition|)
block|{
name|writeVInt
argument_list|(
literal|17
argument_list|)
expr_stmt|;
block|}
else|else
block|{
name|ElasticsearchException
name|ex
decl_stmt|;
if|if
condition|(
name|throwable
operator|instanceof
name|ElasticsearchException
operator|&&
name|ElasticsearchException
operator|.
name|isRegistered
argument_list|(
name|throwable
operator|.
name|getClass
argument_list|()
argument_list|,
name|version
argument_list|)
condition|)
block|{
name|ex
operator|=
operator|(
name|ElasticsearchException
operator|)
name|throwable
expr_stmt|;
block|}
else|else
block|{
name|ex
operator|=
operator|new
name|NotSerializableExceptionWrapper
argument_list|(
name|throwable
argument_list|)
expr_stmt|;
block|}
name|writeVInt
argument_list|(
literal|0
argument_list|)
expr_stmt|;
name|writeVInt
argument_list|(
name|ElasticsearchException
operator|.
name|getId
argument_list|(
name|ex
operator|.
name|getClass
argument_list|()
argument_list|)
argument_list|)
expr_stmt|;
name|ex
operator|.
name|writeTo
argument_list|(
name|this
argument_list|)
expr_stmt|;
return|return;
block|}
if|if
condition|(
name|writeMessage
condition|)
block|{
name|writeOptionalString
argument_list|(
name|throwable
operator|.
name|getMessage
argument_list|()
argument_list|)
expr_stmt|;
block|}
if|if
condition|(
name|writeCause
condition|)
block|{
name|writeException
argument_list|(
name|throwable
operator|.
name|getCause
argument_list|()
argument_list|)
expr_stmt|;
block|}
name|ElasticsearchException
operator|.
name|writeStackTraces
argument_list|(
name|throwable
argument_list|,
name|this
argument_list|)
expr_stmt|;
block|}
block|}
comment|/**      * Writes a {@link NamedWriteable} to the current stream, by first writing its name and then the object itself      */
DECL|method|writeNamedWriteable
specifier|public
name|void
name|writeNamedWriteable
parameter_list|(
name|NamedWriteable
name|namedWriteable
parameter_list|)
throws|throws
name|IOException
block|{
name|writeString
argument_list|(
name|namedWriteable
operator|.
name|getWriteableName
argument_list|()
argument_list|)
expr_stmt|;
name|namedWriteable
operator|.
name|writeTo
argument_list|(
name|this
argument_list|)
expr_stmt|;
block|}
comment|/**      * Write an optional {@link NamedWriteable} to the stream.      */
DECL|method|writeOptionalNamedWriteable
specifier|public
name|void
name|writeOptionalNamedWriteable
parameter_list|(
annotation|@
name|Nullable
name|NamedWriteable
name|namedWriteable
parameter_list|)
throws|throws
name|IOException
block|{
if|if
condition|(
name|namedWriteable
operator|==
literal|null
condition|)
block|{
name|writeBoolean
argument_list|(
literal|false
argument_list|)
expr_stmt|;
block|}
else|else
block|{
name|writeBoolean
argument_list|(
literal|true
argument_list|)
expr_stmt|;
name|writeNamedWriteable
argument_list|(
name|namedWriteable
argument_list|)
expr_stmt|;
block|}
block|}
comment|/**      * Writes the given {@link GeoPoint} to the stream      */
DECL|method|writeGeoPoint
specifier|public
name|void
name|writeGeoPoint
parameter_list|(
name|GeoPoint
name|geoPoint
parameter_list|)
throws|throws
name|IOException
block|{
name|writeDouble
argument_list|(
name|geoPoint
operator|.
name|lat
argument_list|()
argument_list|)
expr_stmt|;
name|writeDouble
argument_list|(
name|geoPoint
operator|.
name|lon
argument_list|()
argument_list|)
expr_stmt|;
block|}
comment|/**      * Write a {@linkplain DateTimeZone} to the stream.      */
DECL|method|writeTimeZone
specifier|public
name|void
name|writeTimeZone
parameter_list|(
name|DateTimeZone
name|timeZone
parameter_list|)
throws|throws
name|IOException
block|{
name|writeString
argument_list|(
name|timeZone
operator|.
name|getID
argument_list|()
argument_list|)
expr_stmt|;
block|}
comment|/**      * Write an optional {@linkplain DateTimeZone} to the stream.      */
DECL|method|writeOptionalTimeZone
specifier|public
name|void
name|writeOptionalTimeZone
parameter_list|(
annotation|@
name|Nullable
name|DateTimeZone
name|timeZone
parameter_list|)
throws|throws
name|IOException
block|{
if|if
condition|(
name|timeZone
operator|==
literal|null
condition|)
block|{
name|writeBoolean
argument_list|(
literal|false
argument_list|)
expr_stmt|;
block|}
else|else
block|{
name|writeBoolean
argument_list|(
literal|true
argument_list|)
expr_stmt|;
name|writeTimeZone
argument_list|(
name|timeZone
argument_list|)
expr_stmt|;
block|}
block|}
comment|/**      * Writes a list of {@link Streamable} objects      */
DECL|method|writeStreamableList
specifier|public
name|void
name|writeStreamableList
parameter_list|(
name|List
argument_list|<
name|?
extends|extends
name|Streamable
argument_list|>
name|list
parameter_list|)
throws|throws
name|IOException
block|{
name|writeVInt
argument_list|(
name|list
operator|.
name|size
argument_list|()
argument_list|)
expr_stmt|;
for|for
control|(
name|Streamable
name|obj
range|:
name|list
control|)
block|{
name|obj
operator|.
name|writeTo
argument_list|(
name|this
argument_list|)
expr_stmt|;
block|}
block|}
comment|/**      * Writes a list of {@link Writeable} objects      */
DECL|method|writeList
specifier|public
name|void
name|writeList
parameter_list|(
name|List
argument_list|<
name|?
extends|extends
name|Writeable
argument_list|>
name|list
parameter_list|)
throws|throws
name|IOException
block|{
name|writeVInt
argument_list|(
name|list
operator|.
name|size
argument_list|()
argument_list|)
expr_stmt|;
for|for
control|(
name|Writeable
name|obj
range|:
name|list
control|)
block|{
name|obj
operator|.
name|writeTo
argument_list|(
name|this
argument_list|)
expr_stmt|;
block|}
block|}
comment|/**      * Writes a list of strings      */
DECL|method|writeStringList
specifier|public
name|void
name|writeStringList
parameter_list|(
name|List
argument_list|<
name|String
argument_list|>
name|list
parameter_list|)
throws|throws
name|IOException
block|{
name|writeVInt
argument_list|(
name|list
operator|.
name|size
argument_list|()
argument_list|)
expr_stmt|;
for|for
control|(
name|String
name|string
range|:
name|list
control|)
block|{
name|this
operator|.
name|writeString
argument_list|(
name|string
argument_list|)
expr_stmt|;
block|}
block|}
comment|/**      * Writes a list of {@link NamedWriteable} objects.      */
DECL|method|writeNamedWriteableList
specifier|public
name|void
name|writeNamedWriteableList
parameter_list|(
name|List
argument_list|<
name|?
extends|extends
name|NamedWriteable
argument_list|>
name|list
parameter_list|)
throws|throws
name|IOException
block|{
name|writeVInt
argument_list|(
name|list
operator|.
name|size
argument_list|()
argument_list|)
expr_stmt|;
for|for
control|(
name|NamedWriteable
name|obj
range|:
name|list
control|)
block|{
name|writeNamedWriteable
argument_list|(
name|obj
argument_list|)
expr_stmt|;
block|}
block|}
block|}
end_class

end_unit

