...
condition|)
block|{
comment|// for multi-valued sort field, we replace the SortedSetSortField with a simple SortField.
comment|// It works because the sort field is only used to merge results from different shards.
name|SortField
name|newSortField
init|=
operator|new
