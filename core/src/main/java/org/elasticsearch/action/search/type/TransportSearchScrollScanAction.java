...
name|shardResult
argument_list|,
literal|"QueryFetchSearchResult can't be null"
argument_list|)
expr_stmt|;
name|queryFetchResults
operator|.
...
condition|)
block|{
comment|// we found more than we want for this round, remove this from our scrolling, so we don't go back to
comment|// this shard, since all hits have been processed.
comment|// The SearchContext already gets freed on the node holding the shard, via a similar check.
name|queryFetchResults
operator|.
name|set
argument_list|(
