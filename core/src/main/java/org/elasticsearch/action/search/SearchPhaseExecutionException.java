...
block|{
comment|// We don't have a cause when all shards failed, but we do have shards failures so we can "guess" a cause
comment|// (see {@link #getCause()}). Here, we use super.getCause() because we don't want the guessed exception to
comment|// be rendered twice (one in the "cause" field, one in "failed_shards")
