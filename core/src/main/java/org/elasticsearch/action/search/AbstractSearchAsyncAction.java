...
parameter_list|)
block|{
comment|// this is only a temporary fix since field collapsing executes a blocking call on response
comment|// which could be a network thread. we are fixing this but for now we just fork off again.
comment|// this should be removed once https://github.com/elastic/elasticsearch/issues/23048 is fixed
name|getExecutor
argument_list|()
operator|.
name|execute
