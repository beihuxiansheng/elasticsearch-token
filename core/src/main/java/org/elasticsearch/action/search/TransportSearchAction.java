...
expr_stmt|;
block|}
comment|//TODO this is just a temporary workaround, alias filters need to be retrieved, at the moment they are ignored for remote indices
comment|//they will be retrieved from search_shards from 5.1 on.
for|for
control|(
name|String
name|remoteUUID
...
argument_list|)
decl_stmt|;
comment|//TODO make selection smarter: aliases can still contain any character and remote indices should have the precedence all the time.
comment|//e.g. we could skip the remote logic if no remote clusters are registered. Also don't go remotely if the prefix is not
comment|//a registered cluster rather than throwing an error?
specifier|final
name|List
argument_list|<
name|String
...
index|]
expr_stmt|;
comment|// don't search on ALL if nothing is specified
block|}
else|else
block|{
name|indices
