...
condition|)
block|{
comment|// make sure we set the shard index before we add it - the consumer didn't do that yet
name|setShardIndex
argument_list|(
name|td
argument_list|,
...
condition|)
block|{
comment|// only one shard and no pagination we can just return the topDocs as we got them.
return|return
name|topDocs
return|;
block|}
...
condition|)
block|{
comment|// once there is a single shard index initialized all others will be initialized too
comment|// there are many asserts down in lucene land that this is actually true. we can shortcut it here.
return|return;
block|}
name|doc
operator|.
...
name|numReducePhases
decl_stmt|;
comment|// the searches merged top docs
...
name|scoreDocs
decl_stmt|;
comment|// the top docs sort fields used to sort the score docs,<code>null</code> if the results are not sorted
...
name|sortField
decl_stmt|;
comment|//<code>true</code> iff the result score docs is sorted by a field (not score), this implies that<code>sortField</code> is set.
DECL|field|isSortedByField
specifier|final
name|boolean
name|isSortedByField
decl_stmt|;
comment|// the size of the top hits to return
DECL|field|size
specifier|final
name|int
name|size
decl_stmt|;
comment|//<code>true</code> iff the query phase had no results. Otherwise<code>false</code>
DECL|field|isEmptyResult
specifier|final
name|boolean
name|isEmptyResult
decl_stmt|;
comment|// the offset into the merged top hits
DECL|field|from
specifier|final
name|int
name|from
decl_stmt|;
comment|// sort value formats used to sort / format the result
DECL|field|sortValueFormats
specifier|final
name|DocValueFormat
index|[]
name|sortValueFormats
...
expr_stmt|;
block|}
comment|/**          * Creates a new search response from the given merged hits.          * @see #merge(boolean, ReducedQueryPhase, Collection, IntFunction)          */
...
name|IllegalArgumentException
argument_list|(
literal|"either aggs or top docs must be present"
argument_list|)
throw|;
block|}
name|this
...
name|size
argument_list|()
comment|// we have to merge here in the same way we collect on a shard
argument_list|,
literal|0
argument_list|)
decl_stmt|;
...
return|;
block|}
DECL|method|getRemainingTopDocs
specifier|private
specifier|synchronized
name|List
argument_list|<
...
condition|)
block|{
comment|// no incremental reduce if scroll is used - we only hit a single shard or sometimes more...
if|if
condition|(
name|request
operator|.
