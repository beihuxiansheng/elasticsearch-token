...
argument_list|)
expr_stmt|;
comment|// TODO look up the task's result from the .tasks index now that it is done
name|listener
operator|.
name|onResponse
argument_list|(
...
block|}
block|}
comment|/**      * Send a {@link GetRequest} to the results index looking for the results of the task. It'll only be found only if the task's result was      * persisted. Called on the node that once had the task if that node is part of the cluster or on the coordinating node if the node      * wasn't part of the cluster.      */
