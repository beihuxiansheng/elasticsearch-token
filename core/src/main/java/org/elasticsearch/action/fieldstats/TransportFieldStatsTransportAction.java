...
block|}
block|}
comment|// Check the field with conflicts and remove them.
for|for
control|(
name|String
name|conflictKey
...
condition|)
block|{
comment|// If the field stats didn't occur in the list of fields in the original request
comment|// we need to remove the field stats, because it was never requested and was only needed to
comment|// validate the index constraint.
if|if
condition|(
name|fieldStatFields
operator|.
...
block|{
comment|// The index constraint didn't match or was empty,
comment|// so we remove all the field stats of the index we're checking.
name|iterator
operator|.
name|remove
argument_list|()
