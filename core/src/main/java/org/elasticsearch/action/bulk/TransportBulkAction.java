...
condition|)
block|{
comment|// Attempt to create all the indices that we're going to need during the bulk before we start.
comment|// Step 1: collect all the indices in the request
specifier|final
name|Set
argument_list|<
name|String
...
name|stream
argument_list|()
comment|// delete requests should not attempt to create the index (if the index does not
comment|// exists), unless an external versioning is used
operator|.
name|filter
argument_list|(
name|request
...
argument_list|)
decl_stmt|;
comment|/* Step 2: filter that to indices that don't exist and we can create. At the same time build a map of indices we can't create              * that we'll use when we try to run the requests. */
specifier|final
name|Map
argument_list|<
name|String
...
block|}
block|}
comment|// Step 3: create all the indices that are missing, if there are any missing. start the bulk after all the creates come back.
if|if
condition|(
name|autoCreateIndices
operator|.
...
condition|)
block|{
comment|// fail all requests involving this index, if create didn't work
for|for
control|(
name|int
name|i
...
name|observer
decl_stmt|;
DECL|field|indicesThatCannotBeCreated
specifier|private
specifier|final
name|Map
argument_list|<
