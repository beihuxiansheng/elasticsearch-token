...
DECL|method|processUpdateResponse
specifier|static
name|BulkItemResultHolder
...
operator|:
literal|"failed result should not have a sequence number"
assert|;
name|Engine
operator|.
name|Operation
...
name|replicaRequest
decl_stmt|;
comment|// enrich update response and set translated update (index/delete) request for replica execution in bulk items
if|if
condition|(
name|opType
operator|==
...
return|;
block|}
comment|/**      * Executes update request once, delegating to a index or delete operation after translation.      * NOOP updates are indicated by returning a<code>null</code> operation in {@link BulkItemResultHolder}      */
DECL|method|executeUpdateRequestOnce
specifier|static
name|BulkItemResultHolder
name|executeUpdateRequestOnce
parameter_list|(
name|UpdateRequest
name|updateRequest
parameter_list|,
...
condition|)
block|{
comment|// There was a result, and the result was a failure
return|return
operator|new
name|BulkItemResultHolder
argument_list|(
...
block|{
comment|// It was successful, we need to construct the response and return it
return|return
name|processUpdateResponse
argument_list|(
name|updateRequest
...
block|}
block|}
comment|/**      * Executes update request, delegating to a index or delete operation after translation,      * handles retries on version conflict and constructs update response      * NOOP updates are indicated by returning a<code>null</code> operation      * in {@link BulkItemResultHolder}      */
...
name|updateRequest
operator|:
literal|"expected bulk item request to contain the original update request, got: "
operator|+
name|primaryItemRequest
operator|.
name|request
...
literal|null
decl_stmt|;
comment|// There must be at least one attempt
name|int
name|maxAttempts
init|=
name|Math
...
argument_list|)
expr_stmt|;
comment|// It was either a successful request, or it was a non-conflict failure
if|if
condition|(
name|holder
operator|.
...
block|}
block|}
comment|// We ran out of tries and haven't returned a valid bulk item response, so return the last one generated
return|return
name|holder
return|;
block|}
