...
block|}
block|}
comment|/**      * Primary operation on node with primary copy.      *      * @param shardRequest the request to the primary shard      * @param primary      the primary shard to perform the operation on      */
...
name|request
decl_stmt|;
comment|/** targetAllocationID of the shard this request is meant for */
...
name|replicaRequest
decl_stmt|;
DECL|field|finalResponseIfSuccessful
specifier|public
specifier|final
name|Response
name|finalResponseIfSuccessful
decl_stmt|;
DECL|field|finalFailure
specifier|public
specifier|final
name|Exception
name|finalFailure
...
name|request
decl_stmt|;
comment|// allocation id of the replica this request is meant for
DECL|field|targetAllocationID
specifier|private
specifier|final
name|String
...
argument_list|()
argument_list|,
literal|"expected aID [{}] but found [{}]"
argument_list|,
name|targetAllocationID
argument_list|,
name|actualAllocationId
...
name|trace
argument_list|(
literal|"send action [{}] to local primary [{}] for request [{}] with cluster state version [{}] to [{}] "
argument_list|,
name|transportPrimaryAction
argument_list|,
name|request
...
argument_list|()
operator|+
literal|"] with a primary failure ["
operator|+
name|result
operator|.
name|finalFailure
...
block|{
comment|// 5.x used to read empty responses, which don't really read anything off the stream, so just do nothing.
block|}
block|}
annotation|@
name|Override
...
block|{
comment|// we use to write empty responses
name|Empty
operator|.
name|INSTANCE
operator|.
...
block|}
block|}
comment|/**      * The {@code ReplicasProxy} is an implementation of the {@code Replicas}      * interface that performs the actual {@code ReplicaRequest} on the replica      * shards. It also encapsulates the logic required for failing the replica      * if deemed necessary as well as marking it as stale when needed.      */
...
parameter_list|)
block|{
comment|// This does not need to fail the shard. The idea is that this
comment|// is a non-write operation (something like a refresh or a global
comment|// checkpoint sync) and therefore the replica should still be
comment|// "alive" if it were to fail.
name|onSuccess
operator|.
name|run
argument_list|()
...
parameter_list|)
block|{
comment|// This does not need to make the shard stale. The idea is that this
comment|// is a non-write operation (something like a refresh or a global
comment|// checkpoint sync) and therefore the replica should still be
comment|// "alive" if it were to be marked as stale.
name|onSuccess
operator|.
name|run
argument_list|()
...
block|}
block|}
comment|/**      * Sends the specified replica request to the specified node.      *      * @param replicaRequest the replica request      * @param node           the node to send the request to      * @param listener       callback for handling the response or failure      */
...
name|TransportRequest
block|{
comment|/** {@link AllocationId#getId()} of the shard this request is sent to **/
DECL|field|targetAllocationID
specifier|private
name|String
name|targetAllocationID
...
name|request
decl_stmt|;
DECL|method|ConcreteShardRequest
name|ConcreteShardRequest
parameter_list|(
name|Supplier
argument_list|<
...
argument_list|()
expr_stmt|;
comment|// null now, but will be populated by reading from the streams
name|targetAllocationID
operator|=
literal|null
expr_stmt|;
...
block|}
block|}
DECL|class|ConcreteReplicaRequest
specifier|protected
specifier|static
specifier|final
class|class
...
name|globalCheckpoint
decl_stmt|;
DECL|method|ConcreteReplicaRequest
specifier|public
name|ConcreteReplicaRequest
parameter_list|(
specifier|final
...
block|{
return|return
literal|"ConcreteReplicaRequest{"
operator|+
literal|"targetAllocationID='"
operator|+
name|getTargetAllocationID
argument_list|()
operator|+
