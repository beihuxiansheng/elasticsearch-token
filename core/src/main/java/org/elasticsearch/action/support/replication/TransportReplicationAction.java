...
block|}
block|}
comment|/**      * Responsible for performing primary operation locally and delegating to replication action once successful      *<p>      * Note that as soon as we move to replication action, state responsibility is transferred to {@link ReplicationPhase}.      */
...
parameter_list|()
block|{
comment|// TODO: validate the cluster state and retry?
name|onReplicaFailure
argument_list|(
name|nodeId
argument_list|,
...
argument_list|)
expr_stmt|;
comment|// TODO: recheck the cluster state and retry indefinitely?
name|onReplicaFailure
argument_list|(
name|nodeId
argument_list|,
...
block|{
comment|// the shard has already been failed, so just signal replica failure
name|onReplicaFailure
argument_list|(
name|nodeId
argument_list|,
...
parameter_list|)
block|{
comment|// verify that the shard we requested to fail is no longer in the cluster state
name|RoutingNode
name|routingNode
init|=
name|clusterState
...
block|{
comment|// the same shard is gone
name|ShardRouting
name|sr
init|=
name|routingNode
