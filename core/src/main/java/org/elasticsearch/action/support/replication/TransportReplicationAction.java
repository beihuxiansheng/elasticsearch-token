...
parameter_list|)
block|{     }
comment|/**      * Primary operation on node with primary copy      *      * @return A tuple containing not null values, as first value the result of the primary operation and as second value      * the request to be executed on the replica shards.      */
...
block|}
block|}
comment|/**      * returns a new reference to {@link IndexShard} to perform a primary operation. Released after performing primary operation locally      * and replication of the operation to all replica shards is completed / failed (see {@link ReplicationOperation}).      */
...
return|;
block|}
comment|/**      * Get a reference to a replica shard. The reference is released as soon as      * replication is completed on the node.      */
DECL|method|getReplicaShardReference
specifier|protected
name|ShardReference
name|getReplicaShardReference
parameter_list|(
