...
argument_list|)
block|,
DECL|enum constant|FIXED_AUTO_QUEUE_SIZE
name|FIXED_AUTO_QUEUE_SIZE
argument_list|(
literal|"fixed_auto_queue_size"
argument_list|)
block|,
...
name|scheduler
decl_stmt|;
DECL|field|cachedTimeThread
specifier|private
specifier|final
name|CachedTimeThread
name|cachedTimeThread
decl_stmt|;
...
expr_stmt|;
block|}
comment|/**      * Returns a value of milliseconds that may be used for relative time calculations.      *      * This method should only be used for calculating time deltas. For an epoch based      * timestamp, see {@link #absoluteTimeInMillis()}.      */
DECL|method|relativeTimeInMillis
specifier|public
name|long
...
return|;
block|}
comment|/**      * Returns the value of milliseconds since UNIX epoch.      *      * This method should only be used for exact date/time formatting. For calculating      * time deltas that should not suffer from negative deltas, which are possible with      * this method, see {@link #relativeTimeInMillis()}.      */
DECL|method|absoluteTimeInMillis
specifier|public
name|long
...
block|}
block|}
comment|/**      * A thread to cache millisecond time values from      * {@link System#nanoTime()} and {@link System#currentTimeMillis()}.      *      * The values are updated at a specified interval.      */
DECL|class|CachedTimeThread
specifier|static
class|class
name|CachedTimeThread
extends|extends
...
literal|true
decl_stmt|;
DECL|field|relativeMillis
specifier|volatile
name|long
name|relativeMillis
decl_stmt|;
DECL|field|absoluteMillis
specifier|volatile
name|long
name|absoluteMillis
decl_stmt|;
DECL|method|CachedTimeThread
name|CachedTimeThread
parameter_list|(
name|String
name|name
...
expr_stmt|;
block|}
comment|/**          * Return the current time used for relative calculations. This is          * {@link System#nanoTime()} truncated to milliseconds.          */
DECL|method|relativeTimeInMillis
name|long
name|relativeTimeInMillis
parameter_list|()
...
return|;
block|}
comment|/**          * Return the current epoch time, used to find absolute time. This is          * a cached version of {@link System#currentTimeMillis()}.          */
DECL|method|absoluteTimeInMillis
name|long
name|absoluteTimeInMillis
parameter_list|()
...
condition|)
block|{
comment|// 5.x doesn't know about the "fixed_auto_queue_size" thread pool type, just write fixed.
name|out
operator|.
name|writeString
argument_list|(
