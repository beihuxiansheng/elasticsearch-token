...
literal|true
decl_stmt|;
DECL|field|idleSinceNanoTime
name|long
name|idleSinceNanoTime
init|=
operator|-
literal|1
decl_stmt|;
comment|// contains the first time we saw this shard with no operations done on it
comment|/** update status based on a new sample. updates all internal variables */
DECL|method|updateWith
specifier|public
name|void
name|updateWith
parameter_list|(
...
condition|)
block|{
comment|// no indexing activity detected.
if|if
condition|(
name|idleSinceNanoTime
operator|<
...
condition|)
block|{
comment|// first time we see this, start the clock.
name|idleSinceNanoTime
operator|=
name|currentNanoTime
expr_stmt|;
...
condition|)
block|{
comment|// shard is inactive. mark it as such.
name|activeIndexing
operator|=
literal|false
expr_stmt|;
...
operator|==
literal|false
comment|// we weren't indexing before
operator|&&
name|idle
operator|==
literal|false
comment|// but we do now
operator|&&
name|current
operator|.
name|translogNumberOfOperations
operator|>
literal|0
comment|// but only if we're really sure - see note bellow
condition|)
block|{
comment|// since we sync flush once a shard becomes inactive, the translog id can change, however that
comment|// doesn't mean the an indexing operation has happened. Note that if we're really unlucky and a flush happens
comment|// immediately after an indexing operation we may not become active immediately. The following
comment|// indexing operation will mark the shard as active, so it's OK. If that one doesn't come, we might as well stay
comment|// inactive
name|activeIndexing
operator|=
literal|true
