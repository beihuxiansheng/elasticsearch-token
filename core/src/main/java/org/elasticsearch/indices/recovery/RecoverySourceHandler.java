...
name|request
decl_stmt|;
DECL|field|currentClusterStateVersionSupplier
specifier|private
specifier|final
name|Supplier
argument_list|<
...
expr_stmt|;
block|}
comment|/**      * Perform phase two of the recovery process.      *<p>      * Phase two uses a snapshot of the current translog *without* acquiring the write lock (however, the translog snapshot is      * point-in-time view of the translog). It then sends each translog operation to the target node so it can be replayed into the new      * shard.      *      * @param startingSeqNo the sequence number to start recovery from, or {@link SequenceNumbersService#UNASSIGNED_SEQ_NO} if all      *                      ops should be sent      * @param snapshot      a snapshot of the translog      *      * @return the local checkpoint on the target      */
DECL|method|phase2
name|long
name|phase2
parameter_list|(
...
condition|)
block|{
comment|// in case of primary relocation we have to ensure that the cluster state on the primary relocation target has all
comment|// replica shards that have recovered or are still recovering from the current primary, otherwise replication actions
comment|// will not be send to these replicas. To accomplish this, first block new recoveries, then take version of latest cluster
comment|// state. This means that no new recovery can be completed based on information of a newer cluster state than the current one.
try|try
init|(
name|Releasable
name|ignored
...
name|apply
argument_list|(
literal|"primary relocation hand-off in progress or completed for "
operator|+
name|shardId
argument_list|)
init|)
...
expr_stmt|;
block|}
DECL|class|SendSnapshotResult
specifier|static
class|class
name|SendSnapshotResult
block|{
DECL|field|targetLocalCheckpoint
specifier|final
name|long
name|targetLocalCheckpoint
decl_stmt|;
...
name|totalOperations
decl_stmt|;
DECL|method|SendSnapshotResult
name|SendSnapshotResult
parameter_list|(
specifier|final
name|long
...
argument_list|()
expr_stmt|;
comment|/*              * If we are doing a sequence-number-based recovery, we have to skip older ops for which no sequence number was assigned, and              * any ops before the starting sequence number.              */
specifier|final
name|long
name|seqNo
init|=
