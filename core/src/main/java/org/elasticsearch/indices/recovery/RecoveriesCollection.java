...
return|;
block|}
DECL|method|startRecoveryInternal
specifier|private
name|void
name|startRecoveryInternal
parameter_list|(
...
expr_stmt|;
block|}
comment|/**      * Resets the recovery and performs a recovery restart on the currently recovering index shard      *      * @see IndexShard#performRecoveryRestart()      * @return newly created RecoveryTarget      */
...
init|)
block|{
comment|// swap recovery targets in a synchronized block to ensure that the newly added recovery target is picked up by
comment|// cancelRecoveriesForShard whenever the old recovery target is picked up
name|oldRecoveryTarget
operator|=
name|onGoingRecoveries
operator|.
...
expr_stmt|;
block|}
comment|// Closes the current recovery target
specifier|final
name|AtomicBoolean
...
parameter_list|)
block|{
comment|// new recovery target is already cancelled (probably due to shard closing or recovery source changing)
assert|assert
name|onGoingRecoveries
operator|.
name|containsKey
...
name|trace
argument_list|(
literal|"{} recovery reset cancelled, recovery from {}, id [{}], previous id [{}]"
argument_list|,
name|newRecoveryTarget
operator|.
name|shardId
...
name|cancel
argument_list|(
literal|"recovery reset cancelled"
argument_list|)
expr_stmt|;
comment|// if finalOldRecoveryTarget.resetRecovery did not even get to execute
return|return
literal|null
return|;
block|}
...
argument_list|()
argument_list|,
literal|"failed to reset recovery"
argument_list|)
expr_stmt|;
return|return
literal|null
...
name|trace
argument_list|(
literal|"{} restarted recovery from {}, id [{}], previous id [{}]"
argument_list|,
name|newRecoveryTarget
operator|.
name|shardId
...
argument_list|()
argument_list|,
literal|"failed to retry recovery"
argument_list|,
name|e
argument_list|)
argument_list|,
...
block|}
block|}
DECL|method|getRecoveryTarget
specifier|public
name|RecoveryTarget
name|getRecoveryTarget
parameter_list|(
name|long
name|id
parameter_list|)
