...
argument_list|()
decl_stmt|;
comment|// latch that can be used to blockingly wait for RecoveryTarget to be closed
DECL|field|closedLatch
specifier|private
specifier|final
name|CountDownLatch
name|closedLatch
...
name|debug
argument_list|(
literal|"reset of recovery with shard {} and id [{}]"
argument_list|,
name|shardId
argument_list|,
name|recoveryId
...
name|trace
argument_list|(
literal|"new recovery target cancelled for shard {} while waiting on old recovery target with id [{}] to close"
argument_list|,
name|shardId
argument_list|,
name|recoveryId
...
condition|)
block|{
comment|// once primary relocation has moved past the finalization step, the relocation source can be moved to RELOCATED state
comment|// and start indexing as primary into the target shard (see TransportReplicationAction). Resetting the target shard in this
comment|// state could mean that indexing is halted until the recovery retry attempt is completed and could also destroy existing
comment|// documents indexed and acknowledged before the reset.
assert|assert
name|stage
operator|!=
name|RecoveryState
...
name|DONE
operator|:
literal|"recovery should not have completed when it's being reset"
assert|;
throw|throw
operator|new
name|IllegalStateException
argument_list|(
literal|"cannot reset recovery as previous attempt made it past finalization step"
argument_list|)
throw|;
block|}
name|indexShard
...
throw|;
block|}
comment|// first convert all translog operations to engine operations to check for mapping updates
name|List
argument_list|<
name|Engine
operator|.
...
argument_list|)
decl_stmt|;
comment|// actually apply engine operations
for|for
control|(
name|Engine
operator|.
