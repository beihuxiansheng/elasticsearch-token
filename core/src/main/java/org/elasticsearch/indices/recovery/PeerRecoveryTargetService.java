...
block|}
block|}
comment|/**      * Cancel all ongoing recoveries for the given shard.      *      * @param reason  reason for cancellation      * @param shardId shard ID for which to cancel recoveries      * @return {@code true} if a recovery was cancelled      */
...
name|trace
argument_list|(
literal|"not running recovery with id [{}] - can not find it (probably finished)"
argument_list|,
name|recoveryId
argument_list|)
expr_stmt|;
...
literal|null
operator|:
literal|"can not do a recovery without a source node"
assert|;
name|request
operator|=
name|getStartRecoveryRequest
argument_list|(
name|recoveryTarget
argument_list|)
expr_stmt|;
...
name|trace
argument_list|(
literal|"{} preparing shard for peer recovery"
argument_list|,
name|recoveryTarget
operator|.
name|shardId
...
name|trace
argument_list|(
literal|"unexpected error while preparing shard for peer recovery, failing recovery"
argument_list|,
name|e
argument_list|)
expr_stmt|;
...
argument_list|()
argument_list|,
literal|"failed to prepare shard for recovery"
argument_list|,
name|e
argument_list|)
argument_list|,
...
begin_function
DECL|method|getStoreMetadataSnapshot
unit|private
name|Store
operator|.
name|MetadataSnapshot
name|getStoreMetadataSnapshot
parameter_list|(
specifier|final
name|RecoveryTarget
name|recoveryTarget
...
name|trace
argument_list|(
literal|"{} shard folder empty, recovering all files"
argument_list|,
name|recoveryTarget
argument_list|)
expr_stmt|;
...
name|warn
argument_list|(
literal|"error while listing local files, recovering as if there are none"
argument_list|,
name|e
argument_list|)
expr_stmt|;
...
begin_comment
comment|/**      * Prepare the start recovery request.      *      * @param recoveryTarget the target of the recovery      * @return a start recovery request      */
end_comment

begin_function
DECL|method|getStartRecoveryRequest
specifier|private
name|StartRecoveryRequest
name|getStartRecoveryRequest
parameter_list|(
...
name|trace
argument_list|(
literal|"{} preparing for file-based recovery from [{}]"
argument_list|,
name|recoveryTarget
operator|.
name|shardId
...
name|trace
argument_list|(
literal|"{} preparing for sequence-number-based recovery starting at local checkpoint [{}] from [{}]"
argument_list|,
name|recoveryTarget
operator|.
name|shardId
...
begin_comment
comment|/**      * Get the starting sequence number for a sequence-number-based request.      *      * @param recoveryTarget the target of the recovery      * @return the starting sequence number or {@link SequenceNumbersService#UNASSIGNED_SEQ_NO} if obtaining the starting sequence number      * failed      */
end_comment

begin_function
DECL|method|getStartingSeqNo
specifier|public
specifier|static
name|long
name|getStartingSeqNo
