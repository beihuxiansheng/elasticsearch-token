...
return|;
block|}
comment|/**      * creates a new mapper service for the given index, in order to do administrative work like mapping updates.      * This *should not* be used for document parsing. Doing so will result in an exception.      *      * Note: the returned {@link MapperService} should be closed when unneeded.      */
DECL|method|createIndexMapperService
specifier|public
specifier|synchronized
name|MapperService
...
name|AssertionError
argument_list|(
literal|"Could not serialize response"
argument_list|,
name|e
argument_list|)
throw|;
...
name|IllegalStateException
argument_list|(
literal|"Failed to write field stats output"
argument_list|,
name|e
argument_list|)
throw|;
...
parameter_list|()
block|{
comment|// No need to take the IndexShard into account since it is shared
comment|// across many entities
return|return
name|BASE_RAM_BYTES_USED
return|;
block|}
