...
argument_list|()
argument_list|,
literal|"Could not get the indices in the repository."
argument_list|,
name|e
argument_list|)
throw|;
...
throw|;
block|}
comment|// update the index file if created pre 5.0 to include the indices in the repository
...
condition|)
block|{
comment|// nothing to update on read only repositories
return|return;
block|}
if|if
condition|(
...
argument_list|()
argument_list|,
literal|"failed to update the repository index blob with indices data on startup"
argument_list|,
name|e
argument_list|)
throw|;
...
block|{
comment|// we don't know the version, first try the current format, then the legacy format
try|try
block|{
name|snapshotFormat
operator|.
...
parameter_list|)
block|{
comment|// neither snapshot file could be deleted, log the error
name|logger
operator|.
name|warn
argument_list|(
...
parameter_list|)
block|{
comment|// neither global metadata file could be deleted, log the error
name|logger
operator|.
name|warn
argument_list|(
...
expr_stmt|;
block|}
comment|// delete the old index file (non-generational) if it exists
if|if
condition|(
name|snapshotsBlobContainer
operator|.
