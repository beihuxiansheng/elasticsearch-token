...
try|try
block|{
comment|// This is the same logic in QueryShardContext#toQuery() where we reset also twice.
comment|// Personally I think a reset at the end is sufficient, but I kept the logic consistent with this method.
comment|// The reason we need to invoke reset at all here is because inner hits may modify the QueryShardContext#nestedScope,
comment|// so we need to reset at the end.
name|queryShardContext
operator|.
name|reset
argument_list|()
...
name|context
argument_list|,
literal|"failed to create InnerHitsContext"
argument_list|,
name|e
argument_list|)
throw|;
