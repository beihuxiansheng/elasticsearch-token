...
return|;
block|}
comment|/**      * This parser outputs a temporary map of the objects needed to create the      * SearchHit instead of directly creating the SearchHit. The reason for this      * is that this way we can reuse the parser when parsing xContent from      * {@link CompletionSuggestion.Entry.Option} which unfortunately inlines the      * output of      * {@link #toInnerXContent(XContentBuilder, org.elasticsearch.common.xcontent.ToXContent.Params)}      * of the included search hit. The output of the map is used to create the      * actual SearchHit instance via {@link #createFromMap(Map)}      */
DECL|field|MAP_PARSER
specifier|private
specifier|static
name|ObjectParser
argument_list|<
...
return|;
block|}
DECL|method|declareInnerHitsParseFields
specifier|public
specifier|static
name|void
name|declareInnerHitsParseFields
...
init|)
block|{
comment|// the original document gets slightly modified: whitespaces or
comment|// pretty printing are not preserved,
...
block|}
block|}
comment|/**      * we need to declare parse fields for each metadata field, except for _ID, _INDEX and _TYPE which are      * handled individually. All other fields are parsed to an entry in the fields map      */
DECL|method|declareMetaDataFields
specifier|private
specifier|static
name|void
name|declareMetaDataFields
...
return|;
block|}
DECL|method|parseHighlightFields
specifier|private
specifier|static
name|Map
argument_list|<
