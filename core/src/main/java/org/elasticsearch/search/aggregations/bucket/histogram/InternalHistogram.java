...
comment|// they are already stored and tested on the InternalHistogram object
return|return
name|key
operator|==
name|that
...
condition|)
block|{
comment|// fill with empty buckets
for|for
control|(
name|double
name|key
...
condition|)
block|{
comment|// fill with empty buckets until the first key
for|for
control|(
name|double
name|key
...
block|{
comment|// sorted by compound order or sub-aggregation, need to fall back to a costly n*log(n) sort
name|CollectionUtil
operator|.
name|introSort
argument_list|(
