...
argument_list|)
decl_stmt|;
comment|// validation was not available prior to 2.x, so to support bwc percolation queries we only ignore_malformed
comment|// on 2.x created indexes
name|GeoPoint
index|[]
name|localPoints
init|=
...
condition|)
block|{
comment|// we might get here if the geo point is " number, number] " and the parser already moved over the
comment|// opening bracket in this case we cannot use GeoUtils.parseGeoPoint(..) because this expects an opening
comment|// bracket
name|double
name|lon
init|=
