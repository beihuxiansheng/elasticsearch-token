...
try|try
block|{
comment|// This check is here because the DEFAULT_TEMPLATE_LANG(mustache) is not
comment|// installed for use by REST tests. `propertyValue` will not be
comment|// modified if templating is not available so a script that simply returns an unmodified `propertyValue`
comment|// is returned.
if|if
condition|(
name|scriptService
...
argument_list|)
block|{
block|@Override                     public String execute(
block|)
block|{
return|return
name|propertyValue
