...
return|;
block|}
DECL|method|getPluginSettingsFilter
specifier|public
name|List
argument_list|<
name|String
...
return|;
block|}
comment|/** Returns all classes injected into guice by plugins which extend {@link LifecycleComponent}. */
...
return|;
block|}
DECL|method|checkForFailedPluginRemovals
specifier|static
name|void
...
name|IOException
block|{
comment|/*          * Check for the existence of a marker file that indicates any plugins are in a garbage state from a failed attempt to remove the          * plugin.          */
try|try
init|(
name|DirectoryStream
argument_list|<
...
name|ROOT
argument_list|,
literal|"found file [%s] from a failed attempt to remove the plugin [%s]; execute [elasticsearch-plugin remove %2$s]"
argument_list|,
name|removing
argument_list|,
name|name
...
argument_list|()
decl_stmt|;
comment|// check we don't have conflicting codebases
name|Set
argument_list|<
name|URL
argument_list|>
...
name|IllegalStateException
argument_list|(
literal|"jar hell! duplicate codebases between"
operator|+
literal|" plugin and core: "
operator|+
name|intersection
argument_list|)
throw|;
block|}
comment|// check we don't have conflicting classes
name|Set
argument_list|<
name|URL
argument_list|>
...
throw|;
block|}
comment|// create a child to load the plugin in this bundle
name|ClassLoader
name|loader
init|=
name|URLClassLoader
