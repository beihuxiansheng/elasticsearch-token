...
expr_stmt|;
block|}
comment|/**      * Collects all nodes on the connected cluster and returns / passes a nodeID to {@link DiscoveryNode} lookup function      * that returns<code>null</code> if the node ID is not found.      */
...
argument_list|)
expr_stmt|;
comment|// run this on the node that gets the request it's as good as any other
specifier|final
name|DiscoveryNode
name|node
init|=
...
comment|// this will cause some back pressure on the search end and eventually will cause rejections but that's fine
comment|// we can't proceed with a search on a cluster level.
comment|// in the future we might want to just skip the remote nodes in such a case but that can already be implemented on the
comment|// caller end since they provide the listener.
name|ensureConnected
argument_list|(
name|ActionListener
operator|.
