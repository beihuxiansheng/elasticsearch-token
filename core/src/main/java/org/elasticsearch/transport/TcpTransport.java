...
comment|// #onNodeChannelsClosed will remove it..
assert|assert
name|openConnections
operator|.
name|contains
...
literal|false
operator|:
literal|"NodeChannel#close should remove the connetion"
assert|;
comment|// we can only be connected and published to a single node with one connection. So if disconnectFromNode
comment|// returns true we can safely break out from here since we cleaned up everything needed
break|break
name|outer
break|;
block|}
block|}
comment|// now if we haven't found the right connection in the connected nodes we have to go through the open connections
comment|// it might be that the channel belongs to a connection that is not published
for|for
control|(
name|NodeChannels
name|channels
...
name|nodeChannels
argument_list|,
literal|"due to explicit disconnect call"
argument_list|)
expr_stmt|;
block|}
block|}
...
block|}
block|}
DECL|method|onNodeChannelsClosed
specifier|private
name|void
name|onNodeChannelsClosed
parameter_list|(
...
parameter_list|)
block|{
comment|// don't assert here since the channel / connection might not have been registered yet
specifier|final
name|boolean
name|remove
init|=
...
return|;
block|}
DECL|method|getNumConnectedNodes
specifier|final
name|int
