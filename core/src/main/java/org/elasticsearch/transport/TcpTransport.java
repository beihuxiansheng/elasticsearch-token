...
argument_list|)
decl_stmt|;
DECL|field|handshakeNotSupported
specifier|final
name|AtomicBoolean
name|handshakeNotSupported
init|=
operator|new
name|AtomicBoolean
argument_list|(
...
comment|// this will happen if we talk to a node (pre 5.2) that doesn't have a handshake handler
comment|// we will just treat the node as a 5.0.0 node unless the discovery node that is used to connect has a higher version.
operator|&&
name|cause
operator|.
name|getMessage
...
name|equals
argument_list|(
literal|"No handler for action [internal:tcp/handshake]"
argument_list|)
condition|)
block|{
name|handshakeNotSupported
