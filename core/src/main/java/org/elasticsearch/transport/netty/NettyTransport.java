...
argument_list|()
decl_stmt|;
comment|// TODO: We can remove the special casing for the default profile and store it in the profile map to reduce the complexity here
if|if
condition|(
operator|!
name|DEFAULT_PROFILE
...
condition|)
block|{
comment|// check to see if an address is already bound for this profile
name|BoundTransportAddress
name|boundTransportAddress
init|=
name|profileBoundAddresses
...
condition|)
block|{
comment|// no address is bound, so lets create one with the publish address information from the settings or the bound address as a fallback
name|int
name|publishPort
init|=
name|profileSettings
...
block|{
comment|// TODO: support real multihoming with publishing. Today we update the bound addresses so only the prioritized address is published
comment|// an address already exists. add the new bound address to the end of a new array and create a new BoundTransportAddress with the array and existing publish address
comment|// the new bound address is appended in order to preserve the ordering/priority of bound addresses
name|TransportAddress
index|[]
name|existingBoundAddress
init|=
name|boundTransportAddress
operator|.
name|boundAddresses
...
condition|)
block|{
comment|// this is the first address that has been bound for the default profile so we get the publish address information and create a new BoundTransportAddress
comment|// these calls are different from the profile ones due to the way the settings for a profile are created. If we want to merge the code for the default profile and
comment|// other profiles together, we need to change how the profileSettings are built for the default profile...
name|int
name|publishPort
init|=
name|settings
...
block|{
comment|// the default profile is already bound to one address and has the publish address, copy the existing bound addresses as is and append the new address.
comment|// the new bound address is appended in order to preserve the ordering/priority of bound addresses
name|TransportAddress
index|[]
name|existingBoundAddress
