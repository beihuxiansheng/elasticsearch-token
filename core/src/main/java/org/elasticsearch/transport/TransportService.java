...
decl_stmt|;
DECL|field|connectToRemoteCluster
specifier|private
specifier|final
name|boolean
...
literal|null
decl_stmt|;
DECL|field|localNodeConnection
specifier|private
specifier|final
name|Transport
operator|.
...
expr_stmt|;
block|}
comment|/**      * Connect to the specified node with the given connection profile      *      * @param node the node to connect to      * @param connectionProfile the connection profile to use when connecting to this node      */
DECL|method|connectToNode
specifier|public
name|void
name|connectToNode
...
lambda|->
block|{
comment|// We don't validate cluster names to allow for tribe node connections.
specifier|final
name|DiscoveryNode
name|remote
init|=
...
name|node
argument_list|,
literal|"handshake failed. unexpected remote node "
operator|+
name|remote
argument_list|)
throw|;
...
parameter_list|)
block|{
comment|// the caller might not handle this so we invoke the handler
name|futureHandler
operator|.
name|handleException
argument_list|(
...
parameter_list|)
block|{
comment|// The parent task is already cancelled - just fail the request
name|handler
operator|.
name|handleException
argument_list|(
...
name|IllegalStateException
argument_list|(
literal|"can't send request to a null connection"
argument_list|)
throw|;
block|}
name|DiscoveryNode
...
argument_list|)
expr_stmt|;
comment|// local node optimization happens upstream
block|}
catch|catch
parameter_list|(
specifier|final
...
name|debug
argument_list|(
literal|"Rejected execution on onConnectionClosed"
argument_list|,
name|ex
argument_list|)
expr_stmt|;
...
name|discoveryNode
argument_list|,
literal|"discovery node must not be null"
argument_list|)
operator|.
name|equals
argument_list|(
