...
argument_list|)
decl_stmt|;
DECL|field|MAX_PENDING_CLUSTER_STATES_SETTING
specifier|public
specifier|static
specifier|final
name|Setting
...
name|committedState
decl_stmt|;
comment|// last committed cluster state
DECL|field|stateMutex
specifier|private
specifier|final
name|Object
name|stateMutex
...
init|)
block|{
comment|// do the join on a different thread, the caller of this method waits for 30s anyhow till it is discovered
name|joinThreadControl
operator|.
...
literal|null
operator|:
literal|"accessing cluster state before it is set"
assert|;
return|return
name|clusterState
return|;
...
argument_list|()
assert|;
comment|// state got changed locally (maybe because another master published to us)
if|if
condition|(
name|clusterChangedEvent
operator|.
...
argument_list|(
literal|"state was mutated while calculating new CS update"
argument_list|)
throw|;
block|}
name|pendingStatesQueue
...
name|ElasticsearchException
argument_list|(
literal|"failed to publish cluster state"
argument_list|)
argument_list|)
expr_stmt|;
name|rejoin
...
name|FailedToCommitClusterStateException
argument_list|(
literal|"local state was mutated while CS update was published to other nodes"
argument_list|)
throw|;
block|}
name|boolean
...
block|}
block|}
comment|// indefinitely wait for cluster state to be applied locally
try|try
block|{
name|latch
operator|.
...
name|ParameterizedMessage
argument_list|(
literal|"interrupted while applying cluster state locally [{}]"
argument_list|,
name|clusterChangedEvent
operator|.
name|source
...
block|}
block|}
comment|// return true if state has been sent to applier
DECL|method|processNextCommittedClusterState
name|boolean
name|processNextCommittedClusterState
parameter_list|(
name|String
...
argument_list|)
expr_stmt|;
comment|// update failure detection only after the state has been updated to prevent race condition with handleLeaveRequest
comment|// and handleNodeFailure as those check the current state to determine whether the failure is to be handled by this node
if|if
condition|(
name|adaptedNewClusterState
operator|.
...
condition|)
block|{
comment|// update the set of nodes to ping
name|nodesFD
operator|.
...
argument_list|(
literal|"apply cluster state (from master ["
operator|+
name|reason
operator|+
literal|"])"
...
name|ParameterizedMessage
argument_list|(
literal|"unexpected failure applying [{}]"
argument_list|,
name|reason
argument_list|)
argument_list|,
...
try|try
block|{
comment|// TODO: use cluster state uuid instead of full cluster state so that we don't keep reference to CS around
...
literal|false
operator|:
literal|"NO_MASTER_BLOCK should only be added by ZenDiscovery"
assert|;
name|ClusterBlocks
name|clusterBlocks
init|=
...
argument_list|)
expr_stmt|;
comment|// don't wait for state to be applied
block|}
block|}
...
block|{
comment|// TODO: do this outside mutex
name|logger
operator|.
name|warn
argument_list|(
