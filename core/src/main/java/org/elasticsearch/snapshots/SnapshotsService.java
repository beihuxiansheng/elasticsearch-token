...
block|}
block|}
comment|/**      * Gets the {@link RepositoryData} for the given repository.      *      * @param repositoryName repository name      * @return repository data      */
...
expr_stmt|;
block|}
name|removeFinishedSnapshotFromClusterState
argument_list|(
name|event
argument_list|)
expr_stmt|;
name|finalizeSnapshotDeletionFromPreviousMaster
argument_list|(
name|event
argument_list|)
expr_stmt|;
...
block|}
block|}
comment|/**      * Finalizes a snapshot deletion in progress if the current node is the master but it      * was not master in the previous cluster state and there is still a lingering snapshot      * deletion in progress in the cluster state.  This means that the old master failed      * before it could clean up an in-progress snapshot deletion.  We attempt to delete the      * snapshot files and remove the deletion from the cluster state.  It is possible that the      * old master was in a state of long GC and then it resumes and tries to delete the snapshot      * that has already been deleted by the current master.  This is acceptable however, since      * the old master's snapshot deletion will just respond with an error but in actuality, the      * snapshot was deleted and a call to GET snapshots would reveal that the snapshot no longer exists.      */
DECL|method|finalizeSnapshotDeletionFromPreviousMaster
specifier|private
name|void
name|finalizeSnapshotDeletionFromPreviousMaster
parameter_list|(
...
literal|1
operator|:
literal|"only one in-progress deletion allowed per cluster"
assert|;
name|SnapshotDeletionsInProgress
operator|.
name|Entry
...
block|}
block|}
comment|/**      * Removes a finished snapshot from the cluster state.  This can happen if the previous      * master node processed a cluster state update that marked the snapshot as finished,      * but the previous master node died before removing the snapshot in progress from the      * cluster state.  It is then the responsibility of the new master node to end the      * snapshot and remove it from the cluster state.      */
DECL|method|removeFinishedSnapshotFromClusterState
specifier|private
name|void
name|removeFinishedSnapshotFromClusterState
parameter_list|(
...
name|snapshotName
argument_list|,
literal|"cannot delete incompatible snapshot"
argument_list|)
throw|;
block|}
name|Optional
...
expr_stmt|;
block|}
comment|/**      * Deletes snapshot from repository.      *<p>      * If the snapshot is still running cancels the snapshot first and then deletes it from the repository.      *      * @param snapshot snapshot      * @param listener listener      * @param repositoryStateId the unique id for the state of the repository      */
DECL|method|deleteSnapshot
specifier|private
name|void
name|deleteSnapshot
...
condition|)
block|{
comment|// don't allow snapshot deletions while a restore is taking place,
comment|// otherwise we could end up deleting a snapshot that is being restored
comment|// and the files the restore depends on would all be gone
if|if
condition|(
name|restoreInProgress
operator|.
...
condition|)
block|{
comment|// This snapshot is not running - delete
if|if
condition|(
name|snapshots
operator|!=
...
throw|;
block|}
comment|// add the snapshot deletion to the cluster state
name|SnapshotDeletionsInProgress
operator|.
name|Entry
name|entry
...
return|;
block|}
comment|/**      * Deletes snapshot from repository      *      * @param snapshot   snapshot      * @param listener   listener      * @param repositoryStateId the unique id representing the state of the repository at the time the deletion began      */
...
argument_list|)
expr_stmt|;
name|removeSnapshotDeletionFromClusterState
argument_list|(
name|snapshot
argument_list|,
literal|null
...
expr_stmt|;
block|}
comment|/**      * Removes the snapshot deletion from {@link SnapshotDeletionsInProgress} in the cluster state.      */
DECL|method|removeSnapshotDeletionFromClusterState
specifier|private
name|void
name|removeSnapshotDeletionFromClusterState
parameter_list|(
...
name|submitStateUpdateTask
argument_list|(
literal|"remove snapshot deletion metadata"
argument_list|,
operator|new
name|ClusterStateUpdateTask
argument_list|()
...
literal|1
operator|:
literal|"should have exactly one deletion in progress"
assert|;
name|SnapshotDeletionsInProgress
operator|.
name|Entry
...
name|ParameterizedMessage
argument_list|(
literal|"[{}] failed to remove snapshot deletion metadata"
argument_list|,
name|snapshot
argument_list|)
argument_list|,
