...
argument_list|()
decl_stmt|;
DECL|field|snapshotStateExecutor
specifier|private
specifier|final
name|SnapshotStateExecutor
name|snapshotStateExecutor
init|=
operator|new
name|SnapshotStateExecutor
...
parameter_list|)
block|{
comment|// abort any snapshots occurring on the soon-to-be closed shard
name|Map
argument_list|<
name|Snapshot
argument_list|,
...
name|debug
argument_list|(
literal|"[{}] shard closing, abort snapshotting for snapshot [{}]"
argument_list|,
name|shardId
argument_list|,
name|snapshotShards
...
block|{
comment|// abort any running snapshots of shards for the removed entry;
comment|// this could happen if for some reason the cluster state update for aborting
comment|// running shards is missed, then the snapshot is removed is a subsequent cluster
comment|// state update, which is being processed here
for|for
control|(
name|IndexShardSnapshotStatus
name|snapshotStatus
...
expr_stmt|;
block|}
DECL|class|SnapshotStateExecutor
class|class
name|SnapshotStateExecutor
implements|implements
name|ClusterStateTaskExecutor
