...
block|}
block|}
comment|/**      * Bootstrap check that if system call filters are enabled, then system call filters must have installed successfully.      */
DECL|class|SystemCallFilterCheck
specifier|static
class|class
name|SystemCallFilterCheck
implements|implements
...
name|Check
block|{
DECL|field|areSystemCallFiltersEnabled
specifier|private
specifier|final
name|boolean
name|areSystemCallFiltersEnabled
decl_stmt|;
DECL|method|SystemCallFilterCheck
name|SystemCallFilterCheck
parameter_list|(
specifier|final
name|boolean
...
block|}
block|}
comment|/**      * Bootstrap check for versions of HotSpot that are known to have issues that can lead to index corruption when G1GC is enabled.      */
...
name|ROOT
argument_list|,
literal|"JVM version [%s] can cause data corruption when used with G1GC; upgrade to at least Java 8u40"
argument_list|,
name|jvmVersion
argument_list|()
argument_list|)
