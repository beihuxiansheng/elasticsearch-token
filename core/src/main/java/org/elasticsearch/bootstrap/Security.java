...
control|)
block|{
comment|// write permissions are not needed here, we are not going to be writing to any paths here
name|addPath
argument_list|(
name|policy
argument_list|,
...
argument_list|)
expr_stmt|;
name|addSocketPermissionForTransportProfiles
argument_list|(
name|policy
argument_list|,
name|settings
argument_list|)
expr_stmt|;
name|addSocketPermissionForTribeNodes
argument_list|(
name|policy
argument_list|,
name|settings
...
DECL|method|addSocketPermissionForTransportProfiles
specifier|private
specifier|static
name|void
name|addSocketPermissionForTransportProfiles
...
parameter_list|)
block|{
comment|// transport is way over-engineered
specifier|final
name|Map
argument_list|<
name|String
...
argument_list|)
expr_stmt|;
comment|// loop through all profiles and add permissions for each one, if it's valid; otherwise Netty transports are lenient and ignores it
for|for
control|(
specifier|final
name|Map
...
argument_list|()
decl_stmt|;
comment|// a profile is only valid if it's the default profile, or if it has an actual name and specifies a port
comment|// TODO: can this leniency be removed?
specifier|final
name|boolean
name|valid
init|=
...
expr_stmt|;
block|}
DECL|method|addSocketPermissionForTribeNodes
specifier|private
specifier|static
name|void
name|addSocketPermissionForTribeNodes
