...
literal|0
decl_stmt|;
comment|/**    * Calls {@code #XAnalyzingSuggester(Analyzer,Analyzer,int,int,int,boolean,FST,boolean,int,int,int,int,int)    * AnalyzingSuggester(analyzer, analyzer, EXACT_FIRST |    * PRESERVE_SEP, 256, -1)}    *    * @param analyzer Analyzer that will be used for analyzing suggestions while building the index.    */
...
expr_stmt|;
block|}
comment|/**    * Calls {@code #XAnalyzingSuggester(Analyzer,Analyzer,int,int,int,boolean,FST,boolean,int,int,int,int,int)    * AnalyzingSuggester(indexAnalyzer, queryAnalyzer, EXACT_FIRST |    * PRESERVE_SEP, 256, -1)}    *    * @param indexAnalyzer Analyzer that will be used for analyzing suggestions while building the index.    * @param queryAnalyzer Analyzer that will be used for analyzing query text during lookup    */
DECL|method|XAnalyzingSuggester
specifier|public
name|XAnalyzingSuggester
parameter_list|(
...
name|reason
operator|=
literal|"access temp directory for building index"
argument_list|)
...
return|;
block|}
comment|// EDIT: Adrien, needed by lookup providers
comment|// NOTE: these XForks are unmaintainable, we need to get rid of them...
...
return|;
block|}
comment|/**    * Returns the weight associated with an input string, or null if it does not exist.    *    * Unsupported in this implementation (and will throw an {@link UnsupportedOperationException}).    *    * @param key input string    * @return the weight associated with the input string, or {@code null} if it does not exist.    */
DECL|method|get
specifier|public
name|Object
name|get
...
throw|;
block|}
comment|/**    * cost -&gt; weight    *    * @param encoded Cost    * @return Weight    */
...
return|;
block|}
comment|/**    * weight -&gt; cost    *    * @param value Weight    * @return Cost    */
