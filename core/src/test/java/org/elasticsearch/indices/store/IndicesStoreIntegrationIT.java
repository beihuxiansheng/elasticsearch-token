...
DECL|method|relocateAndBlockCompletion
specifier|public
specifier|static
name|BlockClusterStateProcessing
name|relocateAndBlockCompletion
...
argument_list|)
decl_stmt|;
comment|// use a tracer on the target node to track relocation start and end
name|transportService
operator|.
name|addTracer
argument_list|(
...
name|info
argument_list|(
literal|"received: {}, relocation starts"
argument_list|,
name|action
argument_list|)
expr_stmt|;
...
condition|)
block|{
comment|// Whenever a node deletes a shard because it was relocated somewhere else, it first
comment|// checks if enough other copies are started somewhere else. The node sends a ShardActiveRequest
comment|// to the other nodes that should have a copy according to cluster state.
name|receivedShardExistsRequestLatch
operator|.
name|countDown
argument_list|()
...
name|info
argument_list|(
literal|"received: {}, waiting on cluster state"
argument_list|,
name|action
argument_list|)
expr_stmt|;
comment|// ensure that relocation target node is on the same cluster state as relocation source before proceeding with
comment|// this request. If the target does not have the relocating cluster state exposed through ClusterService.state(),
comment|// then waitForClusterState will have to register a ClusterObserver with the ClusterService, which can cause
comment|// a race with the BlockClusterStateProcessing block that is added below.
try|try
block|{
name|assertBusy
argument_list|(
...
name|info
argument_list|(
literal|"--> relocation completed (but cluster state processing block still in place)"
argument_list|)
expr_stmt|;
return|return
name|disruption
...
name|fail
argument_list|(
literal|"Excepted proper response "
operator|+
name|ExceptionsHelper
operator|.
