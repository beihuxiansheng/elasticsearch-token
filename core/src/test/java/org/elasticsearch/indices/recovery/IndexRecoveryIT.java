...
name|String
argument_list|>
name|assertNodeHasThrottleTimeAndNoRecoveries
init|=
name|nodeName
lambda|->
block|{
...
name|nodeName
operator|+
literal|" throttling should be>0"
argument_list|,
name|recoveryStats
operator|.
name|throttleTime
...
block|}
decl_stmt|;
comment|// we have to use assertBusy as recovery counters are decremented only when the last reference to the RecoveryTarget
comment|// is decremented, which may happen after the recovery was done.
name|assertBusy
argument_list|(
parameter_list|()
lambda|->
...
block|}
block|}
comment|/**      * Tests scenario where recovery target successfully sends recovery request to source but then the channel gets closed while      * the source is working on the recovery process.      */
annotation|@
name|TestLogging
argument_list|(
literal|"_root:DEBUG,org.elasticsearch.indices.recovery:TRACE"
argument_list|)
DECL|method|testDisconnectsDuringRecovery
specifier|public
name|void
name|testDisconnectsDuringRecovery
parameter_list|()
throws|throws
name|Exception
block|{
...
condition|)
block|{
comment|// ensures that it's considered as valid recovery attempt by source
try|try
block|{
name|awaitBusy
argument_list|(
...
argument_list|()
argument_list|,
literal|"DISCONNECT: simulation disconnect after successfully sending "
operator|+
name|action
operator|+
literal|" request"
...
name|info
argument_list|(
literal|"--> starting primary relocation recovery from blue to red"
argument_list|)
expr_stmt|;
name|client
argument_list|()
...
argument_list|()
expr_stmt|;
comment|// also waits for relocation / recovery to complete
comment|// if a primary relocation fails after the source shard has been marked as relocated, both source and target are failed. If the
comment|// source shard is moved back to started because the target fails first, it's possible that there is a cluster state where the
comment|// shard is marked as started again (and ensureGreen returns), but while applying the cluster state the primary is failed and
comment|// will be reallocated. The cluster will thus become green, then red, then green again. Triggering a refresh here before
comment|// searching helps, as in contrast to search actions, refresh waits for the closed shard to be reallocated.
name|client
argument_list|()
operator|.
name|admin
...
name|info
argument_list|(
literal|"--> starting replica recovery from blue to red"
argument_list|)
expr_stmt|;
name|client
argument_list|()
