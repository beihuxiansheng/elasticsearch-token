begin_unit|revision:0.9.5;language:Java;cregit-version:0.0.1
begin_comment
comment|/*  * Licensed to Elasticsearch under one or more contributor  * license agreements. See the NOTICE file distributed with  * this work for additional information regarding copyright  * ownership. Elasticsearch licenses this file to you under  * the Apache License, Version 2.0 (the "License"); you may  * not use this file except in compliance with the License.  * You may obtain a copy of the License at  *  *    http://www.apache.org/licenses/LICENSE-2.0  *  * Unless required by applicable law or agreed to in writing,  * software distributed under the License is distributed on an  * "AS IS" BASIS, WITHOUT WARRANTIES OR CONDITIONS OF ANY  * KIND, either express or implied.  See the License for the  * specific language governing permissions and limitations  * under the License.  */
end_comment

begin_package
DECL|package|org.elasticsearch.rest.util
package|package
name|org
operator|.
name|elasticsearch
operator|.
name|rest
operator|.
name|util
package|;
end_package

begin_import
import|import
name|org
operator|.
name|elasticsearch
operator|.
name|rest
operator|.
name|support
operator|.
name|RestUtils
import|;
end_import

begin_import
import|import
name|org
operator|.
name|elasticsearch
operator|.
name|test
operator|.
name|ESTestCase
import|;
end_import

begin_import
import|import
name|java
operator|.
name|util
operator|.
name|HashMap
import|;
end_import

begin_import
import|import
name|java
operator|.
name|util
operator|.
name|Locale
import|;
end_import

begin_import
import|import
name|java
operator|.
name|util
operator|.
name|Map
import|;
end_import

begin_import
import|import
name|java
operator|.
name|util
operator|.
name|regex
operator|.
name|Pattern
import|;
end_import

begin_import
import|import static
name|org
operator|.
name|hamcrest
operator|.
name|Matchers
operator|.
name|equalTo
import|;
end_import

begin_import
import|import static
name|org
operator|.
name|hamcrest
operator|.
name|Matchers
operator|.
name|is
import|;
end_import

begin_import
import|import static
name|org
operator|.
name|hamcrest
operator|.
name|Matchers
operator|.
name|nullValue
import|;
end_import

begin_comment
comment|/**  *  */
end_comment

begin_class
DECL|class|RestUtilsTests
specifier|public
class|class
name|RestUtilsTests
extends|extends
name|ESTestCase
block|{
DECL|method|randomDelimiter
specifier|static
name|char
name|randomDelimiter
parameter_list|()
block|{
return|return
name|randomBoolean
argument_list|()
condition|?
literal|'&'
else|:
literal|';'
return|;
block|}
DECL|method|testDecodeQueryString
specifier|public
name|void
name|testDecodeQueryString
parameter_list|()
block|{
name|Map
argument_list|<
name|String
argument_list|,
name|String
argument_list|>
name|params
init|=
operator|new
name|HashMap
argument_list|<>
argument_list|()
decl_stmt|;
name|String
name|uri
init|=
literal|"something?test=value"
decl_stmt|;
name|RestUtils
operator|.
name|decodeQueryString
argument_list|(
name|uri
argument_list|,
name|uri
operator|.
name|indexOf
argument_list|(
literal|'?'
argument_list|)
operator|+
literal|1
argument_list|,
name|params
argument_list|)
expr_stmt|;
name|assertThat
argument_list|(
name|params
operator|.
name|size
argument_list|()
argument_list|,
name|equalTo
argument_list|(
literal|1
argument_list|)
argument_list|)
expr_stmt|;
name|assertThat
argument_list|(
name|params
operator|.
name|get
argument_list|(
literal|"test"
argument_list|)
argument_list|,
name|equalTo
argument_list|(
literal|"value"
argument_list|)
argument_list|)
expr_stmt|;
name|params
operator|.
name|clear
argument_list|()
expr_stmt|;
name|uri
operator|=
name|String
operator|.
name|format
argument_list|(
name|Locale
operator|.
name|ROOT
argument_list|,
literal|"something?test=value%ctest1=value1"
argument_list|,
name|randomDelimiter
argument_list|()
argument_list|)
expr_stmt|;
name|RestUtils
operator|.
name|decodeQueryString
argument_list|(
name|uri
argument_list|,
name|uri
operator|.
name|indexOf
argument_list|(
literal|'?'
argument_list|)
operator|+
literal|1
argument_list|,
name|params
argument_list|)
expr_stmt|;
name|assertThat
argument_list|(
name|params
operator|.
name|size
argument_list|()
argument_list|,
name|equalTo
argument_list|(
literal|2
argument_list|)
argument_list|)
expr_stmt|;
name|assertThat
argument_list|(
name|params
operator|.
name|get
argument_list|(
literal|"test"
argument_list|)
argument_list|,
name|equalTo
argument_list|(
literal|"value"
argument_list|)
argument_list|)
expr_stmt|;
name|assertThat
argument_list|(
name|params
operator|.
name|get
argument_list|(
literal|"test1"
argument_list|)
argument_list|,
name|equalTo
argument_list|(
literal|"value1"
argument_list|)
argument_list|)
expr_stmt|;
name|params
operator|.
name|clear
argument_list|()
expr_stmt|;
name|uri
operator|=
literal|"something"
expr_stmt|;
name|RestUtils
operator|.
name|decodeQueryString
argument_list|(
name|uri
argument_list|,
name|uri
operator|.
name|length
argument_list|()
argument_list|,
name|params
argument_list|)
expr_stmt|;
name|assertThat
argument_list|(
name|params
operator|.
name|size
argument_list|()
argument_list|,
name|equalTo
argument_list|(
literal|0
argument_list|)
argument_list|)
expr_stmt|;
name|params
operator|.
name|clear
argument_list|()
expr_stmt|;
name|uri
operator|=
literal|"something"
expr_stmt|;
name|RestUtils
operator|.
name|decodeQueryString
argument_list|(
name|uri
argument_list|,
operator|-
literal|1
argument_list|,
name|params
argument_list|)
expr_stmt|;
name|assertThat
argument_list|(
name|params
operator|.
name|size
argument_list|()
argument_list|,
name|equalTo
argument_list|(
literal|0
argument_list|)
argument_list|)
expr_stmt|;
block|}
DECL|method|testDecodeQueryStringEdgeCases
specifier|public
name|void
name|testDecodeQueryStringEdgeCases
parameter_list|()
block|{
name|Map
argument_list|<
name|String
argument_list|,
name|String
argument_list|>
name|params
init|=
operator|new
name|HashMap
argument_list|<>
argument_list|()
decl_stmt|;
name|String
name|uri
init|=
literal|"something?"
decl_stmt|;
name|RestUtils
operator|.
name|decodeQueryString
argument_list|(
name|uri
argument_list|,
name|uri
operator|.
name|indexOf
argument_list|(
literal|'?'
argument_list|)
operator|+
literal|1
argument_list|,
name|params
argument_list|)
expr_stmt|;
name|assertThat
argument_list|(
name|params
operator|.
name|size
argument_list|()
argument_list|,
name|equalTo
argument_list|(
literal|0
argument_list|)
argument_list|)
expr_stmt|;
name|params
operator|.
name|clear
argument_list|()
expr_stmt|;
name|uri
operator|=
name|String
operator|.
name|format
argument_list|(
name|Locale
operator|.
name|ROOT
argument_list|,
literal|"something?%c"
argument_list|,
name|randomDelimiter
argument_list|()
argument_list|)
expr_stmt|;
name|RestUtils
operator|.
name|decodeQueryString
argument_list|(
name|uri
argument_list|,
name|uri
operator|.
name|indexOf
argument_list|(
literal|'?'
argument_list|)
operator|+
literal|1
argument_list|,
name|params
argument_list|)
expr_stmt|;
name|assertThat
argument_list|(
name|params
operator|.
name|size
argument_list|()
argument_list|,
name|equalTo
argument_list|(
literal|0
argument_list|)
argument_list|)
expr_stmt|;
name|params
operator|.
name|clear
argument_list|()
expr_stmt|;
name|uri
operator|=
name|String
operator|.
name|format
argument_list|(
name|Locale
operator|.
name|ROOT
argument_list|,
literal|"something?p=v%c%cp1=v1"
argument_list|,
name|randomDelimiter
argument_list|()
argument_list|,
name|randomDelimiter
argument_list|()
argument_list|)
expr_stmt|;
name|RestUtils
operator|.
name|decodeQueryString
argument_list|(
name|uri
argument_list|,
name|uri
operator|.
name|indexOf
argument_list|(
literal|'?'
argument_list|)
operator|+
literal|1
argument_list|,
name|params
argument_list|)
expr_stmt|;
name|assertThat
argument_list|(
name|params
operator|.
name|size
argument_list|()
argument_list|,
name|equalTo
argument_list|(
literal|2
argument_list|)
argument_list|)
expr_stmt|;
name|assertThat
argument_list|(
name|params
operator|.
name|get
argument_list|(
literal|"p"
argument_list|)
argument_list|,
name|equalTo
argument_list|(
literal|"v"
argument_list|)
argument_list|)
expr_stmt|;
name|assertThat
argument_list|(
name|params
operator|.
name|get
argument_list|(
literal|"p1"
argument_list|)
argument_list|,
name|equalTo
argument_list|(
literal|"v1"
argument_list|)
argument_list|)
expr_stmt|;
name|params
operator|.
name|clear
argument_list|()
expr_stmt|;
name|uri
operator|=
literal|"something?="
expr_stmt|;
name|RestUtils
operator|.
name|decodeQueryString
argument_list|(
name|uri
argument_list|,
name|uri
operator|.
name|indexOf
argument_list|(
literal|'?'
argument_list|)
operator|+
literal|1
argument_list|,
name|params
argument_list|)
expr_stmt|;
name|assertThat
argument_list|(
name|params
operator|.
name|size
argument_list|()
argument_list|,
name|equalTo
argument_list|(
literal|0
argument_list|)
argument_list|)
expr_stmt|;
name|params
operator|.
name|clear
argument_list|()
expr_stmt|;
name|uri
operator|=
name|String
operator|.
name|format
argument_list|(
name|Locale
operator|.
name|ROOT
argument_list|,
literal|"something?%c="
argument_list|,
name|randomDelimiter
argument_list|()
argument_list|)
expr_stmt|;
name|RestUtils
operator|.
name|decodeQueryString
argument_list|(
name|uri
argument_list|,
name|uri
operator|.
name|indexOf
argument_list|(
literal|'?'
argument_list|)
operator|+
literal|1
argument_list|,
name|params
argument_list|)
expr_stmt|;
name|assertThat
argument_list|(
name|params
operator|.
name|size
argument_list|()
argument_list|,
name|equalTo
argument_list|(
literal|0
argument_list|)
argument_list|)
expr_stmt|;
name|params
operator|.
name|clear
argument_list|()
expr_stmt|;
name|uri
operator|=
literal|"something?a"
expr_stmt|;
name|RestUtils
operator|.
name|decodeQueryString
argument_list|(
name|uri
argument_list|,
name|uri
operator|.
name|indexOf
argument_list|(
literal|'?'
argument_list|)
operator|+
literal|1
argument_list|,
name|params
argument_list|)
expr_stmt|;
name|assertThat
argument_list|(
name|params
operator|.
name|size
argument_list|()
argument_list|,
name|equalTo
argument_list|(
literal|1
argument_list|)
argument_list|)
expr_stmt|;
name|assertThat
argument_list|(
name|params
operator|.
name|get
argument_list|(
literal|"a"
argument_list|)
argument_list|,
name|equalTo
argument_list|(
literal|""
argument_list|)
argument_list|)
expr_stmt|;
name|params
operator|.
name|clear
argument_list|()
expr_stmt|;
name|uri
operator|=
name|String
operator|.
name|format
argument_list|(
name|Locale
operator|.
name|ROOT
argument_list|,
literal|"something?p=v%ca"
argument_list|,
name|randomDelimiter
argument_list|()
argument_list|)
expr_stmt|;
name|RestUtils
operator|.
name|decodeQueryString
argument_list|(
name|uri
argument_list|,
name|uri
operator|.
name|indexOf
argument_list|(
literal|'?'
argument_list|)
operator|+
literal|1
argument_list|,
name|params
argument_list|)
expr_stmt|;
name|assertThat
argument_list|(
name|params
operator|.
name|size
argument_list|()
argument_list|,
name|equalTo
argument_list|(
literal|2
argument_list|)
argument_list|)
expr_stmt|;
name|assertThat
argument_list|(
name|params
operator|.
name|get
argument_list|(
literal|"a"
argument_list|)
argument_list|,
name|equalTo
argument_list|(
literal|""
argument_list|)
argument_list|)
expr_stmt|;
name|assertThat
argument_list|(
name|params
operator|.
name|get
argument_list|(
literal|"p"
argument_list|)
argument_list|,
name|equalTo
argument_list|(
literal|"v"
argument_list|)
argument_list|)
expr_stmt|;
name|params
operator|.
name|clear
argument_list|()
expr_stmt|;
name|uri
operator|=
name|String
operator|.
name|format
argument_list|(
name|Locale
operator|.
name|ROOT
argument_list|,
literal|"something?p=v%ca%cp1=v1"
argument_list|,
name|randomDelimiter
argument_list|()
argument_list|,
name|randomDelimiter
argument_list|()
argument_list|)
expr_stmt|;
name|RestUtils
operator|.
name|decodeQueryString
argument_list|(
name|uri
argument_list|,
name|uri
operator|.
name|indexOf
argument_list|(
literal|'?'
argument_list|)
operator|+
literal|1
argument_list|,
name|params
argument_list|)
expr_stmt|;
name|assertThat
argument_list|(
name|params
operator|.
name|size
argument_list|()
argument_list|,
name|equalTo
argument_list|(
literal|3
argument_list|)
argument_list|)
expr_stmt|;
name|assertThat
argument_list|(
name|params
operator|.
name|get
argument_list|(
literal|"a"
argument_list|)
argument_list|,
name|equalTo
argument_list|(
literal|""
argument_list|)
argument_list|)
expr_stmt|;
name|assertThat
argument_list|(
name|params
operator|.
name|get
argument_list|(
literal|"p"
argument_list|)
argument_list|,
name|equalTo
argument_list|(
literal|"v"
argument_list|)
argument_list|)
expr_stmt|;
name|assertThat
argument_list|(
name|params
operator|.
name|get
argument_list|(
literal|"p1"
argument_list|)
argument_list|,
name|equalTo
argument_list|(
literal|"v1"
argument_list|)
argument_list|)
expr_stmt|;
name|params
operator|.
name|clear
argument_list|()
expr_stmt|;
name|uri
operator|=
name|String
operator|.
name|format
argument_list|(
name|Locale
operator|.
name|ROOT
argument_list|,
literal|"something?p=v%ca%cb%cp1=v1"
argument_list|,
name|randomDelimiter
argument_list|()
argument_list|,
name|randomDelimiter
argument_list|()
argument_list|,
name|randomDelimiter
argument_list|()
argument_list|)
expr_stmt|;
name|RestUtils
operator|.
name|decodeQueryString
argument_list|(
name|uri
argument_list|,
name|uri
operator|.
name|indexOf
argument_list|(
literal|'?'
argument_list|)
operator|+
literal|1
argument_list|,
name|params
argument_list|)
expr_stmt|;
name|assertThat
argument_list|(
name|params
operator|.
name|size
argument_list|()
argument_list|,
name|equalTo
argument_list|(
literal|4
argument_list|)
argument_list|)
expr_stmt|;
name|assertThat
argument_list|(
name|params
operator|.
name|get
argument_list|(
literal|"a"
argument_list|)
argument_list|,
name|equalTo
argument_list|(
literal|""
argument_list|)
argument_list|)
expr_stmt|;
name|assertThat
argument_list|(
name|params
operator|.
name|get
argument_list|(
literal|"b"
argument_list|)
argument_list|,
name|equalTo
argument_list|(
literal|""
argument_list|)
argument_list|)
expr_stmt|;
name|assertThat
argument_list|(
name|params
operator|.
name|get
argument_list|(
literal|"p"
argument_list|)
argument_list|,
name|equalTo
argument_list|(
literal|"v"
argument_list|)
argument_list|)
expr_stmt|;
name|assertThat
argument_list|(
name|params
operator|.
name|get
argument_list|(
literal|"p1"
argument_list|)
argument_list|,
name|equalTo
argument_list|(
literal|"v1"
argument_list|)
argument_list|)
expr_stmt|;
block|}
DECL|method|testCorsSettingIsARegex
specifier|public
name|void
name|testCorsSettingIsARegex
parameter_list|()
block|{
name|assertCorsSettingRegex
argument_list|(
literal|"/foo/"
argument_list|,
name|Pattern
operator|.
name|compile
argument_list|(
literal|"foo"
argument_list|)
argument_list|)
expr_stmt|;
name|assertCorsSettingRegex
argument_list|(
literal|"/.*/"
argument_list|,
name|Pattern
operator|.
name|compile
argument_list|(
literal|".*"
argument_list|)
argument_list|)
expr_stmt|;
name|assertCorsSettingRegex
argument_list|(
literal|"/https?:\\/\\/localhost(:[0-9]+)?/"
argument_list|,
name|Pattern
operator|.
name|compile
argument_list|(
literal|"https?:\\/\\/localhost(:[0-9]+)?"
argument_list|)
argument_list|)
expr_stmt|;
name|assertCorsSettingRegexMatches
argument_list|(
literal|"/https?:\\/\\/localhost(:[0-9]+)?/"
argument_list|,
literal|true
argument_list|,
literal|"http://localhost:9200"
argument_list|,
literal|"http://localhost:9215"
argument_list|,
literal|"https://localhost:9200"
argument_list|,
literal|"https://localhost"
argument_list|)
expr_stmt|;
name|assertCorsSettingRegexMatches
argument_list|(
literal|"/https?:\\/\\/localhost(:[0-9]+)?/"
argument_list|,
literal|false
argument_list|,
literal|"htt://localhost:9200"
argument_list|,
literal|"http://localhost:9215/foo"
argument_list|,
literal|"localhost:9215"
argument_list|)
expr_stmt|;
name|assertCorsSettingRegexIsNull
argument_list|(
literal|"//"
argument_list|)
expr_stmt|;
name|assertCorsSettingRegexIsNull
argument_list|(
literal|"/"
argument_list|)
expr_stmt|;
name|assertCorsSettingRegexIsNull
argument_list|(
literal|"/foo"
argument_list|)
expr_stmt|;
name|assertCorsSettingRegexIsNull
argument_list|(
literal|"foo"
argument_list|)
expr_stmt|;
name|assertCorsSettingRegexIsNull
argument_list|(
literal|""
argument_list|)
expr_stmt|;
block|}
DECL|method|testCrazyURL
specifier|public
name|void
name|testCrazyURL
parameter_list|()
block|{
name|Map
argument_list|<
name|String
argument_list|,
name|String
argument_list|>
name|params
init|=
operator|new
name|HashMap
argument_list|<>
argument_list|()
decl_stmt|;
comment|// This is a valid URL
name|String
name|uri
init|=
name|String
operator|.
name|format
argument_list|(
name|Locale
operator|.
name|ROOT
argument_list|,
literal|"example.com/:@-._~!$%c'()*+,=;:@-._~!$%c'()*+,=:@-._~!$%c'()*+,==?/?:@-._~!$'()*+,=/?:@-._~!$'()*+,==#/?:@-._~!$%c'()*+,;="
argument_list|,
name|randomDelimiter
argument_list|()
argument_list|,
name|randomDelimiter
argument_list|()
argument_list|,
name|randomDelimiter
argument_list|()
argument_list|,
name|randomDelimiter
argument_list|()
argument_list|)
decl_stmt|;
name|RestUtils
operator|.
name|decodeQueryString
argument_list|(
name|uri
argument_list|,
name|uri
operator|.
name|indexOf
argument_list|(
literal|'?'
argument_list|)
operator|+
literal|1
argument_list|,
name|params
argument_list|)
expr_stmt|;
name|assertThat
argument_list|(
name|params
operator|.
name|get
argument_list|(
literal|"/?:@-._~!$'()* ,"
argument_list|)
argument_list|,
name|equalTo
argument_list|(
literal|"/?:@-._~!$'()* ,=="
argument_list|)
argument_list|)
expr_stmt|;
name|assertThat
argument_list|(
name|params
operator|.
name|size
argument_list|()
argument_list|,
name|equalTo
argument_list|(
literal|1
argument_list|)
argument_list|)
expr_stmt|;
block|}
DECL|method|assertCorsSettingRegexIsNull
specifier|private
name|void
name|assertCorsSettingRegexIsNull
parameter_list|(
name|String
name|settingsValue
parameter_list|)
block|{
name|assertThat
argument_list|(
name|RestUtils
operator|.
name|checkCorsSettingForRegex
argument_list|(
name|settingsValue
argument_list|)
argument_list|,
name|is
argument_list|(
name|nullValue
argument_list|()
argument_list|)
argument_list|)
expr_stmt|;
block|}
DECL|method|assertCorsSettingRegex
specifier|private
name|void
name|assertCorsSettingRegex
parameter_list|(
name|String
name|settingsValue
parameter_list|,
name|Pattern
name|pattern
parameter_list|)
block|{
name|assertThat
argument_list|(
name|RestUtils
operator|.
name|checkCorsSettingForRegex
argument_list|(
name|settingsValue
argument_list|)
operator|.
name|toString
argument_list|()
argument_list|,
name|is
argument_list|(
name|pattern
operator|.
name|toString
argument_list|()
argument_list|)
argument_list|)
expr_stmt|;
block|}
DECL|method|assertCorsSettingRegexMatches
specifier|private
name|void
name|assertCorsSettingRegexMatches
parameter_list|(
name|String
name|settingsValue
parameter_list|,
name|boolean
name|expectMatch
parameter_list|,
name|String
modifier|...
name|candidates
parameter_list|)
block|{
name|Pattern
name|pattern
init|=
name|RestUtils
operator|.
name|checkCorsSettingForRegex
argument_list|(
name|settingsValue
argument_list|)
decl_stmt|;
for|for
control|(
name|String
name|candidate
range|:
name|candidates
control|)
block|{
name|assertThat
argument_list|(
name|String
operator|.
name|format
argument_list|(
name|Locale
operator|.
name|ROOT
argument_list|,
literal|"Expected pattern %s to match against %s: %s"
argument_list|,
name|settingsValue
argument_list|,
name|candidate
argument_list|,
name|expectMatch
argument_list|)
argument_list|,
name|pattern
operator|.
name|matcher
argument_list|(
name|candidate
argument_list|)
operator|.
name|matches
argument_list|()
argument_list|,
name|is
argument_list|(
name|expectMatch
argument_list|)
argument_list|)
expr_stmt|;
block|}
block|}
block|}
end_class

end_unit

