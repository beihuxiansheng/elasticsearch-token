...
lambda|->
block|{
comment|// we can't use tlog.needsSync() here since it also takes the global checkpoint into account
comment|// we explicitly want to check here if our durability checks are taken into account so we only
comment|// check if we are synced upto the current write location
name|Translog
operator|.
name|Location
name|lastWriteLocation
init|=
name|tlog
operator|.
...
try|try
block|{
comment|// the lastWriteLocaltion has a Integer.MAX_VALUE size so we have to create a new one
return|return
name|tlog
operator|.
name|ensureSynced
...
expr_stmt|;
block|}
DECL|method|testMaybeRollTranslogGeneration
specifier|public
name|void
name|testMaybeRollTranslogGeneration
parameter_list|()
throws|throws
name|Exception
block|{
...
condition|)
block|{
comment|// wait until the roll completes
name|assertBusy
argument_list|(
parameter_list|()
lambda|->
...
block|}
block|}
DECL|method|testStressMaybeFlushOrRollTranslogGeneration
specifier|public
name|void
name|testStressMaybeFlushOrRollTranslogGeneration
parameter_list|()
throws|throws
name|Exception
block|{
...
expr_stmt|;
block|}
comment|// size of the operation plus header and footer
specifier|final
name|Settings
name|settings
init|=
