...
expr_stmt|;
block|}
DECL|method|assertFilePresences
specifier|private
name|void
name|assertFilePresences
parameter_list|(
name|Translog
name|translog
parameter_list|)
...
operator|+
literal|"numOps=55, generation=2, minSeqNo=45, maxSeqNo=99, globalCheckpoint=-2, minTranslogGeneration=1} but got: Checkpoint{offset=0, numOps=0, "
operator|+
literal|"generation=0, minSeqNo=-1, maxSeqNo=-1, globalCheckpoint=-2, minTranslogGeneration=0}"
argument_list|,
name|ex
operator|.
name|getMessage
...
begin_comment
unit|}         }     }
comment|/**      * Tests the situation where the node crashes after a translog gen was committed to lucene, but before the translog had the chance      * to clean up its files.      */
end_comment

begin_function
DECL|method|testRecoveryFromAFutureGenerationCleansUp
unit|public
name|void
name|testRecoveryFromAFutureGenerationCleansUp
parameter_list|()
throws|throws
name|IOException
block|{
...
block|}
block|}
comment|// engine blows up, after committing the above generation
name|translog
operator|.
name|close
argument_list|()
...
argument_list|)
expr_stmt|;
comment|// no trimming done yet, just recovered
for|for
control|(
name|long
name|gen
...
begin_function
DECL|method|testRecoveryFromFailureOnTrimming
specifier|public
name|void
name|testRecoveryFromFailureOnTrimming
parameter_list|()
throws|throws
name|IOException
block|{
...
init|)
block|{
comment|// we don't know when things broke exactly
name|assertThat
argument_list|(
name|translog
operator|.
...
condition|)
block|{
comment|// simulate going OOM and dieing just at the wrong moment.
throw|throw
operator|new
...
condition|)
block|{
comment|// we were committing and blew up in one of the syncs, but they made it through
name|syncedDocs
operator|.
name|clear
argument_list|()
...
argument_list|)
expr_stmt|;
comment|// simulate legacy delete serialization
name|out
operator|=
operator|new
name|BytesStreamOutput
...
name|Exception
block|{
comment|// make sure we keep some files around
specifier|final
name|boolean
name|longRetention
init|=
