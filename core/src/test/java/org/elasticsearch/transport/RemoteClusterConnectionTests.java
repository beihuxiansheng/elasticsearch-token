...
argument_list|()
operator|&&
literal|"operation was cancelled reason [connect handler is closed]"
operator|.
name|equals
argument_list|(
name|message
...
condition|)
block|{
comment|// we do cancel the operation and that means that if timing allows it, the caller
comment|// of a blocking call as well as the handler will get the exception from the
comment|// ExecutionCancelledException concurrently. unless that is the case we fail
comment|// if we get called more than once!
name|AssertionError
name|assertionError
init|=
operator|new
...
begin_comment
comment|// exec again we are already connected
end_comment
