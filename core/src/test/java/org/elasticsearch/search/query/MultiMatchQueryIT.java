...
literal|"category"
block|,
literal|"missing_field"
block|,
literal|"missing_fields*"
block|}
decl_stmt|;
name|String
index|[]
...
argument_list|)
expr_stmt|;
comment|// Check that cross fields works with date fields
name|searchResponse
operator|=
name|client
argument_list|()
...
expr_stmt|;
block|}
comment|/**      * Test for edge case where field level boosting is applied to field that doesn't exist on documents on      * one shard. There was an issue reported in https://github.com/elastic/elasticsearch/issues/18710 where a      * `multi_match` query using the fuzziness parameter with a boost on one of two fields returns the      * same document score if both documents are placed on different shard. This test recreates that scenario      * and checks that the returned scores are different.      */
DECL|method|testFuzzyFieldLevelBoosting
specifier|public
name|void
name|testFuzzyFieldLevelBoosting
parameter_list|()
throws|throws
name|InterruptedException
throws|,
...
argument_list|(
literal|"both documents should be on different shards"
argument_list|,
name|hits
index|[
literal|0
