...
argument_list|)
expr_stmt|;
comment|// Will be fixed once https://github.com/elastic/elasticsearch/pull/20965 is in
comment|// resp = client().prepareSearch("test")
comment|//         .setQuery(queryStringQuery("Foo Bar").splitOnWhitespcae(false))
comment|//         .get();
comment|// assertHits(resp.getHits(), "1", "2", "3");
comment|// assertHitCount(resp, 3L);
block|}
...
literal|"f_ip"
argument_list|)
comment|// Usually this would mean we wouldn't search "all" fields
operator|.
name|useAllFields
argument_list|(
literal|true
argument_list|)
argument_list|)
comment|// ... unless explicitly requested
operator|.
name|get
argument_list|()
decl_stmt|;
