...
annotation|@
name|BeforeClass
DECL|method|beforeClassDisruption
specifier|public
specifier|static
name|void
name|beforeClassDisruption
parameter_list|()
block|{
name|assumeTrue
argument_list|(
literal|"test cannot run with security manager: does evil stuff with threads"
argument_list|,
name|System
operator|.
...
specifier|public
name|void
name|testReadOnPostRecoveryShards
parameter_list|()
throws|throws
name|Exception
block|{
...
name|info
argument_list|(
literal|"--> creating index [test] with one shard and zero replica"
argument_list|)
expr_stmt|;
name|assertAcked
argument_list|(
...
name|info
argument_list|(
literal|"--> starting three more data nodes"
argument_list|)
expr_stmt|;
name|List
argument_list|<
...
name|info
argument_list|(
literal|"--> move shard from node_1 to node_2, and wait for relocation to finish"
argument_list|)
expr_stmt|;
comment|// block cluster state updates on node_3 so that it only sees the shard on node_1
name|BlockClusterStateProcessing
name|disruptionNode3
init|=
operator|new
name|BlockClusterStateProcessing
argument_list|(
...
argument_list|()
expr_stmt|;
comment|// register a Tracer that notifies begin and end of a relocation
name|MockTransportService
...
decl_stmt|;
name|CountDownLatch
name|beginRelocationLatchNode2
init|=
operator|new
name|CountDownLatch
argument_list|(
...
argument_list|(
operator|new
name|StartRecoveryToShardStaredTracer
argument_list|(
name|logger
argument_list|,
name|beginRelocationLatchNode2
...
argument_list|)
expr_stmt|;
comment|// block cluster state updates on node_1 and node_2 so that we end up with two primaries
name|BlockClusterStateProcessing
name|disruptionNode2
init|=
operator|new
name|BlockClusterStateProcessing
argument_list|(
...
name|info
argument_list|(
literal|"--> move shard from node_1 to node_2"
argument_list|)
expr_stmt|;
comment|// don't block on the relocation. cluster state updates are blocked on node_3 and the relocation would timeout
name|Future
argument_list|<
name|ClusterRerouteResponse
argument_list|>
...
name|info
argument_list|(
literal|"--> wait for relocation to start"
argument_list|)
expr_stmt|;
...
argument_list|()
expr_stmt|;
comment|// start to block cluster state updates on node_1 and node_2 so that we end up with two primaries
comment|// one STARTED on node_1 and one in POST_RECOVERY on node_2
name|disruptionNode1
operator|.
name|startDisrupting
argument_list|()
...
argument_list|)
expr_stmt|;
comment|//sometimes refresh and sometimes flush
name|int
name|refreshOrFlushType
init|=
...
argument_list|)
expr_stmt|;
comment|// the total shards is num replicas + 1 so that can be lower here because one shard
comment|// is relocating and counts twice as successful
name|assertThat
argument_list|(
name|refreshResponse
operator|.
...
name|fail
argument_list|(
literal|"this is  test bug, number should be between 1 and 2"
argument_list|)
expr_stmt|;
block|}
comment|// now stop disrupting so that node_3 can ack last cluster state to master and master can continue
comment|// to publish the next cluster state
name|logger
operator|.
name|info
argument_list|(
literal|"--> stop disrupting node_3"
argument_list|)
expr_stmt|;
name|disruptionNode3
operator|.
...
name|info
argument_list|(
literal|"--> wait for node_4 to get new cluster state"
argument_list|)
expr_stmt|;
comment|// wait until node_4 actually has the new cluster state in which node_1 has no shard
name|assertBusy
argument_list|(
operator|new
name|Runnable
...
argument_list|()
decl_stmt|;
comment|// get the node id from the name. TODO: Is there a better way to do this?
name|String
name|nodeId
init|=
literal|null
...
argument_list|)
expr_stmt|;
comment|// check that node_1 does not have the shard in local cluster state
name|assertFalse
argument_list|(
name|clusterState
operator|.
...
name|info
argument_list|(
literal|"--> stop disrupting node_1 and node_2"
argument_list|)
expr_stmt|;
name|disruptionNode2
operator|.
...
name|info
argument_list|(
literal|"--> wait for relocation to finish"
argument_list|)
expr_stmt|;
name|clusterHealth
operator|=
...
block|}
block|}
comment|/**      * This Tracer can be used to signal start of a recovery and shard started event after translog was copied      */
DECL|class|StartRecoveryToShardStaredTracer
specifier|public
specifier|static
class|class
name|StartRecoveryToShardStaredTracer
...
decl_stmt|;
DECL|field|sentShardStartedLatch
specifier|private
specifier|final
name|CountDownLatch
name|sentShardStartedLatch
decl_stmt|;
DECL|method|StartRecoveryToShardStaredTracer
specifier|public
name|StartRecoveryToShardStaredTracer
parameter_list|(
name|ESLogger
...
block|}
block|}
DECL|method|logLocalClusterStates
specifier|private
name|void
name|logLocalClusterStates
parameter_list|(
...
name|info
argument_list|(
literal|"--> cluster state on node_{} {}"
argument_list|,
name|counter
argument_list|,
name|clusterState
