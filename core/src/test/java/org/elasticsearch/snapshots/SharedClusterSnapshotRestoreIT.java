...
argument_list|()
decl_stmt|;
comment|// At least something should be stored
if|if
condition|(
name|testTemplate
condition|)
...
name|info
argument_list|(
literal|"-->  creating test pipeline"
argument_list|)
expr_stmt|;
name|BytesReference
name|pipelineSource
...
name|info
argument_list|(
literal|"--> check that pipeline is restored"
argument_list|)
expr_stmt|;
name|GetPipelineResponse
name|getPipelineResponse
...
name|info
argument_list|(
literal|"--> check that script is restored"
argument_list|)
expr_stmt|;
name|GetStoredScriptResponse
name|getStoredScriptResponse
...
name|info
argument_list|(
literal|"-->  delete global state and index "
argument_list|)
expr_stmt|;
name|cluster
argument_list|()
...
name|info
argument_list|(
literal|"--> check that global state wasn't restored but index was"
argument_list|)
expr_stmt|;
name|getIndexTemplatesResponse
operator|=
...
specifier|public
name|void
name|testDataFileCorruptionDuringRestore
parameter_list|()
throws|throws
name|Exception
block|{
...
argument_list|)
expr_stmt|;
comment|// we have to delete the index here manually, otherwise the cluster will keep
...
expr_stmt|;
block|}
DECL|method|testSnapshotWithMissingShardLevelIndexFile
specifier|public
name|void
name|testSnapshotWithMissingShardLevelIndexFile
parameter_list|()
throws|throws
name|Exception
block|{
...
name|info
argument_list|(
literal|"--> deleting shard level index file"
argument_list|)
expr_stmt|;
try|try
init|(
...
name|info
argument_list|(
literal|"--> creating another snapshot"
argument_list|)
expr_stmt|;
name|CreateSnapshotResponse
name|createSnapshotResponse
...
name|info
argument_list|(
literal|"--> restoring the first snapshot, the repository should not have lost any shard data despite deleting index-N, "
operator|+
literal|"because it should have iterated over the snap-*.data files as backup"
argument_list|)
expr_stmt|;
name|client
argument_list|()
...
argument_list|)
expr_stmt|;
comment|// test that getting an unavailable snapshot status throws an exception if ignoreUnavailable is false on the request
name|SnapshotMissingException
name|ex
init|=
name|expectThrows
...
name|assertEquals
argument_list|(
literal|"[test-repo:test-snap-doesnt-exist] is missing"
argument_list|,
name|ex
operator|.
name|getMessage
...
argument_list|)
expr_stmt|;
comment|// test that getting an unavailable snapshot status does not throw an exception if ignoreUnavailable is true on the request
name|response
operator|=
name|client
operator|.
...
argument_list|)
expr_stmt|;
comment|// test getting snapshot status for available and unavailable snapshots where ignoreUnavailable is true
comment|// (available one should be returned)
name|response
operator|=
name|client
operator|.
...
expr_stmt|;
block|}
DECL|method|testDeleteSnapshotWhileRestoringFails
specifier|public
name|void
name|testDeleteSnapshotWhileRestoringFails
parameter_list|()
throws|throws
name|Exception
block|{
...
name|info
argument_list|(
literal|"--> delete index before restoring"
argument_list|)
expr_stmt|;
name|assertAcked
argument_list|(
...
name|info
argument_list|(
literal|"--> try deleting the snapshot while the restore is in progress (should throw an error)"
argument_list|)
expr_stmt|;
...
name|containsString
argument_list|(
literal|"cannot delete snapshot during a restore"
argument_list|)
argument_list|)
expr_stmt|;
name|logger
...
name|info
argument_list|(
literal|"-- try deleting another snapshot while the restore is in progress (should throw an error)"
argument_list|)
expr_stmt|;
name|e
operator|=
...
block|}
block|}
DECL|method|testCannotCreateSnapshotsWithSameName
specifier|public
name|void
name|testCannotCreateSnapshotsWithSameName
parameter_list|()
throws|throws
name|Exception
block|{
...
name|info
argument_list|(
literal|"--> second snapshot of the same name should fail"
argument_list|)
expr_stmt|;
try|try
block|{
...
name|fail
argument_list|(
literal|"should not be allowed to create a snapshot with the same name as an already existing snapshot: "
operator|+
name|createSnapshotResponse
operator|.
name|getSnapshotInfo
...
name|info
argument_list|(
literal|"--> delete the first snapshot"
argument_list|)
expr_stmt|;
name|client
operator|.
...
name|info
argument_list|(
literal|"--> try creating a snapshot with the same name, now it should work because the first one was deleted"
argument_list|)
expr_stmt|;
name|createSnapshotResponse
operator|=
...
expr_stmt|;
block|}
DECL|method|testGetSnapshotsRequest
specifier|public
name|void
name|testGetSnapshotsRequest
parameter_list|()
throws|throws
name|Exception
block|{
...
name|info
argument_list|(
literal|"--> get snapshots on an empty repository"
argument_list|)
expr_stmt|;
name|expectThrows
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// with ignore unavailable set to true, should not throw an exception
name|GetSnapshotsResponse
name|getSnapshotsResponse
init|=
name|client
...
argument_list|()
expr_stmt|;
comment|// make sure we return only the in-progress snapshot when taking the first snapshot on a clean repository
comment|// take initial snapshot with a block, making sure we only get 1 in-progress snapshot returned
comment|// block a node so the create snapshot operation can remain in progress
specifier|final
name|String
name|initialBlockedNode
init|=
name|blockNodeWithIndex
argument_list|(
name|repositoryName
...
argument_list|)
expr_stmt|;
comment|// wait for block to kick in
name|getSnapshotsResponse
operator|=
name|client
operator|.
...
argument_list|)
expr_stmt|;
comment|// timeout after 10 seconds
name|client
operator|.
name|admin
argument_list|()
...
name|info
argument_list|(
literal|"--> take another snapshot to be in-progress"
argument_list|)
expr_stmt|;
comment|// add documents so there are data files to block on
for|for
control|(
name|int
name|i
...
name|info
argument_list|(
literal|"--> get all snapshots with a current in-progress"
argument_list|)
expr_stmt|;
comment|// with ignore unavailable set to true, should not throw an exception
specifier|final
...
condition|)
block|{
comment|// use _current plus the individual names of the finished snapshots
name|snapshotsToGet
operator|.
name|add
argument_list|(
...
name|info
argument_list|(
literal|"--> make sure duplicates are not returned in the response"
argument_list|)
expr_stmt|;
name|String
name|regexName
...
expr_stmt|;
block|}
comment|/**      * This test ensures that when a shard is removed from a node (perhaps due to the node      * leaving the cluster, then returning), all snapshotting of that shard is aborted, so      * all Store references held onto by the snapshot are released.      *      * See https://github.com/elastic/elasticsearch/issues/20876      */
DECL|method|testSnapshotCanceledOnRemovedShard
specifier|public
name|void
name|testSnapshotCanceledOnRemovedShard
parameter_list|()
throws|throws
name|Exception
block|{
...
name|info
argument_list|(
literal|"--> removing primary shard that is being snapshotted"
argument_list|)
expr_stmt|;
name|ClusterState
name|clusterState
...
name|assertNotNull
argument_list|(
literal|"should be at least one node with a primary shard"
argument_list|,
name|nodeWithPrimary
argument_list|)
expr_stmt|;
...
name|info
argument_list|(
literal|"--> ensuring snapshot is aborted and the aborted shard was marked as failed"
argument_list|)
expr_stmt|;
name|SnapshotInfo
name|snapshotInfo
...
name|assertEquals
argument_list|(
literal|"IndexShardSnapshotFailedException[Aborted]"
argument_list|,
name|snapshotInfo
operator|.
name|shardFailures
...
specifier|public
name|void
name|testSnapshotSucceedsAfterSnapshotFailure
parameter_list|()
throws|throws
name|Exception
block|{
...
name|info
argument_list|(
literal|"--> snapshot with potential I/O failures"
argument_list|)
expr_stmt|;
try|try
block|{
...
parameter_list|)
block|{
comment|// sometimes, the snapshot will fail with a top level I/O exception
name|assertThat
argument_list|(
name|ExceptionsHelper
operator|.
...
name|info
argument_list|(
literal|"--> snapshot with no I/O failures"
argument_list|)
expr_stmt|;
name|assertAcked
argument_list|(
...
expr_stmt|;
block|}
DECL|method|testSnapshotStatusOnFailedIndex
specifier|public
name|void
name|testSnapshotStatusOnFailedIndex
parameter_list|()
throws|throws
name|Exception
block|{
...
literal|0
argument_list|)
comment|// set shard allocation to none so the primary cannot be
comment|// allocated - simulates a "bad" index that fails to snapshot
operator|.
name|put
argument_list|(
name|EnableAllocationDecider
...
name|info
argument_list|(
literal|"--> snapshot bad index and get status"
argument_list|)
expr_stmt|;
name|client
operator|.
...
name|info
argument_list|(
literal|"--> snapshot both good and bad index and get status"
argument_list|)
expr_stmt|;
name|client
operator|.
...
argument_list|)
expr_stmt|;
comment|// verify a FAILED status is returned instead of a 500 status code
comment|// see https://github.com/elastic/elasticsearch/issues/23716
name|SnapshotStatus
name|snapshotStatus
init|=
name|snapshotsStatusResponse
...
block|}
block|}
DECL|method|testGetSnapshotsFromIndexBlobOnly
specifier|public
name|void
name|testGetSnapshotsFromIndexBlobOnly
parameter_list|()
throws|throws
name|Exception
block|{
...
name|info
argument_list|(
literal|"--> creating random number of indices"
argument_list|)
expr_stmt|;
specifier|final
name|int
...
name|info
argument_list|(
literal|"--> creating random number of snapshots"
argument_list|)
expr_stmt|;
specifier|final
name|int
...
control|)
block|{
comment|// index some additional docs (maybe) for each index
for|for
control|(
name|int
name|j
...
name|info
argument_list|(
literal|"--> verify _all returns snapshot info"
argument_list|)
expr_stmt|;
name|GetSnapshotsResponse
name|response
...
name|info
argument_list|(
literal|"--> verify wildcard returns snapshot info"
argument_list|)
expr_stmt|;
name|response
operator|=
...
name|info
argument_list|(
literal|"--> verify individual requests return snapshot info"
argument_list|)
expr_stmt|;
for|for
control|(
...
block|}
block|}
DECL|method|verifySnapshotInfo
specifier|private
name|void
name|verifySnapshotInfo
parameter_list|(
