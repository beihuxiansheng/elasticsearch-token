...
name|policy
decl_stmt|;
comment|// if its an insecure plugin, we use a wrapper policy impl to try
...
argument_list|)
decl_stmt|;
comment|// in case we are running from the IDE:
if|if
condition|(
name|artifact
operator|==
...
argument_list|(
literal|"plugin classname is needed for insecure plugin unit tests: something wrong with build"
argument_list|)
throw|;
block|}
block|}
