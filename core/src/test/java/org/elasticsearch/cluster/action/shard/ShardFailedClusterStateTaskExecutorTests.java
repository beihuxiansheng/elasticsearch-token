...
condition|)
block|{
comment|// the shard was successfully failed and so should not
comment|// be in the routing table
for|for
control|(
name|ShardRouting
name|shard
...
return|;
block|}
DECL|method|randomValidSourceShard
specifier|private
specifier|static
name|ShardRouting
name|randomValidSourceShard
...
parameter_list|)
block|{
comment|// for the request node ID to be valid, either the request is
comment|// from the node the shard is assigned to, or the request is
comment|// from the node holding the primary shard
if|if
condition|(
name|randomBoolean
argument_list|()
condition|)
block|{
comment|// request from local node
return|return
name|shardRouting
return|;
block|}
...
block|{
comment|// request from primary node unless in the case of
comment|// non-existent shards there is not one and we fallback to
...
block|}
block|}
DECL|method|randomInvalidSourceShard
specifier|private
specifier|static
name|ShardRouting
name|randomInvalidSourceShard
