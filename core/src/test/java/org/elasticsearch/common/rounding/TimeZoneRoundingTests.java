...
block|}
block|}
comment|/**      * tests for dst transition with overlaps and day roundings.      */
DECL|method|testDST_END_Edgecases
specifier|public
name|void
name|testDST_END_Edgecases
parameter_list|()
block|{
comment|// First case, dst happens at 1am local time, switching back one hour.
comment|// We want the overlapping hour to count for the next day, making it a 25h interval
name|DateTimeUnit
name|timeUnit
init|=
name|DateTimeUnit
...
argument_list|)
decl_stmt|;
comment|// Sunday, 29 October 2000, 01:00:00 clocks were turned backward 1 hour
comment|// to Sunday, 29 October 2000, 00:00:00 local standard time instead
name|long
name|midnightBeforeTransition
init|=
name|time
argument_list|(
literal|"2000-10-29T00:00:00"
argument_list|,
name|tz
argument_list|)
decl_stmt|;
...
argument_list|)
expr_stmt|;
comment|// Second case, dst happens at 0am local time, switching back one hour to 23pm local time.
comment|// We want the overlapping hour to count for the previous day here
name|tz
operator|=
name|DateTimeZone
operator|.
...
argument_list|)
expr_stmt|;
comment|// Sunday, 1 April 1990, 00:00:00 clocks were turned backward 1 hour to
comment|// Saturday, 31 March 1990, 23:00:00 local standard time instead
name|midnightBeforeTransition
operator|=
name|time
argument_list|(
literal|"1990-03-31T00:00:00.000-04:00"
argument_list|)
expr_stmt|;
name|nextMidnight
operator|=
name|time
argument_list|(
literal|"1990-04-01T00:00:00.000-05:00"
argument_list|)
expr_stmt|;
name|assertInterval
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// make sure the next interval is 24h long again
name|long
name|midnightAfterTransition
init|=
name|time
argument_list|(
literal|"1990-04-01T00:00:00.000-05:00"
...
name|time
argument_list|(
literal|"1990-04-02T00:00:00.000-05:00"
argument_list|)
expr_stmt|;
name|assertInterval
argument_list|(
...
expr_stmt|;
block|}
comment|/**      * Test that time zones are correctly parsed. There is a bug with      * Joda 2.9.4 (see https://github.com/JodaOrg/joda-time/issues/373)      */
DECL|method|testsTimeZoneParsing
specifier|public
name|void
name|testsTimeZoneParsing
parameter_list|()
block|{
specifier|final
name|DateTime
...
argument_list|)
decl_stmt|;
comment|// Formatter used to print and parse the sample date.
comment|// Printing the date works but parsing it back fails
comment|// with Joda 2.9.4
name|DateTimeFormatter
name|formatter
init|=
name|DateTimeFormat
