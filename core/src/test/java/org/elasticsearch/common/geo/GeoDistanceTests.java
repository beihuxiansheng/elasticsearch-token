...
expr_stmt|;
block|}
comment|/**      * The old plane calculation in 1.x/2.x incorrectly computed the plane distance in decimal degrees. This test is      * well intended but bogus. todo: fix w/ new plane distance calculation      * note: plane distance error varies by latitude so the test will need to correctly estimate expected error      */
annotation|@
name|AwaitsFix
argument_list|(
name|bugUrl
operator|=
literal|"old plane calculation incorrectly computed everything in degrees. fix this bogus test"
argument_list|)
