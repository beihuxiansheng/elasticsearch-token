...
block|}
block|}
comment|/**      * Returns the bytes that represent the XContent output of the provided {@link ToXContent} object, using the provided      * {@link XContentType}. Wraps the output into a new anonymous object according to the value returned      * by the {@link ToXContent#isFragment()} method returns. Shuffles the keys to make sure that parsing never relies on keys ordering.      */
DECL|method|toShuffledXContent
specifier|protected
specifier|final
name|BytesReference
...
block|}
block|}
comment|/**      * Randomly shuffles the fields inside objects parsed using the {@link XContentParser} passed in.      * Recursively goes through inner objects and also shuffles them. Exceptions for this      * recursive shuffling behavior can be made by passing in the names of fields which      * internally should stay untouched.      */
DECL|method|shuffleXContent
specifier|public
name|XContentBuilder
name|shuffleXContent
...
expr_stmt|;
block|}
comment|/**      * Assert that two objects are equals, calling {@link ToXContent#toXContent(XContentBuilder, ToXContent.Params)} to print out their      * differences if they aren't equal.      */
DECL|method|assertEqualsWithErrorMessageFromXContent
specifier|public
specifier|static
parameter_list|<
name|T
...
name|AssertionError
argument_list|(
literal|"Expected null be actual was ["
operator|+
name|actual
operator|.
name|toString
...
name|AssertionError
argument_list|(
literal|"Didn't expect null but actual was [null]"
argument_list|)
throw|;
block|}
try|try
...
name|AssertionError
argument_list|(
literal|"IOException while building failure message"
argument_list|,
name|e
argument_list|)
throw|;
...
return|;
block|}
comment|/**      * Create a "mock" script for use either with {@link MockScriptEngine} or anywhere where you need a script but don't really care about      * its contents.      */
DECL|method|mockScript
specifier|public
specifier|static
specifier|final
name|Script
