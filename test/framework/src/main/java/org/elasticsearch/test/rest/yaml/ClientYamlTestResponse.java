...
name|startsWith
argument_list|(
literal|"Deprecated use of the [source] parameter without the [source_content_type] parameter."
argument_list|)
condition|)
block|{
comment|// this is because we do not send the source content type header when the node is 5.3.0 or below and the request
comment|// might have been sent to a node with a version> 5.3.0 when running backwards 5.0 tests. The Java RestClient
comment|// has control of the node the request is sent to so we can only detect this after the fact right now
comment|// TODO remove this when we bump versions
block|}
else|else
block|{
name|warningHeaders
