...
return|;
block|}
comment|//we only get here if there is no response body or the body is text
assert|assert
name|bodyContentType
operator|==
literal|null
...
condition|)
block|{
comment|//content-type null means that text was returned
if|if
condition|(
name|bodyContentType
operator|==
...
block|{
comment|//if the body is in a binary format and gets requested as a string (e.g. to log a test failure), we convert it to json
try|try
init|(
name|XContentBuilder
name|jsonBuilder
...
name|UncheckedIOException
argument_list|(
literal|"unable to convert response body to a string format"
argument_list|,
name|e
argument_list|)
throw|;
