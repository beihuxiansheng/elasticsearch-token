...
condition|)
block|{
comment|// to make sure that a master node will not pick up on the data folder of a data only node
comment|// once restarted we append the role suffix to each path.
name|String
index|[]
name|dataPath
init|=
...
expr_stmt|;
block|}
comment|// starting master nodes first, for now so restart will be quick. If we'll start
comment|// the data nodes first, they will wait for 30s for a master
name|List
argument_list|<
name|DiscoveryNode
argument_list|>
