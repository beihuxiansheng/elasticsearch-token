...
name|executor
decl_stmt|;
DECL|field|autoManageMinMasterNodes
specifier|private
specifier|final
name|boolean
...
return|;
block|}
comment|/** returns true if the {@link ElectMasterService#DISCOVERY_ZEN_MINIMUM_MASTER_NODES_SETTING} setting is auto managed by this cluster */
DECL|method|getAutoManageMinMasterNode
specifier|public
name|boolean
...
block|}
block|}
comment|/**      * builds a new node given the settings.      *      * @param settings              the settings to use      * @param defaultMinMasterNodes min_master_nodes value to use if min_master_nodes is auto managed      */
...
return|;
block|}
comment|/**      * builds a new node with default settings      *      * @param defaultMinMasterNodes min_master_nodes value to use if min_master_nodes is auto managed      */
DECL|method|buildNode
specifier|private
name|NodeAndClient
name|buildNode
...
return|;
block|}
comment|/**      * builds a new node      *      * @param nodeId                the node internal id (see {@link NodeAndClient#nodeAndClientId()}      * @param seed                  the node's random seed      * @param settings              the settings to use      * @param reuseExisting         if a node with the same name is already part of {@link #nodes}, no new node will be built and      *                              the method will return the existing one      * @param defaultMinMasterNodes min_master_nodes value to use if min_master_nodes is auto managed      */
DECL|method|buildNode
specifier|private
name|NodeAndClient
name|buildNode
...
expr_stmt|;
block|}
comment|/**          * closes the current node if not already closed, builds a new node object using the current node settings and starts it          */
DECL|method|restart
name|void
name|restart
parameter_list|(
...
expr_stmt|;
block|}
comment|/**          * rebuilds a new node object using the current node settings and starts it          */
DECL|method|recreateNodeOnRestart
name|void
name|recreateNodeOnRestart
parameter_list|(
name|RestartCallback
...
literal|false
operator|:
literal|"min master nodes is auto managed"
assert|;
name|newSettings
operator|.
name|put
...
expr_stmt|;
block|}
comment|// validation is (optionally) done in fullRestart/rollingRestart
name|newSettings
operator|.
name|put
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// make sure cached client points to new node
name|resetClient
argument_list|()
expr_stmt|;
block|}
...
condition|)
block|{
name|updateMinMasterNodes
argument_list|(
name|getMasterNodesCount
argument_list|()
argument_list|)
expr_stmt|;
block|}
...
argument_list|()
decl_stmt|;
comment|// we want to start nodes in one go due to min master nodes
for|for
control|(
name|int
name|i
...
expr_stmt|;
block|}
name|startAndPublishNodesAndClients
argument_list|(
name|toStartAndPublish
argument_list|)
expr_stmt|;
...
name|AssertionError
argument_list|(
literal|"expected index shard counter on shard "
operator|+
name|indexShard
operator|.
name|shardId
...
name|name
operator|+
literal|" to be 0 but was "
operator|+
name|activeOperationsCount
operator|+
literal|". Current replication tasks on node:\n"
operator|+
name|builder
operator|.
name|string
...
literal|false
argument_list|)
comment|// filter out old masters
operator|.
name|count
argument_list|()
decl_stmt|;
...
condition|)
block|{
comment|// special case for 1 node master - we can't update the min master nodes before we add more nodes.
name|updateMinMasterNodes
argument_list|(
name|currentMasters
operator|+
...
condition|)
block|{
comment|// update once masters have joined
name|validateClusterFormed
argument_list|()
expr_stmt|;
name|updateMinMasterNodes
...
specifier|final
name|int
name|masterNodesCount
init|=
name|getMasterNodesCount
argument_list|()
decl_stmt|;
comment|// special case to allow stopping one node in a two node cluster and keep it functional
specifier|final
name|boolean
name|updateMinMaster
init|=
name|nodeAndClient
operator|.
name|isMasterEligible
...
name|Set
index|[]
name|rolesOrderedByOriginalStartupOrder
init|=
operator|new
name|Set
index|[
...
condition|)
block|{
comment|// if some nodes were stopped, we want have a role for that ordinal
continue|continue;
block|}
specifier|final
name|List
...
literal|0
assert|;
comment|// do two rounds to minimize pinging (mock zen pings pings with no delay and can create a lot of logs)
for|for
control|(
name|NodeAndClient
name|nodeAndClient
...
condition|)
block|{
comment|// there should be at least one master to update
name|logger
operator|.
name|debug
argument_list|(
literal|"updating min_master_nodes to [{}]"
argument_list|,
name|minMasterNodes
argument_list|)
expr_stmt|;
...
name|ElasticsearchException
argument_list|(
literal|"failed to update minimum master node to [{}] (current masters [{}])"
argument_list|,
name|e
argument_list|,
name|minMasterNodes
...
return|;
block|}
comment|/** returns true if the restart should also validate the cluster has reformed */
DECL|method|validateClusterForming
specifier|public
name|boolean
name|validateClusterForming
parameter_list|()
