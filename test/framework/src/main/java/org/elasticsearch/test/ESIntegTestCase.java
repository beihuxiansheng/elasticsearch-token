...
condition|)
block|{
comment|// this is set by the test infra
name|transientSettings
operator|.
name|remove
argument_list|(
...
return|;
block|}
comment|/**      * Creates a new {@link CreateIndexRequestBuilder} with the settings obtained from {@link #indexSettings()}, augmented      * by the given builder      */
DECL|method|prepareCreate
specifier|public
name|CreateIndexRequestBuilder
name|prepareCreate
...
literal|true
argument_list|)
comment|// We currently often use ensureGreen or ensureYellow to check whether the cluster is back in a good state after shutting down
comment|// a node. If the node that is stopped is the master node, another node will become master and publish a cluster state where it
comment|// is master but where the node that was stopped hasn't been removed yet from the cluster state. It will only subsequently
comment|// publish a second state where the old master is removed. If the ensureGreen/ensureYellow is timed just right, it will get to
comment|// execute before the second cluster state update removes the old master and the condition ensureGreen / ensureYellow will
comment|// trivially hold if it held before the node was shut down. The following "waitForNodes" condition ensures that the node has
comment|// been removed by the master so that the health check applies to the set of nodes we expect to be part of the cluster.
operator|.
name|waitForNodes
argument_list|(
name|Integer
...
name|info
argument_list|(
literal|"{} timed out, cluster state:\n{}\n{}"
argument_list|,
name|method
argument_list|,
name|client
...
condition|)
block|{
comment|// If it's internal cluster - using existing registry in case plugin registered custom data
name|namedWriteableRegistry
operator|=
name|internalCluster
argument_list|()
...
block|{
comment|// If it's external cluster - fall back to the standard set
name|namedWriteableRegistry
operator|=
operator|new
name|NamedWriteableRegistry
...
argument_list|)
expr_stmt|;
name|ensureFullyConnectedCluster
argument_list|()
expr_stmt|;
block|}
comment|/**      * Ensures that all nodes in the cluster are connected to each other.      *      * Some network disruptions may leave nodes that are not the master disconnected from each other.      * {@link org.elasticsearch.cluster.NodeConnectionsService} will eventually reconnect but it's      * handy to be able to ensure this happens faster      */
DECL|method|ensureFullyConnectedCluster
specifier|protected
name|void
name|ensureFullyConnectedCluster
parameter_list|()
...
argument_list|)
expr_stmt|;
comment|// We configure a routing key in case the mapping requires it
name|builders
operator|.
name|add
argument_list|(
...
literal|true
function_decl|;
comment|/**          * The cluster automatically manages the {@link ElectMasterService#DISCOVERY_ZEN_MINIMUM_MASTER_NODES_SETTING} by default          * as nodes are started and stopped. Set this to false to manage the setting manually.          */
DECL|field|true
name|boolean
...
return|;
block|}
DECL|method|getAutoMinMasterNodes
specifier|private
name|boolean
name|getAutoMinMasterNodes
parameter_list|()
block|{
name|ClusterScope
name|annotation
...
return|;
block|}
comment|/**      * Asserts that all segments are sorted with the provided {@link Sort}.      */
DECL|method|assertSortedSegments
specifier|public
name|void
