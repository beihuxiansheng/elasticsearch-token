...
annotation|@
name|Test
DECL|method|testRangeFilterNoCacheWithNow
specifier|public
name|void
name|testRangeFilterNoCacheWithNow
parameter_list|()
throws|throws
name|Exception
block|{
...
literal|"test"
argument_list|)
comment|//no replicas to make sure we always hit the very same shard and verify the caching behaviour
operator|.
name|setSettings
argument_list|(
name|ImmutableSettings
...
literal|"date"
argument_list|,
literal|"type=date,format=YYYY-mm-dd"
argument_list|)
argument_list|)
expr_stmt|;
name|ensureGreen
...
argument_list|)
expr_stmt|;
comment|// filter cache should not contain any thing, b/c `now` is used in `to`.
name|IndicesStatsResponse
name|statsResponse
init|=
name|client
...
argument_list|)
expr_stmt|;
comment|// Now with rounding is used, so we must have something in filter cache
name|statsResponse
operator|=
name|client
argument_list|()
...
argument_list|)
expr_stmt|;
comment|// and because we use term filter, it is also added to filter cache, so it should contain more than before
name|statsResponse
operator|=
name|client
argument_list|()
