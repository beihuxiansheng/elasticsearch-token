...
name|GLOBAL
case|:
comment|// never put the global cluster into the map otherwise it will be closed
if|if
condition|(
name|GLOBAL_CLUSTER
operator|==
...
name|GLOBAL_CLUSTER
operator|:
literal|"Global cluster must be handled via the GLOBAL_CLUSTER static member"
assert|;
name|clusters
operator|.
name|put
...
control|)
block|{
comment|// for now on windows we only ensure that there is at least no segments_N file left on the path
comment|// we don't have a retry mechanism in place yet.
name|assertFalse
argument_list|(
name|foundFiles
operator|.
