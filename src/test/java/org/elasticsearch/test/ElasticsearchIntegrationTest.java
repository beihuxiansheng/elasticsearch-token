...
literal|null
operator|:
literal|"Scope.GLOBAL cluster must be initialied in a static context"
assert|;
name|currentCluster
operator|=
name|GLOBAL_CLUSTER
...
argument_list|)
operator|:
literal|"Scope.SUITE cluster must be initialized in a static context"
assert|;
name|currentCluster
operator|=
...
block|{
comment|// if we carry that cluster over ie. it was already there we remove it and then call clearClusters
comment|// and put it back afterwards such that all pending leftover clusters are closed and disposed...
name|clusters
operator|.
name|remove
argument_list|(
...
argument_list|()
decl_stmt|;
comment|// if we have suite scoped cluster here we need to initialize the
comment|// cluster before the first test starts for reproducibility
name|instance
operator|.
name|buildAndPutCluster
argument_list|(
...
return|;
block|}
DECL|method|isSuiteScopeCluster
specifier|private
specifier|static
name|boolean
name|isSuiteScopeCluster
parameter_list|(
name|Class
argument_list|<
name|?
