...
decl_stmt|;
DECL|field|REST_TESTS_SECTION
specifier|public
specifier|static
specifier|final
name|String
name|REST_TESTS_SECTION
init|=
literal|"tests.rest.section"
decl_stmt|;
...
argument_list|)
expr_stmt|;
comment|//we continue so that we collect all parse errors and show them all at once
continue|continue;
block|}
name|Description
...
block|}
block|}
comment|//we add the suite only if it has at least a section left
if|if
condition|(
name|testSuiteDescription
operator|.
...
block|}
block|}
comment|//we add the api only if it has at least a suite left
if|if
condition|(
name|apiDescription
operator|.
