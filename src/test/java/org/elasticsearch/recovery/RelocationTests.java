...
specifier|public
name|void
name|testRelocationWithBusyClusterUpdateThread
parameter_list|()
throws|throws
name|Exception
block|{
...
argument_list|)
expr_stmt|;
comment|// capture the incoming state indicate that the replicas have upgraded and assigned
specifier|final
name|CountDownLatch
name|allReplicasAssigned
init|=
operator|new
name|CountDownLatch
argument_list|(
...
condition|)
block|{
comment|// we have no replicas or they are not assigned yet
return|return;
block|}
name|allReplicasAssigned
operator|.
...
name|info
argument_list|(
literal|"--> starting replica recovery"
argument_list|)
expr_stmt|;
comment|// we don't expect this to be acknowledge by node1 where we block the cluster state thread
name|assertFalse
argument_list|(
name|client
argument_list|()
...
name|info
argument_list|(
literal|"--> waiting for node1 to process replica existence"
argument_list|)
expr_stmt|;
name|allReplicasAssigned
operator|.
...
name|info
argument_list|(
literal|"--> waiting for recovery to fail"
argument_list|)
expr_stmt|;
name|unassignedShardsAfterReplicasAssigned
operator|.
...
name|info
argument_list|(
literal|"--> releasing cluster state update thread"
argument_list|)
expr_stmt|;
name|releaseClusterState
operator|.
...
name|info
argument_list|(
literal|"--> waiting for recovery to succeed"
argument_list|)
expr_stmt|;
comment|// force a move.
name|client
argument_list|()
operator|.
name|admin
