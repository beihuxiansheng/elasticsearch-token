...
name|fail
argument_list|(
literal|"engine should have failed"
argument_list|)
expr_stmt|;
block|}
catch|catch
...
expr_stmt|;
block|}
comment|// and we should not leak files
name|assertThat
argument_list|(
literal|"there are unreferenced translog files left"
argument_list|,
name|translog
operator|.
...
name|assertThat
argument_list|(
literal|"there are unreferenced translog files left, post flush"
argument_list|,
name|translog
operator|.
name|clearUnreferenced
...
parameter_list|)
block|{
comment|// sometimes we fail after we committed the recovered docs during the finaly refresh call
comment|// that means hte index is consistent and recovered so we can't assert on the num recovered ops below.
try|try
init|(
name|IndexReader
name|reader
...
name|assertEquals
argument_list|(
literal|"translog ID must be incremented by 2 after initial recovery"
argument_list|,
name|currentTranslogId
operator|+
literal|2
...
argument_list|)
expr_stmt|;
comment|// we need to reuse the engine config otherwise the parser.mappingModified won't work
name|assertTrue
argument_list|(
name|currentTranslogId
operator|+
