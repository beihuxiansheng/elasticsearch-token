...
condition|)
block|{
comment|/**              * We need 4 nodes since if we have 2 replicas and only 3 nodes we can't get into green state since              * the corrupted node will never be used reallocate a replica since it's marked as corrupted              */
...
condition|)
block|{
comment|// test the dynamic setting
name|client
argument_list|()
operator|.
name|admin
...
literal|false
return|;
comment|// no dirs no write.locks
block|}
block|}
argument_list|)
argument_list|)
