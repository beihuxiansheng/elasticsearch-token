...
begin_comment
comment|// Rev 1.64 from http://gee.cs.oswego.edu/cgi-bin/viewcvs.cgi/jsr166/src/jsr166e/ForkJoinPool.java?view=co
end_comment
...
name|hint
decl_stmt|;
comment|// steal index hint
DECL|field|poolIndex
name|short
name|poolIndex
decl_stmt|;
comment|// index of this queue in pool
DECL|field|mode
specifier|final
name|short
name|mode
...
name|seed
expr_stmt|;
comment|// store initial seed for runWorker
...
return|;
block|}
comment|/**          * Tries to poll for and execute the given task or any other          * task in its CountedCompleter computation.          */
...
return|;
block|}
comment|/**          * Tries to pop and execute the given task or any other task          * in its CountedCompleter computation.          */
DECL|method|externalPopAndExecCC
specifier|final
name|boolean
name|externalPopAndExecCC
parameter_list|(
name|CountedCompleter
argument_list|<
name|?
...
return|;
block|}
comment|/**          * Internal version          */
DECL|method|internalPopAndExecCC
specifier|final
name|boolean
name|internalPopAndExecCC
parameter_list|(
name|CountedCompleter
argument_list|<
name|?
...
name|common
decl_stmt|;
comment|/**      * Common pool parallelism. To allow simpler use and management      * when common pool threads are disabled, we allow the underlying      * common.parallelism field to be zero, but in that case still report      * parallelism as 1 to reflect resulting caller-runs mechanics.      */
...
do|;
block|}
comment|/**      * Tries to create or activate a worker if too few are active.      *      * @param ws the worker array to use to find signallees      * @param q if non-null, the queue holding tasks to be processed      */
...
literal|0
condition|)
comment|// inactive or terminating
return|return
name|awaitWork
argument_list|(
name|w
...
condition|)
block|{
comment|// try to inactivate and enqueue
name|long
name|nc
init|=
operator|(
...
return|;
block|}
comment|/**      * A continuation of scan(), possibly blocking or terminating      * worker w. Returns without blocking if pool state has apparently      * changed since last invocation.  Also, if inactivating w has      * caused the pool to become quiescent, checks for pool      * termination, and, so long as this is not the only worker, waits      * for event for up to a given duration.  On timeout, if ctl has      * not changed, terminates the worker, which will in turn wake up      * another worker to possibly repeat this process.      *      * @param w the calling worker      * @param c the ctl value on entry to scan      * @param ec the worker's eventCount on entry to scan      */
DECL|method|awaitWork
specifier|private
specifier|final
name|int
name|awaitWork
...
condition|)
block|{
comment|// collect steals and retry
name|long
name|sc
decl_stmt|;
name|w
...
condition|)
block|{
comment|// timed wait if last waiter
name|int
name|dc
init|=
operator|-
...
argument_list|)
expr_stmt|;
comment|// must recheck before park
name|w
operator|.
name|parker
operator|=
...
return|;
block|}
comment|/**      * Possibly releases (signals) a worker. Called only from scan()      * when a worker with apparently inactive status finds a non-empty      * queue. This requires revalidating all of the associated state      * from caller.      */
DECL|method|helpRelease
specifier|private
specifier|final
name|void
name|helpRelease
...
return|;
block|}
comment|/**      * Analog of tryHelpStealer for CountedCompleters. Tries to steal      * and run tasks within the target's computation.      *      * @param task the task to join      */
...
return|;
block|}
comment|/**      * Tries to decrement active count (sometimes implicitly) and      * possibly release or create a compensating worker in preparation      * for blocking. Fails on contention or termination. Otherwise,      * adds a new thread if no idle workers are available and pool      * may become starved.      *      * @param c the assumed ctl value      */
...
block|}
block|}
comment|/**      * Returns a (probably) non-empty steal queue, if one is found      * during a scan, else null.  This method must be retried by      * caller if, by the time it tries to use the queue, it is empty.      */
...
return|;
block|}
comment|/**      * Creates a {@code ForkJoinPool} with the given parameters, without      * any security checks or parameter validation.  Invoked directly by      * makeCommonPool.      */
DECL|method|ForkJoinPool
specifier|private
name|ForkJoinPool
parameter_list|(
...
expr_stmt|;
block|}
comment|/**      * Returns the common pool instance. This pool is statically      * constructed; its run state is unaffected by attempts to {@link      * #shutdown} or {@link #shutdownNow}. However this pool and any      * ongoing processing are automatically terminated upon program      * {@link System#exit}.  Any program that relies on asynchronous      * task processing to complete before program termination should      * invoke {@code commonPool().}{@link #awaitQuiescence awaitQuiescence},      * before exit.      *      * @return the common pool instance      * @since 1.8      */
...
name|InterruptedException
function_decl|;
comment|/**          * Returns {@code true} if blocking is unnecessary.          * @return {@code true} if blocking is unnecessary          */
...
name|parallelism
decl_stmt|;
comment|// report 1 even if threads disabled
name|commonParallelism
operator|=
name|par
operator|>
...
expr_stmt|;
block|}
comment|/**      * Creates and returns the common pool, respecting user settings      * specified via system properties.      */
DECL|method|makeCommonPool
specifier|private
specifier|static
name|ForkJoinPool
name|makeCommonPool
...
try|try
block|{
comment|// ignore exceptions in accessing/parsing properties
name|String
name|pp
init|=
name|System
...
literal|0
operator|&&
comment|// default 1 less than #cores
operator|(
name|parallelism
operator|=
name|Runtime
