...
begin_comment
comment|/**  * A non-abstract copy of the abstract {@link BaseFragListBuilder}. that works  * in the same way as {@link SimpleFragListBuilder} but isn't prone to the  * problem of negative offsets. This is fixed in Lucene 4.3 and this class  * should be removed once Elasticsearch upgraded to Lucene 4.3  *<p>  * LUCENE-4899: FastVectorHighlihgter failed with  * {@link StringIndexOutOfBoundsException} if a single highlight phrase or term was  * greater than the fragCharSize producing negative string offsets  *</p>  */
end_comment
...
condition|)
block|{
comment|// if the phrase violates the border of previous fragment, discard
comment|// it and try next phrase
if|if
condition|(
name|phraseInfo
operator|.
