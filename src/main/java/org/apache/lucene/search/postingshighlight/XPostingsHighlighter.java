...
comment|//END EDIT
comment|//BEGIN EDIT
comment|//we need this to fix scoring when highlighting every single value separately, since the score depends on the total length of the field (all values rather than only the current one)
...
DECL|method|XPostingsHighlighter
specifier|public
name|XPostingsHighlighter
parameter_list|()
block|{
...
expr_stmt|;
block|}
comment|/** Returns the {@link java.text.BreakIterator} to use for      *  dividing text into passages.  This returns      *  {@link java.text.BreakIterator#getSentenceInstance(java.util.Locale)} by default;      *  subclasses can override to customize. */
...
return|;
block|}
comment|//BEGIN EDIT: made protected so that we can call from our subclass and pass in the terms by ourselves
...
name|IOException
block|{
comment|//private Map<Integer,Object> highlightField(String field, String contents[], BreakIterator bi, BytesRef terms[], int[] docids, List<AtomicReaderContext> leaves, int maxPassages) throws IOException {
comment|//END EDIT
name|Map
argument_list|<
name|Integer
...
name|IOException
block|{
comment|//BEGIN EDIT added call to method that returns the offset for the current value (discrete highlighting)
name|int
name|valueOffset
init|=
name|getOffsetForCurrentValue
...
throw|;
block|}
comment|//BEGIN EDIT discrete highlighting
comment|// the scoring needs to be based on the length of the whole field (all values rather than only the current one)
name|int
...
condition|)
block|{
comment|//BEGIN EDIT we take into account the length of the whole field (all values) to properly score the snippets
name|weights
index|[
name|i
index|]
...
argument_list|)
expr_stmt|;
comment|//weights[i] = scorer.weight(contentLength, de.freq());
comment|//END EDIT
name|de
operator|.
name|nextPosition
...
continue|continue;
block|}
comment|//BEGIN EDIT support for discrete highlighting (added block code)
comment|//switch to the first match in the current value if there is one
name|boolean
name|seenEnough
init|=
literal|false
...
block|}
block|}
comment|//continue with next term if we've already seen the current one all the times it appears
comment|//that means that the current value doesn't hold matches for the current term
if|if
condition|(
name|seenEnough
condition|)
...
continue|continue;
block|}
comment|//we now subtract the offset of the current value to both start and end
name|start
operator|-=
name|valueOffset
expr_stmt|;
...
comment|//BEGIN EDIT we take into account the value offset when scoring the snippet based on its position
name|current
operator|.
name|score
operator|*=
...
argument_list|)
expr_stmt|;
comment|//current.score *= scorer.norm(current.startOffset);
comment|//END EDIT
...
argument_list|()
expr_stmt|;
comment|//BEGIN EDIT support for discrete highlighting
name|start
operator|=
name|dp
operator|.
...
name|valueOffset
expr_stmt|;
comment|//start = dp.startOffset();
comment|//end = dp.endOffset();
comment|//END EDIT
block|}
if|if
condition|(
