...
begin_class
DECL|class|XReferenceManager
specifier|public
specifier|abstract
class|class
name|XReferenceManager
parameter_list|<
name|G
parameter_list|>
...
name|luceneVersion
operator|:
literal|"Remove this once we are on LUCENE_47 - see LUCENE-5436"
assert|;
block|}
...
expr_stmt|;
block|}
comment|/**    * Decrement reference counting on the given reference.    * @throws java.io.IOException if reference decrement on the given resource failed.    * */
DECL|method|decRef
specifier|protected
specifier|abstract
name|void
...
name|IOException
function_decl|;
comment|/**    * Refresh the given reference if needed. Returns {@code null} if no refresh    * was needed, otherwise a new refreshed reference.    * @throws org.apache.lucene.store.AlreadyClosedException if the reference manager has been {@link #close() closed}.    * @throws java.io.IOException if the refresh operation failed    */
...
name|IOException
function_decl|;
comment|/**    * Try to increment reference counting on the given reference. Return true if    * the operation was successful.    * @throws org.apache.lucene.store.AlreadyClosedException if the reference manager has been {@link #close() closed}.    */
...
parameter_list|)
function_decl|;
comment|/**    *  Called after close(), so subclass can free any resources.    *  @throws java.io.IOException if the after close operation in a sub-class throws an {@link java.io.IOException}    * */
...
return|;
block|}
comment|/**    * You must call this (or {@link #maybeRefresh()}), periodically, if you want    * that {@link #acquire()} will return refreshed instances.    *    *<p>    *<b>Threads</b>: unlike {@link #maybeRefresh()}, if another thread is    * currently refreshing, this method blocks until that thread completes. It is    * useful if you want to guarantee that the next call to {@link #acquire()}    * will return a refreshed instance. Otherwise, consider using the    * non-blocking {@link #maybeRefresh()}.    * @throws java.io.IOException if refreshing the resource causes an {@link java.io.IOException}    * @throws org.apache.lucene.store.AlreadyClosedException if the reference manager has been {@link #close() closed}.    */
...
block|}
block|}
comment|/** Called after a refresh was attempted, regardless of    *  whether a new reference was in fact created.    *  @throws java.io.IOException if a low level I/O exception occurs    **/
...
name|IOException
block|{   }
comment|/**    * Release the reference previously obtained via {@link #acquire()}.    *<p>    *<b>NOTE:</b> it's safe to call this after {@link #close()}.    * @throws java.io.IOException if the release operation on the given resource throws an {@link java.io.IOException}    */
DECL|method|release
specifier|public
specifier|final
name|void
