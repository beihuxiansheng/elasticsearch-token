...
begin_class
DECL|class|XAnalyzingSuggester
specifier|public
class|class
...
literal|0
expr_stmt|;
comment|// FLORIAN EDIT: I added<code>queryPrefix</code> for context dependent suggestions
name|this
operator|.
name|queryPrefix
operator|=
...
return|;
block|}
comment|/** Returns all completion paths to initialize the search. */
...
argument_list|()
decl_stmt|;
DECL|field|surfaceFormsAndPayload
specifier|private
specifier|final
name|SurfaceFormAndPayload
index|[]
name|surfaceFormsAndPayload
decl_stmt|;
DECL|field|count
specifier|private
name|int
...
argument_list|)
decl_stmt|;
comment|/*              * we need to check if we have seen this surface form, if so only use the               * the surface form with the highest weight and drop the rest no matter if               * the payload differs.              */
if|if
condition|(
name|count
operator|>=
...
return|;
block|}
DECL|method|maxSurfaceFormsPerAnalyzedForm
specifier|public
name|int
name|maxSurfaceFormsPerAnalyzedForm
parameter_list|()
