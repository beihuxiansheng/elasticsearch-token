...
expr_stmt|;
block|}
comment|/**      * We override this one so we can get the fuzzy part to be treated as string, so people can do: "age:10~5". Note,      * we would love to support also "timestamp:2012-10-10~5d", but sadly the parser expects only numbers after the ~,      * hopefully we can change that in Lucene.      */
annotation|@
name|Override
...
condition|)
block|{
comment|//            float fms = fuzzyMinSim;
comment|//            try {
comment|//                fms = Float.valueOf(fuzzySlop.image.substring(1)).floatValue();
comment|//            } catch (Exception ignored) {
comment|//            }
comment|//            if (fms< 0.0f) {
comment|//                throw new ParseException("Minimum similarity for a FuzzyQuery has to be between 0.0f and 1.0f !");
comment|//            } else if (fms>= 1.0f&& fms != (int) fms) {
comment|//                throw new ParseException("Fractional edit distances are not allowed!");
comment|//            }
comment|//            q = getFuzzyQuery(qfield, termImage, fms);
if|if
condition|(
name|fuzzySlop
operator|.
