...
return|;
block|}
comment|/**      * Checks if any new shards should be snapshotted on this node      *      * @param snapshotMetaData snapshot metadata to be processed      */
...
control|)
block|{
comment|// Check if we have new shards to start processing on
if|if
condition|(
name|localNodeId
operator|.
...
argument_list|)
expr_stmt|;
comment|// We have new snapshots to process -
if|if
condition|(
name|newSnapshots
operator|!=
...
name|snapshotId
argument_list|,
literal|"Missing routing table for index ["
operator|+
name|index
operator|+
literal|"]"
...
parameter_list|()
function_decl|;
comment|/**          * Called if delete operation failed          *          * @param t          */
DECL|method|onFailure
name|void
name|onFailure
parameter_list|(
