begin_unit|revision:0.9.5;language:Java;cregit-version:0.0.1
begin_comment
comment|/*  * Licensed to ElasticSearch and Shay Banon under one  * or more contributor license agreements. See the NOTICE file  * distributed with this work for additional information  * regarding copyright ownership. ElasticSearch licenses this  * file to you under the Apache License, Version 2.0 (the  * "License"); you may not use this file except in compliance  * with the License.  You may obtain a copy of the License at  *  *     http://www.apache.org/licenses/LICENSE-2.0  *  * Unless required by applicable law or agreed to in writing, software  * distributed under the License is distributed on an "AS IS" BASIS, WITHOUT  * WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied. See the  * License for the specific language governing permissions and limitations under  * the License.  */
end_comment

begin_package
DECL|package|org.elasticsearch.action.support.master
package|package
name|org
operator|.
name|elasticsearch
operator|.
name|action
operator|.
name|support
operator|.
name|master
package|;
end_package

begin_import
import|import
name|org
operator|.
name|elasticsearch
operator|.
name|action
operator|.
name|ActionRequest
import|;
end_import

begin_import
import|import
name|org
operator|.
name|elasticsearch
operator|.
name|common
operator|.
name|unit
operator|.
name|TimeValue
import|;
end_import

begin_comment
comment|/**  * Interface that allows to mark action requests that support acknowledgements.  * Facilitates consistency across different api.  */
end_comment

begin_interface
DECL|interface|AcknowledgedRequest
specifier|public
interface|interface
name|AcknowledgedRequest
parameter_list|<
name|T
extends|extends
name|ActionRequest
parameter_list|<
name|T
parameter_list|>
parameter_list|>
block|{
comment|/**      * Allows to set the timeout      * @param timeout timeout as a string (e.g. 1s)      * @return the request itself      */
DECL|method|timeout
name|T
name|timeout
parameter_list|(
name|String
name|timeout
parameter_list|)
function_decl|;
comment|/**      * Allows to set the timeout      * @param timeout timeout as a {@link TimeValue}      * @return the request itself      */
DECL|method|timeout
name|T
name|timeout
parameter_list|(
name|TimeValue
name|timeout
parameter_list|)
function_decl|;
comment|/**      * Returns the current timeout      * @return the current timeout as a {@link TimeValue}      */
DECL|method|timeout
name|TimeValue
name|timeout
parameter_list|()
function_decl|;
block|}
end_interface

end_unit

