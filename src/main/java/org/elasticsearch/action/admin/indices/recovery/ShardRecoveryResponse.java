begin_unit|revision:0.9.5;language:Java;cregit-version:0.0.1
begin_comment
comment|/*  * Licensed to Elasticsearch under one or more contributor  * license agreements. See the NOTICE file distributed with  * this work for additional information regarding copyright  * ownership. Elasticsearch licenses this file to you under  * the Apache License, Version 2.0 (the "License"); you may  * not use this file except in compliance with the License.  * You may obtain a copy of the License at  *  *    http://www.apache.org/licenses/LICENSE-2.0  *  * Unless required by applicable law or agreed to in writing,  * software distributed under the License is distributed on an  * "AS IS" BASIS, WITHOUT WARRANTIES OR CONDITIONS OF ANY  * KIND, either express or implied.  See the License for the  * specific language governing permissions and limitations  * under the License.  */
end_comment

begin_package
DECL|package|org.elasticsearch.action.admin.indices.recovery
package|package
name|org
operator|.
name|elasticsearch
operator|.
name|action
operator|.
name|admin
operator|.
name|indices
operator|.
name|recovery
package|;
end_package

begin_import
import|import
name|org
operator|.
name|elasticsearch
operator|.
name|action
operator|.
name|support
operator|.
name|broadcast
operator|.
name|BroadcastShardOperationResponse
import|;
end_import

begin_import
import|import
name|org
operator|.
name|elasticsearch
operator|.
name|common
operator|.
name|xcontent
operator|.
name|ToXContent
import|;
end_import

begin_import
import|import
name|org
operator|.
name|elasticsearch
operator|.
name|common
operator|.
name|xcontent
operator|.
name|XContentBuilder
import|;
end_import

begin_import
import|import
name|org
operator|.
name|elasticsearch
operator|.
name|common
operator|.
name|io
operator|.
name|stream
operator|.
name|StreamInput
import|;
end_import

begin_import
import|import
name|org
operator|.
name|elasticsearch
operator|.
name|common
operator|.
name|io
operator|.
name|stream
operator|.
name|StreamOutput
import|;
end_import

begin_import
import|import
name|org
operator|.
name|elasticsearch
operator|.
name|indices
operator|.
name|recovery
operator|.
name|RecoveryState
import|;
end_import

begin_import
import|import
name|java
operator|.
name|io
operator|.
name|IOException
import|;
end_import

begin_comment
comment|/**  * Information regarding the recovery state of a shard.  */
end_comment

begin_class
DECL|class|ShardRecoveryResponse
specifier|public
class|class
name|ShardRecoveryResponse
extends|extends
name|BroadcastShardOperationResponse
implements|implements
name|ToXContent
block|{
DECL|field|recoveryState
name|RecoveryState
name|recoveryState
decl_stmt|;
DECL|field|detailed
specifier|private
name|boolean
name|detailed
init|=
literal|false
decl_stmt|;
DECL|method|ShardRecoveryResponse
specifier|public
name|ShardRecoveryResponse
parameter_list|()
block|{ }
comment|/**      * Constructs shard recovery information for the given index and shard id.      *      * @param index     Name of the index      * @param shardId   Id of the shard      */
DECL|method|ShardRecoveryResponse
specifier|public
name|ShardRecoveryResponse
parameter_list|(
name|String
name|index
parameter_list|,
name|int
name|shardId
parameter_list|)
block|{
name|super
argument_list|(
name|index
argument_list|,
name|shardId
argument_list|)
expr_stmt|;
block|}
comment|/**      * Sets the recovery state information for the shard.      *      * @param recoveryState Recovery state      */
DECL|method|recoveryState
specifier|public
name|void
name|recoveryState
parameter_list|(
name|RecoveryState
name|recoveryState
parameter_list|)
block|{
name|this
operator|.
name|recoveryState
operator|=
name|recoveryState
expr_stmt|;
block|}
comment|/**      * Gets the recovery state information for the shard.      *      * @return  Recovery state      */
DECL|method|recoveryState
specifier|public
name|RecoveryState
name|recoveryState
parameter_list|()
block|{
return|return
name|recoveryState
return|;
block|}
DECL|method|detailed
specifier|public
name|boolean
name|detailed
parameter_list|()
block|{
return|return
name|detailed
return|;
block|}
DECL|method|detailed
specifier|public
name|void
name|detailed
parameter_list|(
name|boolean
name|detailed
parameter_list|)
block|{
name|this
operator|.
name|detailed
operator|=
name|detailed
expr_stmt|;
name|this
operator|.
name|recoveryState
operator|.
name|setDetailed
argument_list|(
name|detailed
argument_list|)
expr_stmt|;
block|}
annotation|@
name|Override
DECL|method|toXContent
specifier|public
name|XContentBuilder
name|toXContent
parameter_list|(
name|XContentBuilder
name|builder
parameter_list|,
name|Params
name|params
parameter_list|)
throws|throws
name|IOException
block|{
name|recoveryState
operator|.
name|toXContent
argument_list|(
name|builder
argument_list|,
name|params
argument_list|)
expr_stmt|;
return|return
name|builder
return|;
block|}
annotation|@
name|Override
DECL|method|writeTo
specifier|public
name|void
name|writeTo
parameter_list|(
name|StreamOutput
name|out
parameter_list|)
throws|throws
name|IOException
block|{
name|super
operator|.
name|writeTo
argument_list|(
name|out
argument_list|)
expr_stmt|;
name|recoveryState
operator|.
name|writeTo
argument_list|(
name|out
argument_list|)
expr_stmt|;
block|}
annotation|@
name|Override
DECL|method|readFrom
specifier|public
name|void
name|readFrom
parameter_list|(
name|StreamInput
name|in
parameter_list|)
throws|throws
name|IOException
block|{
name|super
operator|.
name|readFrom
argument_list|(
name|in
argument_list|)
expr_stmt|;
name|recoveryState
operator|=
name|RecoveryState
operator|.
name|readRecoveryState
argument_list|(
name|in
argument_list|)
expr_stmt|;
block|}
comment|/**      * Builds a new ShardRecoveryResponse from the give input stream.      *      * @param in    Input stream      * @return      A new ShardRecoveryResponse      * @throws IOException      */
DECL|method|readShardRecoveryResponse
specifier|public
specifier|static
name|ShardRecoveryResponse
name|readShardRecoveryResponse
parameter_list|(
name|StreamInput
name|in
parameter_list|)
throws|throws
name|IOException
block|{
name|ShardRecoveryResponse
name|response
init|=
operator|new
name|ShardRecoveryResponse
argument_list|()
decl_stmt|;
name|response
operator|.
name|readFrom
argument_list|(
name|in
argument_list|)
expr_stmt|;
return|return
name|response
return|;
block|}
block|}
end_class

end_unit

