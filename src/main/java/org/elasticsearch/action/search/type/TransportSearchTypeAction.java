...
argument_list|)
expr_stmt|;
comment|// Not so nice, but we need to know if there're nodes below the supported version
comment|// and if so fall back to classic scroll (based on from). We need to check every node
comment|// because we don't to what nodes we end up sending the request (shard may fail or relocate)
name|boolean
name|useSlowScroll
init|=
literal|false
