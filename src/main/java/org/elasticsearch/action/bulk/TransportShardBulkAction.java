...
block|}
block|}
DECL|method|applyMappingUpdate
specifier|private
name|void
name|applyMappingUpdate
parameter_list|(
name|IndexService
name|indexService
parameter_list|,
...
name|Throwable
block|{
comment|// HACK: Rivers seem to have something specific that triggers potential
comment|// deadlocks when doing concurrent indexing. So for now they keep the
comment|// old behaviour of updating mappings locally first and then
comment|// asynchronously notifying the master
comment|// this can go away when rivers are removed
specifier|final
name|String
name|indexName
init|=
