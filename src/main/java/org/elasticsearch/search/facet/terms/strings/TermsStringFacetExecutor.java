...
condition|)
block|{
comment|// TODO: we need to support this back with the new field data!
comment|//            try {
comment|//                for (AtomicReaderContext readerContext : context.searcher().getTopReaderContext().leaves()) {
comment|//                    FieldData fieldData = fieldDataCache.cache(fieldDataType, readerContext.reader(), indexFieldName);
...
parameter_list|)
block|{
comment|// we have to "makeSafe", even if it exists, since it might not..., need to find a way to optimize it
name|facets
operator|.
