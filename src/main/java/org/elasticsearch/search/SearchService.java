...
argument_list|)
expr_stmt|;
comment|// move to QUERY_THEN_FETCH, and then, when scrolling, move to SCAN
name|context
operator|.
name|size
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// set size to 0 so that we only count matches
assert|assert
name|context
operator|.
name|searchType
...
condition|)
block|{
comment|// so that the optimizations we apply to size=0 also apply to search_type=COUNT
comment|// and that we close contexts when done with the query phase
name|context
operator|.
name|searchType
argument_list|(
...
parameter_list|)
block|{
comment|// No inner template found, use original template from above
block|}
block|}
block|}
block|}
