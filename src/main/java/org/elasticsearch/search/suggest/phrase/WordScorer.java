...
function_decl|;
block|}
comment|/**     * Terms enum wrapper that caches term frequencies in an effort to outright skip seeks.  Only works with seekExact(BytesRef), not next or     * not seekCeil.  Because of this it really only makes sense in this context.     */
...
condition|)
block|{
comment|// This'll fail to work properly if the user seeks but doesn't check the frequency, causing us to cache it.
comment|// That is OK because WordScorer only seeks to check the frequency.
return|return
name|last
operator|.
name|ttf
...
condition|)
block|{
comment|// Found so mark the term uncached.
name|last
operator|.
name|df
operator|=
...
return|;
block|}
comment|// Not found.  The cache will default to 0 for the freqs, meaning not found.
return|return
literal|false
return|;
block|}
