...
block|}
block|}
DECL|method|shouldUseOrdinals
specifier|private
name|boolean
name|shouldUseOrdinals
parameter_list|(
name|Aggregator
name|parent
parameter_list|,
...
parameter_list|)
block|{
comment|// if there is a parent bucket aggregator the number of instances of this aggregator is going to be unbounded and most instances
comment|// may only aggregate few documents, so don't use ordinals
if|if
condition|(
name|hasParentBucketAggregator
argument_list|(
...
return|;
block|}
comment|// be defensive: if the number of unique values is unknown, don't use ordinals
specifier|final
name|long
...
return|;
block|}
comment|// if the number of unique values is high compared to the document count, then ordinals are only going to make things slower
name|int
name|maxDoc
init|=
literal|0
