...
condition|)
block|{
comment|// All child aggs marked as empty
for|for
control|(
name|int
name|i
...
block|}
decl_stmt|;
comment|// Add the collectable sub aggs by asking the collect tree to gather
comment|// results using ordinals that may have undergone transformation as the
comment|// result of the collection process e.g. filtering
comment|// to a subset of buckets then rebasing the numbers in the deferred collection
name|collectableSubAggregators
operator|.
name|gatherAnalysis
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// Also add the results of any non-collecting sub aggs using the top-level ordinals
for|for
control|(
name|int
name|i
...
condition|)
block|{
comment|// Agg is not part of the collect tree - call directly
name|childAggs
operator|.
name|add
argument_list|(
