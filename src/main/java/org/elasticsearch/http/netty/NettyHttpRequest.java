...
parameter_list|()
block|{
comment|// if its a copy, then its not unsafe...
if|if
condition|(
name|contentAsBytes
operator|!=
...
return|;
block|}
comment|// HttpMessageDecoder#content is sliced but out of freshly created buffers for each read
return|return
literal|false
return|;
...
argument_list|)
expr_stmt|;
comment|// clear the content, so it can be GC'ed, we make sure to work from contentAsBytes from here on
name|request
operator|.
name|setContent
argument_list|(
