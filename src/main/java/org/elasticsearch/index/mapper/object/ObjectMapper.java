...
return|;
block|}
comment|/**      * Put a new mapper.      * NOTE: this method must be called under the current {@link DocumentMapper}      * lock if concurrent updates are expected.      */
...
expr_stmt|;
block|}
comment|// add the mappers only after the administration have been done, so it will not be visible to parser (which first try to read with no lock)
for|for
control|(
name|Mapper
name|mapper
