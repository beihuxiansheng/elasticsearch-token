...
literal|false
decl_stmt|;
DECL|field|postingsProvider
specifier|protected
name|PostingsFormatProvider
name|postingsProvider
decl_stmt|;
DECL|field|docValuesProvider
specifier|protected
name|DocValuesFormatProvider
...
annotation|@
name|Nullable
DECL|method|defaultDocValuesFormat
specifier|protected
name|String
...
condition|)
block|{
comment|// indexAnalyzer == searchAnalyzer
if|if
condition|(
name|includeDefaults
operator|||
...
block|{
comment|// both are there but different
if|if
condition|(
name|includeDefaults
operator|||
...
argument_list|)
expr_stmt|;
comment|// We might be in dynamically created field already, so need to clean withinNewMapper flag
comment|// and then restore it, so we wouldn't miss new mappers created from copy_to fields
name|boolean
name|origWithinNewMapper
init|=
name|context
operator|.
