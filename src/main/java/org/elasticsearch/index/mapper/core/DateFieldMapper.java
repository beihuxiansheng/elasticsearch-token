...
parameter_list|)
block|{
comment|// If the current search context is null we're parsing percolator query or a index alias filter.
if|if
condition|(
name|SearchContext
operator|.
...
return|;
block|}
comment|/*      * `timeZone` parameter is only applied when:      * - not null      * - the object to parse is a String (does not apply to ms since epoch which are UTC based time values)      * - the String to parse does not have already a timezone defined (ie. `2014-01-01T00:00:00+03:00`)      */
DECL|method|rangeFilter
specifier|public
name|Filter
name|rangeFilter
...
operator|=
operator|!
name|hasDateExpressionWithNoRounding
argument_list|(
name|value
argument_list|)
expr_stmt|;
...
condition|)
block|{
comment|// We don't cache range filter if `now` date expression is used and also when a compound filter wraps
comment|// a range filter with a `now` date expressions.
return|return
name|NoCacheFilter
operator|.
name|wrap
...
block|}
block|}
DECL|method|hasDateExpressionWithNoRounding
specifier|private
name|boolean
name|hasDateExpressionWithNoRounding
parameter_list|(
...
literal|false
return|;
comment|// We found the rounding char, so we shouldn't forcefully disable caching
block|}
elseif|else
if|if
condition|(
...
literal|true
return|;
comment|// Next token in the date math expression and no rounding found, so we should not cache.
block|}
block|}
do|while
condition|(
...
literal|true
return|;
comment|// Couldn't find rounding char, so we should not cache
block|}
block|}
block|}
else|else
...
name|forcedDateParser
decl_stmt|;
DECL|field|explicitCaching
specifier|final
name|Boolean
name|explicitCaching
decl_stmt|;
DECL|method|LateParsingFilter
specifier|public
name|LateParsingFilter
parameter_list|(
name|IndexNumericFieldData
