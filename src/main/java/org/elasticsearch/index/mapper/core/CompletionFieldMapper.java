...
expr_stmt|;
block|}
comment|// we are just using this as the default to be wrapped by the CompletionPostingsFormatProvider in the SuggesteFieldMapper ctor
name|builder
operator|.
name|postingsFormat
argument_list|(
...
argument_list|()
decl_stmt|;
DECL|field|completionPostingsFormatProvider
specifier|private
specifier|final
name|CompletionPostingsFormatProvider
name|completionPostingsFormatProvider
...
argument_list|()
expr_stmt|;
comment|// always parse a long to make sure we don't get the overflow value
if|if
condition|(
name|weight
argument_list|<
...
parameter_list|)
block|{
comment|/* we also use 0xFF as a SEP_LABEL in the suggester but it's not valid UTF-8 so no need to check.          *  we also don't need to convert to UTF-8 here to check for the 0x00 end label since all multi-byte          *  UTF-8 chars start with 0x10 binary so if the UTF-16 CP is == 0x00 it's the single byte UTF-8 CP */
return|return
name|character
operator|==
name|END_LABEL
