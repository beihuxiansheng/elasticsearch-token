...
name|trace
argument_list|(
literal|"no translog id set (indexShouldExist [{}])"
argument_list|,
name|indexShouldExists
argument_list|)
expr_stmt|;
...
block|}
block|}
comment|// we must do this *after* we capture translog name so the engine creation will not make a new one.
comment|// also we have to do this regardless of whether we have a translog, to follow the recovery stages.
name|indexShard
operator|.
...
expr_stmt|;
block|}
comment|// we flush to trim the translog, in case it was big.
try|try
block|{
name|FlushRequest
name|flushRequest
...
parameter_list|)
block|{
comment|// to be safe we catch the FNAEX , at this point no one can recover
name|logger
operator|.
name|debug
argument_list|(
literal|"skipping flush at end of recovery (not allowed)"
argument_list|,
name|e
argument_list|)
expr_stmt|;
