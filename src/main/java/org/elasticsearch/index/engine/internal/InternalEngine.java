...
argument_list|)
expr_stmt|;
comment|// if the doc does not exist or it exists but is not deleted
if|if
condition|(
name|versionValue
operator|!=
...
return|;
block|}
DECL|method|pruneDeletedVersions
specifier|private
name|void
name|pruneDeletedVersions
parameter_list|(
...
comment|// we only need to prune deletes; the adds/updates are cleared whenever reader is refreshed:
for|for
control|(
name|Map
operator|.
...
condition|)
block|{
comment|// another thread has re-added this uid since we started refreshing:
continue|continue;
block|}
if|if
condition|(
...
name|ElasticsearchIllegalStateException
argument_list|(
literal|"The `force` flag can only be used if the merge policy is an instance of "
operator|+
name|ElasticsearchMergePolicy
operator|.
name|class
...
throw|;
block|}
comment|/*                  * The way we implement "forced forced merges" is a bit hackish in the sense that we set an instance variable and that this                  * setting will thus apply to all forced merges that will be run until `force` is set back to false. However, since                  * InternalEngine.optimize is the only place in code where we call forceMerge and since calls are protected with                  * `optimizeMutex`, this has the expected behavior.                  */
if|if
condition|(
name|optimize
operator|.
...
block|{
comment|// close the engine whatever happens...
name|close
argument_list|()
expr_stmt|;
block|}
