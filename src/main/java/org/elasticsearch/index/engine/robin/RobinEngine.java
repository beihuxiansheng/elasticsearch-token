...
name|indexWriter
decl_stmt|;
comment|// LUCENE MONITOR: 3.6 Remove using the custom SearchManager and use the Lucene 3.6 one
...
condition|)
block|{
comment|// with replicas/recovery, we only check for previous version, we allow to set a future version
if|if
condition|(
name|expectedVersion
operator|<=
...
try|try
block|{
comment|// close the reader as well, since closing the searcher does nothing
comment|// and we want to decRef the inner readers
name|newSearcher
operator|.
name|getIndexReader
argument_list|()
