...
name|translogId
decl_stmt|;
comment|// nextTranslogId, currentTranslogId
try|try
block|{
name|writer
operator|=
...
expr_stmt|;
block|}
comment|/**      * Reads the current stored translog ID (v1) from the IW commit data and generates a new/next translog ID (v2)      * from the largest present translog ID. If there is no stored translog ID v1 is<code>null</code>      */
DECL|method|loadTranslogIds
specifier|private
name|Tuple
argument_list|<
name|Long
...
name|debug
argument_list|(
literal|"no translog ID present in the current commit - creating one"
argument_list|)
expr_stmt|;
return|return
operator|new
