...
init|)
block|{
comment|// if we close the dir we need to make sure nobody writes checksums
try|try
block|{
name|directory
operator|.
...
argument_list|()
expr_stmt|;
comment|// don't call close here we throw an exception there!
block|}
catch|catch
parameter_list|(
name|IOException
...
name|out
decl_stmt|;
comment|// TODO: cut over to lucene's CRC
comment|// *WARNING*: lucene has classes in same o.a.l.store package with very similar names,
comment|// but using CRC, not Adler!
if|if
condition|(
name|underlying
operator|instanceof
