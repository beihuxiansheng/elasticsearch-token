...
begin_comment
comment|/**  * A state-full lightweight per document set of<code>long</code> values.  *  * To iterate over values in a document use the following pattern:  *<pre>  *   LongValues values = ..;  *   final int numValues = values.setDocId(docId);  *   for (int i = 0; i< numValues; i++) {  *       long value = values.nextValue();  *       // process value  *   }  *</pre>  *  */
end_comment
...
name|LongValues
block|{
comment|/**      * An empty {@link LongValues instance}      */
DECL|field|EMPTY
specifier|public
specifier|static
specifier|final
...
name|docId
decl_stmt|;
comment|/**      * Creates a new {@link LongValues} instance      * @param multiValued<code>true</code> iff this instance is multivalued. Otherwise<code>false</code>.      */
...
parameter_list|()
function_decl|;
comment|/**      * Returns the order the values are returned from {@link #nextValue()}.      *<p> Note: {@link LongValues} have {@link AtomicFieldData.Order#NUMERIC} by default.</p>      */
...
return|;
block|}
comment|/**      * Ordinal based {@link LongValues}.      */
...
return|;
block|}
comment|/**          * Returns the value for the given ordinal.          * @param ord the ordinal to lookup.          * @return a long value associated with the given ordinal.          */
...
name|ElasticsearchIllegalStateException
argument_list|(
literal|"Empty LongValues has no next value"
argument_list|)
throw|;
block|}
block|}
comment|/** Wrap a {@link DoubleValues} instance. */
DECL|method|asLongValues
specifier|public
specifier|static
name|LongValues
name|asLongValues
