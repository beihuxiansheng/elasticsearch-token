...
return|;
block|}
comment|/**      * Check if the given iterator can nextDoc() or advance() in sub-linear time      * of the number of documents. For instance, an iterator that would need to      * iterate one document at a time to check for its value would be considered      * broken.      */
...
condition|)
block|{
comment|// this iterator is filtered (likely by some bits)
comment|// unwrap in order to check if the underlying iterator is fast
name|iterator
operator|=
operator|(
operator|(
...
return|;
block|}
comment|// DocValuesDocIdSet produces anonymous slow iterators
if|if
condition|(
name|iterator
operator|!=
...
return|;
block|}
comment|/**      * Given a {@link DocIdSet}, return a {@link Bits} instance that will match      * all documents contained in the set. Note that the returned {@link Bits}      * instance should only be consumed once and in order.      */
