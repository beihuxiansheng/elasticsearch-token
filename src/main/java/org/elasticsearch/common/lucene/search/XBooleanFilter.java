...
return|;
block|}
comment|// now, go over the clauses and apply the "fast" ones first...
name|hasNonEmptyShouldClause
operator|=
literal|false
expr_stmt|;
...
literal|false
decl_stmt|;
comment|// But first we need to handle the "fast" should clauses, otherwise a should clause can unset docs
comment|// that don't match with a must or must_not clause.
name|List
argument_list|<
name|ResultClause
argument_list|>
...
block|}
block|}
comment|// Now we safely handle the "fast" must and must_not clauses.
for|for
control|(
name|int
name|i
...
argument_list|()
decl_stmt|;
name|at_least_one_should_clause_iter
label|:
for|for
control|(
name|int
...
comment|// is already a must or must_not clause. However in the current ES bool filter behaviour at least one should
comment|// clause must match in order for a doc to be a match. What we do here is checking if matched docs match with
comment|// any should filter. TODO: Add an option to have disable minimum_should_match=1 behaviour
if|if
condition|(
operator|!
name|slowOrClauses
