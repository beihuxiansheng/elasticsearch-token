...
expr_stmt|;
block|}
comment|// we only have a single page
if|if
condition|(
name|pos
operator|==
...
operator|)
assert|;
comment|// finally write all buffers
name|channel
operator|.
name|write
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// ByteArray.get(BytesRef) does not work since it flips the
comment|// ref's byte[] pointer, so for now we copy byte-by-byte
name|int
name|written
init|=
literal|0
