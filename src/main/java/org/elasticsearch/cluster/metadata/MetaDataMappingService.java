...
literal|true
decl_stmt|;
comment|// if its an update task, make sure we only process the latest ordered one per node
if|if
condition|(
name|task
operator|instanceof
...
name|task
decl_stmt|;
comment|// we can only do something to compare if we have the order&& node
if|if
condition|(
name|uTask
operator|.
...
condition|)
block|{
comment|// if we have the order, and the node id, then we can compare, and replace if applicable
if|if
condition|(
name|eTask
operator|.
...
condition|)
block|{
comment|// a newer update task, we can replace so we execute it one!
name|tasks
operator|.
name|set
argument_list|(
...
block|}
block|}
comment|// fork sending back updates, so we won't wait to send them back on the cluster state, there
comment|// might be a few of those...
name|threadPool
operator|.
name|generic
argument_list|()
