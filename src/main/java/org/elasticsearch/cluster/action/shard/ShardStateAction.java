...
argument_list|()
decl_stmt|;
DECL|field|rerouteRequired
specifier|private
specifier|final
name|AtomicBoolean
name|rerouteRequired
init|=
operator|new
name|AtomicBoolean
argument_list|()
...
expr_stmt|;
block|}
comment|// we don't do reroute right away, we do it after publishing the fact that it was started
name|RoutingAllocation
operator|.
name|Result
name|routingResult
...
name|submitStateUpdateTask
argument_list|(
literal|"reroute post shard-started ("
operator|+
name|shardRouting
operator|+
literal|"), reason ["
