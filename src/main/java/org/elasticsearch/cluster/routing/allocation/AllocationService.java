...
argument_list|()
operator|+
literal|" is not assigned to a node, but listed on as existing on node ["
operator|+
name|routingNode
operator|.
name|nodeId
...
expr_stmt|;
block|}
comment|// now, go over shards that are initializing and recovering from primary shards that are now down...
for|for
control|(
name|RoutingNode
name|routingNode
...
throw|;
block|}
comment|// we always recover from primaries, so we care about replicas that are not primaries
if|if
condition|(
name|shardRoutingEntry
operator|.
...
continue|continue;
block|}
comment|// if its not initializing, then its not recovering from the primary
if|if
condition|(
operator|!
name|shardRoutingEntry
...
continue|continue;
block|}
comment|// its initializing because its relocating from another node (its replica recovering from another replica)
if|if
condition|(
name|shardRoutingEntry
operator|.
...
control|)
block|{
comment|// double check on the unassignedShardRouting.primary(), but it has to be a primary... (well, we double checked actually before...)
if|if
condition|(
name|unassignedShardRouting
operator|.
