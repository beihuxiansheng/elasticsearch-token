...
literal|true
expr_stmt|;
comment|// we only persist shards state for data nodes
if|if
condition|(
operator|!
name|clusterService
...
argument_list|)
expr_stmt|;
comment|// ugly, but, if we fail to read it, bail completely so we don't have any node corrupting the cluster
name|System
operator|.
name|exit
argument_list|(
