...
argument_list|)
decl_stmt|;
comment|// If the user explicitly did not define a readonly value, we set it by ourselves depending on the location mode setting.
comment|// For secondary_only setting, the repository should be read only
if|if
condition|(
name|forcedReadonly
operator|==
...
operator|+
literal|" creating an azure snapshot repository backed by it."
argument_list|)
throw|;
block|}
block|}
