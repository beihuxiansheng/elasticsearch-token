...
parameter_list|()
block|{      }
DECL|field|isAvailableProcessorsSet
specifier|private
specifier|static
name|AtomicBoolean
name|isAvailableProcessorsSet
init|=
operator|new
name|AtomicBoolean
argument_list|()
decl_stmt|;
comment|/**      * Set the number of available processors that Netty uses for sizing various resources (e.g., thread pools).      *      * @param availableProcessors the number of available processors      * @throws IllegalStateException if available processors was set previously and the specified value does not match the already-set value      */
DECL|method|setAvailableProcessors
specifier|public
specifier|static
name|void
name|setAvailableProcessors
parameter_list|(
specifier|final
name|int
...
parameter_list|)
block|{
comment|// we set this to false in tests to avoid tests that randomly set processors from stepping on each other
specifier|final
name|boolean
name|set
init|=
...
name|getProperty
argument_list|(
literal|"es.set.netty.runtime.available.processors"
argument_list|,
literal|"true"
argument_list|)
argument_list|)
...
return|return;
block|}
comment|/*          * This can be invoked twice, once from Netty4Transport and another time from Netty4HttpServerTransport; however,          * Netty4Runtime#availableProcessors forbids settings the number of processors twice so we prevent double invocation here.          */
if|if
condition|(
name|isAvailableProcessorsSet
operator|.
...
condition|)
block|{
comment|/*              * We have previously set the available processors yet either we are trying to set it to a different value now or there is a bug              * in Netty and our previous value did not take, bail.              */
specifier|final
name|String
name|message
init|=
...
name|ROOT
argument_list|,
literal|"available processors value [%d] did not match current value [%d]"
argument_list|,
name|availableProcessors
argument_list|,
name|NettyRuntime
