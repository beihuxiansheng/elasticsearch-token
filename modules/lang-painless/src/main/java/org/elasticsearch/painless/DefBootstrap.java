...
literal|0
decl_stmt|;
comment|/**      * static bootstrap parameter indicating the binary operator is part of compound assignment (e.g. +=).      *<p>      * may require {@link MethodHandles#explicitCastArguments}, or a dynamic cast      * to cast back to the receiver's type, depending on types seen.      */
...
literal|1
decl_stmt|;
comment|/**      * static bootstrap parameter indicating an explicit cast to the return type.      *<p>      * may require {@link MethodHandles#explicitCastArguments}, depending on types seen.      */
DECL|field|OPERATOR_EXPLICIT_CAST
specifier|public
specifier|static
specifier|final
name|int
...
condition|)
block|{
comment|// static cast to the return type
name|target
operator|=
name|DefMath
operator|.
...
condition|)
block|{
comment|// dynamic cast to the receiver's type
name|target
operator|=
name|DefMath
operator|.
...
argument_list|)
decl_stmt|;
comment|// for math operators: WrongMethodType can be confusing. convert into a ClassCastException if they screw up.
try|try
block|{
name|target
operator|=
...
name|BootstrapMethodError
argument_list|(
literal|"This parameter is only supported for BINARY/SHIFT_OPERATORs"
argument_list|)
throw|;
block|}
return|return
