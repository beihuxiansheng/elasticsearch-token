...
condition|)
block|{
comment|/* Versions of Elasticsearch before 5.0 couldn't parse nanos or micros                  * so we toss out that resolution, rounding up because more scroll                  * timeout seems safer than less. */
name|keepAlive
operator|=
name|timeValueMillis
argument_list|(
...
condition|)
block|{
comment|// Versions before 1.0 don't support `"_source": true` so we have to ask for the source as a stored field.
name|entity
operator|.
name|field
argument_list|(
...
condition|)
block|{
comment|/* Versions of Elasticsearch before 5.0 couldn't parse nanos or micros              * so we toss out that resolution, rounding up so we shouldn't end up              * with 0s. */
name|keepAlive
operator|=
name|timeValueMillis
argument_list|(
