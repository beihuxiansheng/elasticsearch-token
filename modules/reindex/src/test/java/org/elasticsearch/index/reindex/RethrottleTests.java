...
block|{
comment|/* Check that at least one slice was rethrottled. We won't always rethrottle all of them because they might have completed.              * With multiple slices these numbers might not add up perfectly, thus the 1.01F. */
name|long
name|unfinished
init|=
name|status
...
decl_stmt|;
name|float
name|maxExpectedSliceRequestsPerSecond
init|=
name|newRequestsPerSecond
operator|==
name|Float
...
decl_stmt|;
name|float
name|minExpectedSliceRequestsPerSecond
init|=
name|newRequestsPerSecond
operator|==
name|Float
...
condition|)
block|{
comment|// This slice reports as not having completed so it should have been processed.
name|assertThat
argument_list|(
name|slice
operator|.
