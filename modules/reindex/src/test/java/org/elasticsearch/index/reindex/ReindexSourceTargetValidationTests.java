...
begin_comment
comment|/**  * Tests source and target index validation of reindex. Mostly that means testing that indexing from an index back into itself fails the  * request. Note that we can't catch you trying to remotely reindex from yourself into yourself. We actually assert here that reindexes  * from remote don't need to come from existing indexes. It'd be silly to fail requests if the source index didn't exist on the target  * cluster....  */
end_comment

begin_class
DECL|class|ReindexSourceTargetValidationTests
specifier|public
class|class
name|ReindexSourceTargetValidationTests
extends|extends
name|ESTestCase
block|{
DECL|field|STATE
...
expr_stmt|;
block|}
DECL|method|testRemoteInfoSkipsValidation
specifier|public
name|void
name|testRemoteInfoSkipsValidation
parameter_list|()
block|{
comment|// The index doesn't have to exist
name|succeeds
argument_list|(
operator|new
name|RemoteInfo
...
argument_list|)
expr_stmt|;
comment|// And it doesn't matter if they are the same index. They are considered to be different because the remote one is, well, remote.
name|succeeds
argument_list|(
operator|new
name|RemoteInfo
