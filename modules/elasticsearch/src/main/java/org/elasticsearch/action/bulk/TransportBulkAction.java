...
name|request
decl_stmt|;
comment|// handle routing& timestamp
name|boolean
name|needToParseExternalTimestamp
init|=
name|indexRequest
operator|.
name|timestamp
...
block|}
block|}
comment|// Try to parse external timestamp if necessary with no mapping
if|if
condition|(
name|needToParseExternalTimestamp
condition|)
...
expr_stmt|;
block|}
comment|// The timestamp has not been set neither externally nor in the source doc so we generate it
if|if
condition|(
name|indexRequest
operator|.
