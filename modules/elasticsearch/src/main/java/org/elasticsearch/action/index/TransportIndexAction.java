...
condition|)
block|{
comment|// Try to parse externally provided timestamp if necessary (with a mapping)
if|if
condition|(
name|needToParseExternalTimestamp
condition|)
...
literal|false
expr_stmt|;
comment|// parseTimestamp throws an exception if something gone wrong
block|}
name|request
operator|.
...
block|}
block|}
comment|// Try to parse external timestamp if necessary (without mapping)
if|if
condition|(
name|needToParseExternalTimestamp
condition|)
...
expr_stmt|;
block|}
comment|// The timestamp has not been sets neither externally nor in the source doc so we generate it
if|if
condition|(
name|request
operator|.
