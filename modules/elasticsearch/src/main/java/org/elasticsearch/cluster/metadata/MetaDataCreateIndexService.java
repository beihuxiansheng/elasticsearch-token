...
expr_stmt|;
block|}
comment|// initialize the counter only for nodes the shards are allocated to
if|if
condition|(
name|updatedState
operator|.
...
control|)
block|{
comment|// if we have a routing for this shard on a node, and its not the master node (since we already created
comment|// an index on it), then add it
if|if
condition|(
name|shardRouting
operator|.
