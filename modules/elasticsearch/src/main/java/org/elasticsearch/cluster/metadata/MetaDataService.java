...
argument_list|)
decl_stmt|;
comment|// do the reroute after indices have been created on all the other nodes so we can query them for some info
name|clusterService
operator|.
name|submitStateUpdateTask
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// wait till it got processed (on the master, we are the master)
try|try
block|{
name|latch2
operator|.
