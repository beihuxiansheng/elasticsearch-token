...
return|;
block|}
comment|// build connection(s) to the node
name|ArrayList
argument_list|<
name|Channel
argument_list|>
...
condition|)
block|{
comment|// we failed to connect, check if we need to bail or retry
if|if
condition|(
name|i
operator|==
...
block|{
comment|// break out of the retry loop, try another connection
break|break;
block|}
block|}
else|else
...
block|}
block|}
comment|// we got a connection, add it to our connections
name|Channel
name|channel
init|=
name|channelFuture
