...
comment|// checking if the index was completely relocated to another nodes but left unused shards
for|for
control|(
name|Map
operator|.
...
control|)
block|{
comment|// The shard is not yet started - we cannot make determination if it's used or not yet
if|if
condition|(
operator|!
name|routing
...
argument_list|()
decl_stmt|;
comment|// Check if shard is active on the current node or is getting relocated to the current node
if|if
condition|(
name|localNodeId
operator|.
...
condition|)
block|{
comment|// Shard is used locally - keep it
name|shardCanBeDeleted
operator|=
literal|false
expr_stmt|;
