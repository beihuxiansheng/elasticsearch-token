...
name|MergeMappingException
argument_list|(
literal|"Can't merge an object mapping ["
operator|+
name|mergeWithMapper
operator|.
name|name
argument_list|()
operator|+
literal|"] into a non object mapper"
argument_list|)
throw|;
block|}
operator|(
...
block|{
comment|// not an object mapper, bail if we have it, otherwise, add
comment|// we might get fancy later on and allow per field mapper merge
if|if
condition|(
name|mappers
operator|.
...
condition|)
block|{
comment|// simple ignore internal mappings
block|}
else|else
block|{
if|if
