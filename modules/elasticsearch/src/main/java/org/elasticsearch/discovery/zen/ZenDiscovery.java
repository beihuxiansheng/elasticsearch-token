...
condition|)
block|{
comment|// do the join on a different thread
name|threadPool
operator|.
name|execute
argument_list|(
...
name|masterNode
init|=
name|broadPingTillMasterResolved
argument_list|()
decl_stmt|;
if|if
condition|(
...
condition|)
block|{
comment|// we are the master (first)
name|this
operator|.
name|firstMaster
operator|=
...
argument_list|(
literal|"zen-disco-initial_connect(master)"
argument_list|,
operator|new
...
name|warn
argument_list|(
literal|"Failed to send join request to master [{}], retrying..."
argument_list|,
name|e
argument_list|,
name|masterNode
...
continue|continue;
block|}
comment|// cool, we found a master, start an FD on it
name|masterFD
operator|.
name|start
argument_list|(
...
name|submitStateUpdateTask
argument_list|(
literal|"zen-disco-elected_as_master(old master ["
operator|+
name|masterNode
operator|+
literal|"])"
...
condition|)
block|{
comment|// we are not the master, start FD against the possible master
name|masterFD
operator|.
name|restart
argument_list|(
...
block|}
block|}
DECL|method|handleNewClusterState
name|void
name|handleNewClusterState
parameter_list|(
specifier|final
name|ClusterState
name|clusterState
...
block|}
block|}
DECL|method|broadPingTillMasterResolved
specifier|private
name|DiscoveryNode
name|broadPingTillMasterResolved
parameter_list|()
