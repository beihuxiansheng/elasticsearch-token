...
begin_comment
comment|/**  * Binds types (picked using a Matcher) to an type listener. Registrations are created explicitly in  * a module using {@link org.elasticsearch.util.guice.inject.Binder#bindListener(Matcher, TypeListener)} statements:  *  *<pre>  *     register(only(new TypeLiteral&lt;PaymentService&lt;CreditCard>>() {}), listener);</pre>  *  * @author jessewilson@google.com (Jesse Wilson)  * @since 2.0  */
end_comment
