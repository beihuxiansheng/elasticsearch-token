...
name|client
decl_stmt|;
comment|//we don't rely on default headers supported by HttpClient as those cannot be replaced, plus it would get hairy
comment|//when we create the HttpClient instance on our own as there would be two different ways to set the default headers.
...
block|}
block|}
comment|//we get here only when we tried all nodes and they all failed
assert|assert
name|lastSeenException
operator|!=
literal|null
...
expr_stmt|;
block|}
comment|/**      * Sets a {@link FailureListener} to be notified each and every time a host fails      */
...
expr_stmt|;
block|}
comment|/**          * Sets the http client. A new default one will be created if not          * specified, by calling {@link #createDefaultHttpClient(Registry)})}.          *          * @see CloseableHttpClient          */
...
return|;
block|}
comment|/**          * Creates a {@link CloseableHttpClient} with default settings. Used when the http client instance is not provided.          *          * @see CloseableHttpClient          */
